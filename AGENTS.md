...
- Keep delivery vocabulary explicit. Prompts steer by default and promote at the next safe provider-turn boundary while the current drain requires continuation. An explicit `queue` input promotes one at a time at the next provider-turn boundary after in-flight tool calls settle — it does not wait for the Session to become idle; reevaluate continuation before promoting another. Steers take precedence over queued inputs at every boundary. Promoting any new user input resets the selected agent's provider-turn allowance; a batch of steers resets it once.
