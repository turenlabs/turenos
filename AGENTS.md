...
- `AGENTS.md` files (these instructions) and the `CLAUDE.md` beside each one, which only imports it for Claude Code, follow `.agents/skills/turen-context/SKILL.md`; write rules only in `AGENTS.md`, and run `bun .agents/skills/turen-context/scripts/check.ts` after changing any of them.
