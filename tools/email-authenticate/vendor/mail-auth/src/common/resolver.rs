...
use crate::Instant;
use crate::{
    DnsError, Error, IpLookupStrategy, MX, MessageAuthenticator, RecordSet, ResolverCache, Txt,
    dkim::{Atps, DomainKeyReport},
    dmarc::Dmarc,
    mta_sts::{MtaSts, TlsRpt},
    spf::{Macro, Spf},
};
#[cfg(not(feature = "dns-offline"))]
use crate::DnssecStatus;
#[cfg(all(not(feature = "dns-doh"), not(feature = "dns-offline")))]
...
impl MessageAuthenticator {
    ...
    pub async fn txt_raw_lookup(&self, key: impl ToFqdn) -> crate::Result<Vec<u8>> {
        Err(offline_miss(key.to_fqdn().as_ref()))
    }
    ...
    pub async fn txt_lookup<T: TxtRecordParser + Into<Txt> + UnwrapTxtRecord>(
        ...
        let key = key.to_fqdn();
        if let Some(value) = cache.as_ref().and_then(|cache| cache.get::<str>(key.as_ref())) {
            return T::unwrap_txt(value);
        }
        Err(offline_miss(key.as_ref()))
    }
    ...
    pub async fn mx_lookup(
        ...
        let key = key.to_fqdn();
        cache
            .and_then(|cache| cache.get::<str>(key.as_ref()))
            .ok_or_else(|| offline_miss(key.as_ref()))
    }
    ...
    pub async fn ip_lookup(
        ...
        match strategy {
            IpLookupStrategy::Ipv6Only => self.ipv6_lookup(key, cache_ipv6).await.map(|records| {
                records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()
            }),
            IpLookupStrategy::Ipv4Only => self.ipv4_lookup(key, cache_ipv4).await.map(|records| {
                records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()
            }),
            IpLookupStrategy::Ipv4thenIpv6 => match self.ipv4_lookup(key, cache_ipv4).await {
                Ok(records) => Ok(records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()),
                Err(_) => self.ipv6_lookup(key, cache_ipv6).await.map(|records| {
                    records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()
                }),
            },
            IpLookupStrategy::Ipv6thenIpv4 => match self.ipv6_lookup(key, cache_ipv6).await {
                Ok(records) => Ok(records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()),
                Err(_) => self.ipv4_lookup(key, cache_ipv4).await.map(|records| {
                    records.rrset.iter().take(max_results).copied().map(IpAddr::from).collect()
                }),
            },
        }
    ...
    pub async fn ptr_lookup(
        ...
        cache
            ...
            .ok_or_else(|| offline_miss(&addr.to_string()))
    }
    ...
    pub async fn exists(
        ...
        let key = key.to_fqdn();
        if cache_ipv4.and_then(|cache| cache.get::<str>(key.as_ref())).is_some()
            || cache_ipv6.and_then(|cache| cache.get::<str>(key.as_ref())).is_some()
        {
            return Ok(true);
        }
        Err(offline_miss(key.as_ref()))
...
#[cfg(feature = "dns-offline")]
fn offline_miss(key: &str) -> Error {
    Error::Dns(DnsError::Resolver(format!("offline_snapshot_miss:{key}")))
}
