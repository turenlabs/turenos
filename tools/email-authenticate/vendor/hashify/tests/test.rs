...
fn lookup_entity_tiny(key: &[u8]) -> Option<u32> {
...
fn main() {
    ...
    for entitiy in entities {
        ...
        let expected = *ENTITIES.get(entitiy).unwrap();
        assert_eq!(expected, *lookup_entity(entitiy.as_bytes()).unwrap());
        assert_eq!(expected, lookup_entity_tiny(entitiy.as_bytes()).unwrap());
    }
