...
await writeFile(
  ...
  `${JSON.stringify(
    {
      name: "@turenlabs/installer-inspect-wasm",
      version: "0.1.0-turen.1",
      private: true,
      type: "module",
      description: "TurenOS bounded Windows installer (MSI/CAB) inspection and single-entry extraction",
      main: "dist/turen_installer_inspect_wasm.js",
      types: "dist/turen_installer_inspect_wasm.d.ts",
      exports: {
        ".": {
          types: "./dist/turen_installer_inspect_wasm.d.ts",
          import: "./dist/turen_installer_inspect_wasm.js",
          default: "./dist/turen_installer_inspect_wasm.js",
        },
      },
...
async function filesUnder(directory, prefix = "") {
  ...
  return (
    await Promise.all(
      entries.map((entry) =>
        entry.isDirectory() ? filesUnder(directory, path.join(prefix, entry.name)) : [path.join(prefix, entry.name)],
      ),
    )
  ).flat()
}
