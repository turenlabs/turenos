...
>(
  ...
  const _with = ((...args: WithSubquery[]) => {
    const read = getReplica(replicas).with(...args)
    const write = primary.with(...args)
    return { ...read, update: write.update, insert: write.insert, delete: write.delete }
  }) as Q["with"]
  const $with = ((...args: [string] | [string, ColumnsSelection]) =>
    args.length === 1 ? primary.$with(args[0]) : primary.$with(args[0], args[1])) as Q["$with"]
...
export const isWithReplicas = <Q extends object>(
  database: Q | SQLiteEffectWithReplicas<Q>,
): database is SQLiteEffectWithReplicas<Q> => "$replicas" in database
