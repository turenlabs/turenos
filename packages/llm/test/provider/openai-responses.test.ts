...
describe("OpenAI Responses route", () => {
  ...
  it.effect("never ships max_output_tokens to a model whose compatibility forbids it", () =>
    Effect.gen(function* () {
      // The ChatGPT Codex endpoint answers max_output_tokens with HTTP 400. Compaction's
      // summarizer and the runner's output clamp both set generation.maxTokens, so overflow
      // recovery on a ChatGPT sign-in survives only because this strip holds.
      const codex = OpenAIResponses.route
        .with({ endpoint: { baseURL: "https://chatgpt.test/backend-api/codex/" }, auth: Auth.bearer("test") })
        .model({ id: "gpt-5.6-luna", compatibility: { maxOutputTokens: false } })
      const prepared = yield* LLMClient.prepare(
        LLM.updateRequest(request, { model: codex, generation: { maxTokens: 4_096 } }),
      )

      // The in-memory body carries the key as `undefined`; what matters is the wire form,
      // where JSON serialization drops it.
      expect(JSON.stringify(prepared.body)).not.toContain("max_output_tokens")
      expect(prepared.body).toMatchObject({ model: "gpt-5.6-luna" })
    }),
  )

  it.effect("lowers GPT-5.6 service tier, effort, mode, and persisted reasoning context", () =>
    Effect.gen(function* () {
      const input = LLM.updateRequest(request, {
        model: OpenAI.configure({ baseURL: "https://api.openai.test/v1/", apiKey: "test" }).responses("gpt-5.6-sol"),
        providerOptions: {
          openai: {
            serviceTier: "fast",
            reasoningEffort: "max",
            ...
            open: (input) =>
              Effect.succeed({
                ...
                messages: Stream.fromArray([
                  ...
                  ProviderShared.encodeJson({ type: "response.done", response: { id: "resp_ws" } }),
                ]),
                close: Effect.sync(() => {
                  closed = true
                }),
  ...
  it.effect("aliases overlong historical call IDs without breaking call-result pairing", () =>
    Effect.gen(function* () {
      const overlongCallID = `call_${"x".repeat(63)}`
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model,
          messages: [
            Message.assistant([
              ToolCallPart.make({ id: "forge_call_1", name: "first", input: {} }),
              ToolCallPart.make({ id: overlongCallID, name: "second", input: {} }),
            ]),
            Message.tool({ id: "forge_call_1", name: "first", result: "first result" }),
            Message.tool({ id: overlongCallID, name: "second", result: "second result" }),
          ],
        }),
      )

      expect(prepared.body.input).toEqual([
        { type: "function_call", call_id: "forge_call_1", name: "first", arguments: "{}" },
        { type: "function_call", call_id: "forge_call_2", name: "second", arguments: "{}" },
        { type: "function_call_output", call_id: "forge_call_1", output: '"first result"' },
        { type: "function_call_output", call_id: "forge_call_2", output: '"second result"' },
      ])
      expect(overlongCallID).toHaveLength(68)
    }),
  )
  ...
  it.effect("uses the authoritative function-call arguments done event", () =>
    Effect.gen(function* () {
      const finalInput = JSON.stringify({
        todos: [{ content: "Keep the closing brace } literal", status: "pending", priority: "high" }],
      })
      const body = sseEvents(
        {
          type: "response.output_item.added",
          item: { type: "function_call", id: "item_todo", call_id: "call_todo", name: "todowrite", arguments: "" },
        },
        { type: "response.function_call_arguments.delta", item_id: "item_todo", delta: '{"todos":[' },
        { type: "response.function_call_arguments.done", item_id: "item_todo", arguments: finalInput },
        {
          type: "response.output_item.done",
          item: {
            type: "function_call",
            ...
            call_id: "call_todo",
            name: "todowrite",
            arguments: "{stale provider snapshot",
          },
        },
      ...
      const response = yield* LLMClient.generate(
        LLM.updateRequest(request, {
          tools: [{ name: "todowrite", description: "Update todos", inputSchema: { type: "object" } }],
        }),
      ).pipe(Effect.provide(fixedResponse(body)))
      ...
      expect(response.toolCalls).toEqual([
        expect.objectContaining({
          id: "call_todo",
          name: "todowrite",
          input: { todos: [{ content: "Keep the closing brace } literal", status: "pending", priority: "high" }] },
        }),
      ])
    }),
  )

  it.effect("surfaces malformed function-call JSON as a recoverable tool error", () =>
    Effect.gen(function* () {
      const body = sseEvents(
        {
          type: "response.output_item.added",
        ...
        },
        {
          ...
          item_id: "item_patch",
          arguments: '{"patchText":"unterminated',
        },
        {
          type: "response.output_item.done",
          item: {
      ...
      const response = yield* LLMClient.generate(
        LLM.updateRequest(request, {
          tools: [{ name: "apply_patch", description: "Apply a patch", inputSchema: { type: "object" } }],
        }),
      ).pipe(Effect.provide(fixedResponse(body)))

      expect(response.events.filter((event) => event.type === "tool-error")).toEqual([
        expect.objectContaining({
          id: "call_patch",
          name: "apply_patch",
          message: expect.stringContaining("Regenerate this tool call with valid JSON"),
        }),
      ])
      expect(response.events.filter((event) => event.type === "tool-call")).toEqual([])
      expect(response.finishReason).toBe("tool-calls")
    }),
  )

  it.effect("keeps stream transport failures distinct from invalid JSON", () =>
    Effect.gen(function* () {
      ...
      expect(error.reason).toMatchObject({ _tag: "Transport", kind: "Stream" })
      expect(error.message).toContain("Failed to read openai/openai-responses stream")
    }),
  )

  it.effect("preserves the nested socket reason when a response body closes", () =>
    Effect.gen(function* () {
      const socket = Object.assign(new Error("other side closed"), { code: "UND_ERR_SOCKET" })
      const terminated = Object.assign(new Error("terminated"), { cause: socket })
      const decode = Object.assign(
        new Error("Decode error (200 POST https://chatgpt.com/backend-api/codex/responses)"),
        { cause: terminated },
      )
      const error = yield* LLMClient.generate(request).pipe(
        Effect.provide(
      ...
      expect(error.reason.message).toContain("Decode error (200 POST https://chatgpt.com/backend-api/codex/responses)")
      expect(error.reason.message).toContain("terminated: other side closed: UND_ERR_SOCKET")
    }),
  )

  it.effect("redacts secret query values from stream diagnostics", () =>
    Effect.gen(function* () {
      const secret = "stream-secret-value"
      const queryRequest = LLM.updateRequest(request, { http: { query: { api_key: secret } } })
      const failure = new Error(`stream failed at https://api.openai.test/v1/responses?api_key=${secret}`)
      const error = yield* LLMClient.generate(queryRequest).pipe(
        Effect.provide(
          truncatedStream(
            [`data: ${JSON.stringify({ type: "response.output_text.delta", item_id: "msg_1", delta: "Hello" })}\n\n`],
            failure,
  ...
  it.effect("decodes current hosted computer calls and nullable code input", () =>
    Effect.gen(function* () {
      const code = {
        type: "code_interpreter_call",
        id: "ci_null",
      ...
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents(
              { type: "response.output_item.done", item: code },
              { type: "response.output_item.done", item: computer },
              { type: "response.completed", response: { usage: { input_tokens: 1, output_tokens: 1 } } },
            ),
          ),
        ),
      ...
      expect(response.toolCalls).toEqual([
        expect.objectContaining({
          id: "ci_null",
          name: "code_interpreter",
          input: { code: null, container_id: "cnt_xyz" },
        }),
        expect.objectContaining({ id: "computer_1", name: "computer_use", input: { type: "screenshot" } }),
      ])
    }),
  )
  ...
  it.effect("includes the response payload when response.failed has no error payload", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
      ...
      expect(response.events).toEqual([
        {
          type: "provider-error",
          message: 'OpenAI Responses response failed: {"response":{"id":"resp_failed_3"}}',
        },
      ])
    }),
  )
