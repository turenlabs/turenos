...
describe("OpenAI Chat route", () => {
  ...
  it.effect("keeps runtime notes native and chronological without changing prior inputs", () =>
    Effect.gen(function* () {
      ...
      const prior = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(LLM.request({ model, messages }))
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model,
          messages: [
      ...
      expect(prior.body.messages).toEqual([
        { role: "user", content: "Before." },
        { role: "tool", tool_call_id: "call_1", content: JSON.stringify("Tool output.") },
      ])
    }),
  )

  it.effect("flushes tool images before a native runtime note", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model,
          messages: [
            ...
            Message.make({ role: "system", content: "Runtime.", metadata: { forge: { internalContext: "runtime" } } }),
            Message.user("Next."),
          ],
        }),
      )
      expect(prepared.body.messages).toEqual([
        ...
        { role: "user", content: [{ type: "image_url", image_url: { url: "data:image/png;base64,AAEC" } }] },
  ...
  it.effect("keeps reasoning_content on every assistant message when the model requires passback", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model: OpenAIChat.route
            .with({ endpoint: { baseURL: "https://api.openai.test/v1/" }, auth: Auth.bearer("test") })
            .model({ id: "deepseek-v4-flash", compatibility: { reasoningPassback: true } }),
          messages: [
            Message.assistant([
              { type: "reasoning", text: "thinking" },
              { type: "text", text: "Hello" },
            ]),
            Message.assistant([ToolCallPart.make({ id: "call_1", name: "bash", input: { command: "ls" } })]),
          ],
        }),
      )
  ...
  it.effect("omits reasoning_content on assistant messages without reasoning by default", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({ model, messages: [Message.assistant("Hello")] }),
      )

      expect(prepared.body.messages).toEqual([{ role: "assistant", content: "Hello" }])
    }),
  )
    ...
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          ...
          prompt: "think",
          providerOptions: { openai: { reasoningEffort: "max", serviceTier: "fast" } },
        }),
      )

      expect(prepared.body.store).toBe(false)
      expect(prepared.body.reasoning_effort).toBe("max")
      expect(prepared.body.service_tier).toBe("fast")
    }),
  )
  ...
  it.effect("continues provider-executed tool history lowered from another provider", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model,
          // Anthropic and OpenAI Responses carry a provider-executed result inline on the assistant
          // turn. Switching an existing session to an OpenAI Chat model replays that history, and
          // upstream also requires every tool_call to be answered, so the result becomes a tool
          // message rather than being rejected.
          messages: [
            Message.assistant([
              ToolCallPart.make({ id: "call_1", name: "read", input: { path: "a.ts" }, providerExecuted: true }),
              ToolResultPart.make({
                id: "call_1",
                name: "read",
      ...
      expect(prepared.body.messages).toEqual([
        {
          ...
          content: null,
          tool_calls: [
            { id: "call_1", type: "function", function: { name: "read", arguments: encodeJson({ path: "a.ts" }) } },
          ],
        },
