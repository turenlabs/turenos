...
describe("OpenAI-compatible Chat route", () => {
  ...
  it.effect("applies the route redirect policy to fetch", () =>
    Effect.gen(function* () {
      ...
      const fetch = Object.assign(
        (...args: Parameters<typeof globalThis.fetch>) => {
          ...
          return Promise.resolve(
            new Response(sseEvents(deltaChunk({ role: "assistant", content: "Local" }), deltaChunk({}, "stop")), {
              headers: { "content-type": "text/event-stream" },
            }),
          )
        },
      ...
      const response = yield* LLMClient.generate(
        LLM.updateRequest(request, {
          model: OpenAICompatibleChat.route
            .with({
              provider: "ollama",
              endpoint: { baseURL: "http://127.0.0.1:11434/v1" },
              http: { redirect: "error" },
            })
      ...
      expect(response.text).toBe("Local")
