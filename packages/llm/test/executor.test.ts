...
const transportFailureLayer = (
  ...
  failures: number,
  reason: (request: HttpClientRequest.HttpClientRequest) => HttpClientError.HttpClientErrorReason,
) =>
  RequestExecutor.layer.pipe(
    Layer.provide(
      Layer.succeed(
        ...
        HttpClient.make((request) =>
          Effect.gen(function* () {
            ...
            if (index < failures) return yield* new HttpClientError.HttpClientError({ reason: reason(request) })
...
// A client that rejects the way `fetch` does: not an HttpClientError at all,
// and with the real reason hidden on `cause`.
const nonHttpFailureLayer = (error: unknown) =>
  RequestExecutor.layer.pipe(
    Layer.provide(
      Layer.succeed(
        HttpClient.HttpClient,
        HttpClient.make(() => Effect.fail(error as HttpClientError.HttpClientError)),
      ),
    ),
  )
...
describe("RequestExecutor", () => {
  it.effect("carries the underlying cause of a failure that is not an HttpClientError", () =>
    Effect.gen(function* () {
      // This used to be dropped whole and reported as a bare "HTTP transport
      // failed": no kind, no cause, nothing to search a log for. `fetch` rejects
      // with a TypeError whose real reason sits on `cause`.
      ...
      expect(error.reason).toMatchObject({ kind: "TypeError" })
  ...
  it.effect("keeps hard 429 usage limits terminal", () =>
    Effect.gen(function* () {
      const executor = yield* RequestExecutor.Service
      const error = yield* executor.execute(request).pipe(Effect.flip)
  ...
  it.effect("does not retry a quota envelope mislabeled as a server error", () =>
    Effect.gen(function* () {
      const executor = yield* RequestExecutor.Service
      const error = yield* executor.execute(request).pipe(Effect.flip)
  ...
  it.effect("surfaces a deadline longer than its own ceiling instead of retrying early", () =>
    Effect.gen(function* () {
      ...
      // One request, not three. This layer cannot wait a minute -- it clamps to ten seconds -- so
      // retrying here spends requests the provider has already said it will reject and hides the
      // wait from the session-level policy that can actually honor it and tell the user.
  ...
  it.effect("retries a dropped connection and gives up on a request it cannot even encode", () =>
    Effect.gen(function* () {
      const attempts = yield* Ref.make(0)
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          transportFailureLayer(
            attempts,
            1,
            (request) => new HttpClientError.TransportError({ request, cause: new Error("socket hang up") }),
          ),
        ),
      )

      const encodeAttempts = yield* Ref.make(0)
      yield* Effect.gen(function* () {
        const executor = yield* RequestExecutor.Service
        const error = yield* executor.execute(request).pipe(Effect.flip)
        ...
        expect(error.retryable).toBe(false)
        expect(yield* Ref.get(encodeAttempts)).toBe(1)
      }).pipe(
        Effect.provide(
          transportFailureLayer(
            encodeAttempts,
            Number.MAX_SAFE_INTEGER,
            (request) => new HttpClientError.EncodeError({ request, cause: new Error("unserializable body") }),
          ),
        ),
      )
    }).pipe(Effect.provideService(Random.Random, randomMidpoint)),
