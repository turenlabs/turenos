...
Reuse an existing protocol for a provider with a compatible API (DeepSeek, TogetherAI, Cerebras, Baseten, Fireworks, and DeepInfra all use `OpenAIChat.protocol`); add a `Route.make(...)` call, never a route clone.
