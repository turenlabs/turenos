...
import { describeError, isTransientProviderError } from "../protocols/shared"
...
export class Service extends Context.Service<Service, Interface>()("@forge/LLM/RequestExecutor") {}
...
const retryableStatus = (status: number) =>
  status === 408 || status === 425 || status === 429 || status === 503 || status === 504 || status === 529
...
const isQuotaBody = (body: string) =>
  /insufficient[-_\s]?quota|usage[-_\s]?not[-_\s]?included|quota[-_\s]?(?:exceeded|exhausted)|billing[-_\s]?hard[-_\s]?limit|(?:free|go)[-_\s]?usage[-_\s]?limit/i.test(
    body,
  )
...
const statusReason = (input: {
  ...
  if (isTransientProviderError(undefined, body)) {
...
const toHttpError = (redactedNames: ReadonlyArray<string | RegExp>) => (error: unknown) => {
  ...
  // Anything that is not an HttpClientError used to be dropped whole and
  // reported as a bare "HTTP transport failed": no kind, no URL, nothing to
  // search a log for and nothing a user could act on. `fetch` rejects with a
  // TypeError whose `cause` carries the actual reason — ECONNRESET, ENOTFOUND,
  // a TLS failure — so the chain is walked and carried into the message.
  if (!HttpClientError.isHttpClientError(error)) {
    return transportError({ message: `HTTP transport failed: ${describeError(error)}`, kind: errorKind(error) })
  }
  const request = "request" in error ? error.request : undefined
  if (error.reason._tag === "TransportError") {
    return transportError({
      message: error.reason.description ?? `HTTP transport failed: ${describeError(error.cause ?? error)}`,
...
const retryStatusFailures = <A, R>(
  ...
  Effect.catchTag(effect, "LLM.Error", (error): Effect.Effect<A, LLMError, R> => {
    ...
    // A deadline this layer cannot keep belongs to the layer above it. Retrying a `Retry-After: 60`
    // after ten seconds spends two more requests that the provider has already promised to reject,
    // and buries a minute-long wait where nobody can be told about it. Surfacing it instead lets
    // the session-level policy wait the full time and say so -- see
