...
Hosted (provider-defined) tools: surface the model's call and the provider's result as `tool-call` and `tool-result` events with `providerExecuted: true`, and lower hosted tool definitions into the provider-native shape. Which hosted tools each route accepts, and how callers continue after them, is in `docs/tools.md` in this package.
