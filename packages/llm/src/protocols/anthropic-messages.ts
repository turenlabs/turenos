...
const lowerThinking = Effect.fn("AnthropicMessages.lowerThinking")(function* (request: LLMRequest) {
  ...
  if (thinking.type === "adaptive") {
    ...
    if (display === "summarized") return { type: "adaptive" as const, display: "summarized" as const }
    if (display === "omitted") return { type: "adaptive" as const, display: "omitted" as const }
    if (display !== undefined)
      return yield* invalid(`Anthropic adaptive thinking does not support display ${String(display)}`)
...
const lowerOutputConfig = Effect.fn("AnthropicMessages.lowerOutputConfig")(function* (request: LLMRequest) {
  const effort = anthropicOptions(request)?.effort
  if (effort === undefined) return undefined
  if (effort === "low") return { effort: "low" as const }
  if (effort === "medium") return { effort: "medium" as const }
  if (effort === "high") return { effort: "high" as const }
  if (effort === "xhigh") return { effort: "xhigh" as const }
  if (effort === "max") return { effort: "max" as const }
  return yield* invalid(`Anthropic does not support effort ${String(effort)}`)
})
...
const onError = (state: ParserState, event: AnthropicEvent): StepResult => [
  ...
  [
    LLMEvent.providerError({
      ...
      ...(isContextOverflow(event.error?.message ?? "")
        ? { classification: "context-overflow" as const }
        : isTransientProviderError(event.error?.type, event.error?.message ?? "")
          ? { retryable: true }
          : {}),
    }),
  ],
