...
import { JsonObject, optionalArray, optionalNull, ProviderShared, isTransientProviderError } from "./shared"
...
// OpenAI Responses surfaces provider failures in three related shapes, all
// three of which `@ai-sdk/openai` models explicitly (`openai-responses-api.ts`,
// v3.0.84):
//
//   flat    `{ type: "error", sequence_number, code?, message, param? }`
//   nested  `{ type: "error", sequence_number, error: { type, code, message, param? } }`
//   failed  `{ type: "response.failed", response: { error: { code, message } } }`
//
// Only the flat and `response.failed` shapes used to be captured, so a nested
// error decoded to a bare `{ type: "error" }` and reached the user as the
// fallback string with the provider's actual complaint discarded. `code` and
// `param` are nullish upstream, not absent, so they have to be nullable here or
// a real error event fails to decode at all.
...
const lowerToolCall = (part: ToolCallPart, callID: string): OpenAIResponsesInputItem => ({
  type: "function_call",
  call_id: callID,
  name: part.name,
  ...
  const usedCallIDs = new Set(
    request.messages.flatMap((message) =>
      message.content.flatMap((part) =>
        (part.type === "tool-call" || part.type === "tool-result") && part.id.length <= 64 ? [part.id] : [],
      ),
    ),
  )
  const callIDAliases = new Map<string, string>()
  let nextCallID = 1
  const lowerCallID = (callID: string) => {
    if (callID.length <= 64) return callID
    const existing = callIDAliases.get(callID)
    if (existing) return existing
    let alias = `forge_call_${nextCallID++}`
    while (usedCallIDs.has(alias)) alias = `forge_call_${nextCallID++}`
    callIDAliases.set(callID, alias)
    usedCallIDs.add(alias)
    return alias
  }
...
const lowerOptions = Effect.fn("OpenAIResponses.lowerOptions")(function* (request: LLMRequest) {
  ...
  return {
    ...
    ...(include ? { include } : {}),
    ...(effort || summary || mode || context ? { reasoning: { effort, summary, mode, context } } : {}),
...
const onFunctionCallArgumentsDone = Effect.fn("OpenAIResponses.onFunctionCallArgumentsDone")(function* (
  state: ParserState,
  event: OpenAIResponsesEvent,
) {
  if (!event.item_id || event.arguments === undefined) return [state, NO_EVENTS] satisfies StepResult
  const result = yield* ToolStream.finishWithInput(ADAPTER, state.tools, event.item_id, event.arguments)
  const events: LLMEvent[] = []
  ...
  return [
    {
      ...
      tools: result.tools,
      completedTools: completed ? new Set([...state.completedTools, event.item_id]) : state.completedTools,
    },
    events,
...
// Every place the provider may have put the failure detail, most specific
// first. `event.error` is the nested streaming shape, `event.response.error`
// the `response.failed` shape, and the top-level fields the flat one.
const errorPayloads = (event: OpenAIResponsesEvent) => [event.error, event.response?.error, event]
...
const providerErrorCode = (event: OpenAIResponsesEvent) => {
  ...
  // The nested shape carries an OpenAI error *type* (`invalid_request_error`,
  // `authentication_error`) where the flat one carries a code. Either names the
  // failure mode, which is the whole point of the prefix.
  return event.error?.type || undefined
}

/**
 * Build a single human-readable message from whatever the provider supplied.
 * When both code and message are present, prefix the code so consumers see the
 * failure mode (e.g. `rate_limit_exceeded: Slow down`) instead of just the bare
 * message — production rate limits and context-length failures used to be
 * indistinguishable from generic stream drops.
 *
 * When the event carries nothing we recognise, the raw frame goes into the
 * message rather than a bare sentence. A provider failure nobody can read is
 * still a provider failure someone has to debug, and discarding the only
 * evidence of it has cost real hours.
 */
const providerErrorMessage = (event: OpenAIResponsesEvent, fallback: string): string => {
  const message = errorPayloads(event).find((payload) => payload?.message)?.message || undefined
  const code = providerErrorCode(event)
  if (message && code) return `${code}: ${message}`
  if (message || code) return message || code || fallback
  const detail = describeEvent(event)
  return detail ? `${fallback}: ${detail}` : fallback
}

/** Compact, secret-free rendering of an unrecognised error event. */
const describeEvent = (event: OpenAIResponsesEvent) => {
  const { type: _type, delta: _delta, item: _item, item_id: _itemID, summary_index: _index, ...rest } = event
  const entries = Object.entries(rest).filter(([, value]) => value !== undefined && value !== null)
  if (entries.length === 0) return undefined
  const rendered = JSON.stringify(Object.fromEntries(entries))
  return rendered.length > MAX_ERROR_DETAIL ? `${rendered.slice(0, MAX_ERROR_DETAIL)}…` : rendered
}
...
const providerError = (event: OpenAIResponsesEvent, fallback: string) => {
  ...
  const overflow = code === "context_length_exceeded" || isContextOverflow(message)
  return LLMEvent.providerError({
    message,
    ...(overflow
      ? { classification: "context-overflow" as const }
      : isTransientProviderError(code, message)
        ? { retryable: true }
        : {}),
  })
}
...
const step = (state: ParserState, event: OpenAIResponsesEvent) => {
  ...
  if (event.type === "response.function_call_arguments.done") return onFunctionCallArgumentsDone(state, event)
