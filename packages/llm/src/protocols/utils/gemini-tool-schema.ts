...
const projectNode = (schema: unknown): Record<string, unknown> | undefined => {
  ...
  // An empty object is a real parameter contract ("no arguments, but callers must send a keyed
  // object"), not "no schema". Dropping it made strict provider validators reject the tool with
  // "input_schema.type is required"; keep the object type explicit instead.
  if (emptyObjectSchema(schema)) return { type: "object" }
