    ...
    Effect.catch((error) =>
      Effect.succeed(
        LLMEvent.toolError({
          id: tool.id,
          name: tool.name,
          message: `${error.reason.message}. Regenerate this tool call with valid JSON.`,
          recoverable: true,
          providerMetadata: tool.providerMetadata,
        }),
      ),
...
/**
 * Finalize one pending tool call. Malformed model-generated JSON becomes a
 * `tool-error` so the model can self-correct without replaying earlier calls.
