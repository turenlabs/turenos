 ...
 */
/**
 * What a whole logical turn processed, when that turn was more than one
 * provider request.
 *
 * Only transports that run their own agentic loop behind a single TurenOS turn
 * set this — today that is the Claude Code CLI, which reports one
 * run-cumulative `usage` block covering every internal round trip. For those
 * transports the fields on `Usage` describe the *final* request and this
 * describes the run.
 *
 * The distinction matters because the two answer different questions.
 * Cumulative usage is not context occupancy: cache-read tokens are re-counted
 * on every round trip, so a summed figure grows without bound (1.6M against a
 * 200K window is routine) and, being a running total, can never fall after a
 * compaction. Occupancy is a property of one request; cost and budgets are
...
}).annotate({ identifier: "LLM.TurnUsage" })
export type TurnUsage = Schema.Schema.Type<typeof TurnUsage>
...
export class Usage extends Schema.Class<Usage>("LLM.Usage")({
  ...
  /**
   * Present only when this turn ran several provider requests. See
   * {@link TurnUsage} — when it is set, every other field above is the final
   * request's usage, not the run's.
   */
  turn: Schema.optional(TurnUsage),
