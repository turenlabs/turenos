...
// Encoding the request body and building the URL are deterministic: repeating
// them cannot change the outcome. Everything else here is a socket that never
// delivered a response — a dropped connection, a DNS blip, a connect timeout —
// which is the most transient failure a provider call has. The executor's retry
// wrapper only spans connection setup and response status, so nothing has been
// streamed to the caller yet and replaying is safe.
const PERMANENT_TRANSPORT_KINDS = new Set(["EncodeError", "InvalidUrlError"])
...
export class TransportReason extends Schema.Class<TransportReason>("LLM.Error.Transport")({
  ...
  get retryable() {
    return this.kind === undefined || !PERMANENT_TRANSPORT_KINDS.has(this.kind)
  }
}
