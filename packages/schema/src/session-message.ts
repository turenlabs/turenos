...
export const Shell = Schema.Struct({
  ...
  status: Schema.Literals(["running", "completed", "cancelled", "timed_out", "failed"]).pipe(optional),
...
export const Compaction = Schema.Struct({
  ...
  /**
   * Append-only ledger of durable facts, oldest line first.
   *
   * The summary is a fixed-size container: across nine compaction generations of a marathon
   * session it stayed flat at ~4.5-5.5KB while the fact set it was compressing grew from 110 to
   * 897 facts, so recall decayed generation over generation. The ledger is the growing half of
   * the checkpoint: each compaction extracts facts from the newly summarized segment only and
   * appends them here verbatim. Existing lines are never rewritten or re-summarized -- that is
   * what stops the decay -- and the bound is enforced by evicting the oldest lines instead.
   *
   * Optional: every checkpoint written before the ledger landed omits it and must still decode.
   */
  ledger: Schema.Array(Schema.String).pipe(optional),
  /**
   * Aggregate sequence through which the summary is authoritative. Messages after this boundary
   * remain as their original structured records even when they were committed before the
   * compaction event itself. Older checkpoints omit this and retain the legacy event-sequence
   * cutoff.
   */
