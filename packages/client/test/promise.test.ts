...
import { isSessionNotFoundError, isUnauthorizedError, Forge } from "../src"

test("exposes every standard HTTP API group", () => {
  const client = Forge.make({ baseUrl: "http://localhost:3000" })
...
test("sessions expose durable input status and paginated outbox", async () => {
  const calls: string[] = []
  const client = Forge.make({
    baseUrl: "http://localhost:3000",
    fetch: async (input) => {
  ...
  const status = await client.sessions.inputStatus({ sessionID: "ses_test", messageID: "msg_status" })
  const outbox = await client.sessions.outbox({ sessionID: "ses_test", limit: 1, status: "cancelled" })

  expect(status).toMatchObject({ id: "msg_status", status: "cancelled" })
  expect(outbox.data).toEqual([expect.objectContaining({ id: "msg_status", status: "cancelled" })])
  expect(calls).toEqual([
    "http://localhost:3000/api/session/ses_test/input/msg_status",
    "http://localhost:3000/api/session/ses_test/outbox?limit=1&status=cancelled",
  ])
})
...
test("session methods use the public HTTP contract", async () => {
  ...
  const client = Forge.make({
    ...
    fetch: async (input, init) => {
      ...
      if (url.endsWith("/api/session/interrupt-all")) return Response.json({ data: { interrupted: 1, failed: 0 } })
  ...
  const interrupted = await client.sessions.interruptAll()
  ...
  expect(interrupted).toEqual({ interrupted: 1, failed: 0 })
  ...
  expect(requests.map((request) => [request.init?.method, request.url])).toEqual([
    ...
    ["POST", "http://localhost:3000/api/session/interrupt-all"],
