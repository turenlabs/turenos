...
import { AbsolutePath, Agent, Location, Model, Forge, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* Forge.make({ baseUrl: "http://localhost:3000" })
...
test("session methods retain decoded Effect inputs and outputs", async () => {
  ...
  const httpClient = HttpClient.make((request) => {
    ...
    if (url.includes("/history")) {
      ...
      return Effect.succeed(
        HttpClientResponse.fromWeb(
          ...
          Response.json(
            historyPage === 1
              ? { data: [modelSwitchedEvent], hasMore: true, latest: 1 }
              : { data: [], hasMore: false, latest: 1 },
          ),
        ),
      )
    }
    ...
    if (url.endsWith("/api/session/interrupt-all")) {
      return Effect.succeed(HttpClientResponse.fromWeb(request, Response.json({ data: { interrupted: 1, failed: 0 } })))
    }
  ...
  const result = await Effect.gen(function* () {
    ...
    const interrupted = yield* client.sessions.interruptAll()
    ...
    return { page, active, interrupted, created, admitted, context, history, historyNext, events, message }
  }).pipe(Effect.provideService(HttpClient.HttpClient, httpClient), Effect.runPromise)

  expect(DateTime.toEpochMillis(result.page.data[0].time.created)).toBe(1_717_171_717_000)
  expect(result.active).toEqual({ ses_test: { type: "running" } })
  expect(result.interrupted).toEqual({ interrupted: 1, failed: 0 })
  ...
  expect(result.historyNext).toEqual({ data: [], hasMore: false, latest: 1 })
