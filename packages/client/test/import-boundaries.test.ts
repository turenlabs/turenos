...
const effect = dirname(dirname(realpathSync(fileURLToPath(import.meta.resolve("effect")))))
...
describe("public import boundaries", () => {
  test("isolates each public entrypoint", async () => {
    const root = await bundleInputs("@turenlabs/client", "browser")
    ...
    const network = await bundleInputs("@turenlabs/client/effect", "browser")
