...
This is the current reference for expected failures, typed service errors, and HTTP error boundaries.
...
should send structured public errors; Desktop and automation clients should format those
...
that body. Client aggregation helpers should call `FormatError` first, then fall
...
```text
Configuration is invalid at /path/to/forge.json
...
to avoid: a user-authored invalid `forge.json` crossed the HttpApi boundary
...
are startup-blocking and must be rendered clearly in Desktop and automation flows.
