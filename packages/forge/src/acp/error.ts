...
export class McpServersUnsupportedError extends Schema.TaggedErrorClass<McpServersUnsupportedError>()(
  "ACPMcpServersUnsupportedError",
  { count: Schema.Number },
) {}
...
export function toRequestError(error: Error) {
  switch (error._tag) {
    ...
    case "ACPMcpServersUnsupportedError":
      return RequestError.invalidParams({ count: error.count }, "MCP servers must be managed through Extensions")
