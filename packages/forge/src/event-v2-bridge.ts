// TurenOS publish boundary for core events. Attach routed instance location
...
export class Service extends Context.Service<Service, EventV2.Interface>()("@forge/EventV2Bridge") {}
  ...
  Effect.gen(function* () {
    ...
    const unsubscribe = yield* events.listen((event) =>
      Effect.gen(function* () {
        ...
        if (
          SessionTaskV2.isDurableEventType(EventV2.versionedType(event.type, event.durable.version)) ||
          (yield* SessionTaskV2.isProtectedAggregate(db, event.durable.aggregateID))
        )
          return
        GlobalBus.emit("event", {
          directory: event.location?.directory ?? ctx?.directory,
