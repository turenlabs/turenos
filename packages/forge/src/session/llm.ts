...
import { llmClient } from "@turenlabs/core/effect/app-node-platform"
...
import { jsonSchema, streamText, tool, wrapLanguageModel, type ModelMessage, type Tool } from "ai"
import type { LLMEvent, ToolDefinition } from "@turenlabs/llm"
import { LLMClient } from "@turenlabs/llm/route"
import type { LLMClientService } from "@turenlabs/llm/route"
...
export type StreamInput = {
  ...
  constraints?: LLMRequestPrep.RequestConstraints
}
...
export class Service extends Context.Service<Service, Interface>()("@forge/LLM") {}
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      if (isClaudeCode) {
        ...
        return {
          ...
          stream: LLMClaudeCodeDirect.stream({
            model: input.model,
            prepared,
            llmClient,
      ...
      if (!language) throw new Error(`Language model unavailable: ${input.model.providerID}/${input.model.id}`)
      ...
      // Runtime seam: native is an opt-in adapter over @turenlabs/llm. It
      // either returns a ready LLMEvent stream or a concrete fallback reason.
      if (flags.experimentalNativeLlm && !input.connectionPolicy) {
...
export function fixedTools(model: Provider.Model, definitions: readonly ToolDefinition[]) {
  return Object.fromEntries(
    definitions.map((definition) => [
      definition.name,
      tool({
        description: definition.description,
        inputSchema: jsonSchema(ProviderTransform.schema(model, definition.inputSchema)),
      }),
    ]),
  )
}
