import type { JsonSchema, LLMRequest, ProviderMetadata } from "@turenlabs/llm"
import { LLM, Message, SystemPart, ToolCallPart, ToolDefinition, ToolResultPart } from "@turenlabs/llm"
import { AmazonBedrock, Anthropic, Azure, Google, OpenAI, OpenAICompatible, OpenRouter } from "@turenlabs/llm/providers"
...
export type RequestInput = {
  ...
  readonly resolvedModel?: LLMRequest["model"]
...
export const request = (input: RequestInput) => {
  ...
  // @turenlabs/llm request objects from opencode's session/AI SDK-shaped data.
  return LLM.request({
    model: input.resolvedModel ?? model(input, input.headers),
