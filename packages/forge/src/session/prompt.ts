...
import { MAX_STEPS_PROMPT } from "@turenlabs/core/session/runner/max-steps"
...
import { SessionTodoGuidance } from "@turenlabs/core/session/todo-guidance"
...
export const resolveCommand = Effect.fn("SessionPrompt.resolveCommand")(function* (
  commands: Pick<Command.Interface, "get">,
  skills: Pick<Skill.Interface, "get">,
  name: string,
) {
  const configured = yield* commands.get(name)
  if (configured) return configured
  const skill = yield* skills.get(name).pipe(Effect.option, Effect.map(Option.getOrUndefined))
...
export class Service extends Context.Service<Service, Interface>()("@forge/SessionPrompt") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const { db } = database
    const assertLegacyRun = (sessionID: SessionID) => tasks.authorizeMutation({ sessionID }).pipe(Effect.orDie)
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      yield* assertLegacyRun(sessionID)
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      yield* assertLegacyRun(input.sessionID)
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<SessionV1.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        yield* MessageV2.assertLegacyReadable(db, sessionID).pipe(Effect.orDie)
        const ctx = yield* InstanceState.context
        let structured: unknown
        let step = 0
        let todoReconciliationUsed = false
        let processedUserID: MessageID | undefined
        ...
        while (true) {
          ...
          if (processedUserID !== undefined && processedUserID !== lastUser.id) todoReconciliationUsed = false
          processedUserID = lastUser.id
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const mcpInstructions = MCP.formatInstructions(yield* mcp.instructions())
            const system = [
              ...env,
              ...instructions,
              ...(skills ? [skills] : []),
              ...(mcpInstructions ? [mcpInstructions] : []),
              ...(Object.hasOwn(tools, McpTool.SEARCH_TOOL_NAME) ? [McpTool.DISCOVERY_SYSTEM_PROMPT] : []),
              SessionTodoGuidance.SYSTEM,
            ]
            const todoToolAvailable =
              Object.hasOwn(tools, "todowrite") &&
              Permission.evaluate("todowrite", "*", agent.permission, session.permission ?? []).action !== "deny" &&
              model.providerID !== ClaudeCodeCLI.ID
            const previousTodoUpdate =
              lastAssistantMsg?.parts.some(
                (part) => part.type === "tool" && part.tool === "todowrite" && part.state.status === "completed",
              ) ?? false
            if (previousTodoUpdate) todoReconciliationUsed = false
            const todoPrompt = todoToolAvailable
              ? step === 1
                ? "initial"
                : hasToolCalls && !previousTodoUpdate && !todoReconciliationUsed
                  ? "reconcile"
                  : undefined
              : undefined
            if (todoPrompt === "reconcile") todoReconciliationUsed = true
            if (todoPrompt) system.push(SessionTodoGuidance.prompt(todoPrompt, yield* todos.get(sessionID)))
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      const cmd = yield* resolveCommand(commands, skills, input.command)
      if (!cmd) {
