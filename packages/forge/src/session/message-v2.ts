...
import {
  ...
  User,
  WithParts,
} from "@turenlabs/core/v1/session"
...
export const assertLegacyReadable = Effect.fn("MessageV2.assertLegacyReadable")(function* (
  ...
  return yield* new NotFoundError({
    message: `Legacy transcript is unavailable after Session V2 adoption: ${sessionID}; use the V2 context or history endpoint`,
  })
})
...
export const page = Effect.fn("MessageV2.page")(function* (input: {
  ...
  yield* assertLegacyReadable(db, input.sessionID)
...
export function stream(sessionID: SessionID) {
  ...
  return Effect.gen(function* () {
    ...
    while (true) {
      const next = yield* page({ sessionID, limit: size, before })
  ...
  return Effect.gen(function* () {
    ...
    const current = legacy
      ...
      : yield* Database.primary(db)
          .select({ sessionID: SessionMessageTable.session_id })
          .from(SessionMessageTable)
          .where(eq(SessionMessageTable.id, SessionMessage.ID.make(messageID)))
          .get()
          .pipe(Effect.orDie)
    const owner = legacy ?? current
    if (owner) yield* assertLegacyReadable(db, owner.sessionID).pipe(Effect.orDie)
    const rows = yield* db
      .select()
      .from(PartTable)
          ...
          return new APIError(
            {
              ...
              ...(parsed.responseHeaders === undefined ? {} : { responseHeaders: parsed.responseHeaders }),
              responseBody: parsed.responseBody,
            },
            {
              cause: e,
