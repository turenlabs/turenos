...
import { SnapshotShadow } from "@turenlabs/core/snapshot/shadow"
import { Storage } from "@turenlabs/core/storage"
import { Config } from "@/config/config"
import { Global } from "@turenlabs/core/global"
import { Info } from "@turenlabs/schema/file-diff"
...
export class Service extends Context.Service<Service, Interface>()("@forge/Snapshot") {}

type Requirements = FSUtil.Service | AppProcess.Service | Config.Service | Storage.Service
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        const state = {
          ...
          // The shared resolver reads and claims the worktree binding itself, so it needs the
          // storage and filesystem services. `InstanceState.make` builds each instance's state
          // with nothing but a Scope, so they are handed over from the layer's own context here
          // rather than leaking into every method's requirements.
          gitdir: yield* SnapshotShadow.directory({
            data: Global.Path.data,
            projectID: ctx.project.id,
            worktree: ctx.worktree,
          }).pipe(Effect.provideService(Storage.Service, storage), Effect.provideService(FSUtil.Service, fs)),
          vcs: ctx.project.vcs,
        }
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [FSUtil.node, AppProcess.node, Config.node, Storage.node],
})
