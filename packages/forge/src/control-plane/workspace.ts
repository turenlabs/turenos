...
import { WorkspaceEvent } from "@turenlabs/schema/workspace-event"
...
export class Service extends Context.Service<Service, Interface>()("@forge/Workspace") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
          ...
          yield* parseSSE(stream, (evt) =>
            Effect.gen(function* () {
              ...
              if (payload.type === "sync" && payload.syncEvent) {
                if (
                  SessionTaskV2.isDurableEventType(payload.syncEvent.type) ||
                  (yield* SessionTaskV2.isProtectedAggregate(db, payload.syncEvent.aggregateID))
                )
                  return
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const env = {
        FORGE_WORKSPACE_ID: config.id,
        FORGE_EXPERIMENTAL_WORKSPACES: "true",
    ...
    const sessionWarp = Effect.fn("Workspace.sessionWarp")(function* (input: SessionWarpInput) {
      return yield* Effect.gen(function* () {
        // A task graph has immutable root/child placement. Reject before remote
        // sync, ownership claims, patch capture, or destination writes.
