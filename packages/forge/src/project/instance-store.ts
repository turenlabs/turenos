...
import { makeGlobalNode, Node } from "@turenlabs/core/effect/app-node"
...
export class Service extends Context.Service<Service, Interface>()("@forge/InstanceStore") {}
  ...
  Effect.gen(function* () {
    ...
    const boot = (input: LoadInput & { directory: string }) =>
      Effect.gen(function* () {
        ...
        yield* Effect.logInfo("instance boot phase", {
          directory: input.directory,
          phase: "project-resolve.started",
        })
        ...
        yield* Effect.logInfo("instance boot phase", {
          directory: input.directory,
          phase: "project-resolve.completed",
    ...
    const disposeAllOnce = Effect.fnUntraced(function* (excludeDirectory?: string) {
      yield* Effect.logInfo("disposing all instances")
      yield* Effect.forEach(
        [...cache.entries()].filter(([directory]) => directory !== excludeDirectory),
        (item) =>
          Effect.gen(function* () {
    ...
    const disposeAllExcept = Effect.fn("InstanceStore.disposeAllExcept")(function* (directory: string) {
