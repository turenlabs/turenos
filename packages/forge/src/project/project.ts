...
export class Service extends Context.Service<Service, Interface>()("@forge/Project") {}
  ...
  Effect.gen(function* () {
        ...
        .transaction(
          (d) =>
            Effect.gen(function* () {
              ...
              // Anything still pointing at the old row has to be carried over
              // before it is deleted. `permission.project_id` and
              // `memory_wing.project_id` are declared ON DELETE CASCADE, so a
              // row left behind here is not orphaned, it is destroyed — every
              // saved "always allow" grant and every project memory wing would
              // vanish with no receipt. `OR IGNORE` leaves behind only rows the
              // destination already has an equivalent of, which the delete
              // below then reaps.
              yield* d.run(sql`UPDATE OR IGNORE permission SET project_id = ${newID} WHERE project_id = ${oldID}`)
              yield* d.run(sql`DELETE FROM permission WHERE project_id = ${oldID}`)
              yield* d.run(sql`UPDATE OR IGNORE memory_wing SET project_id = ${newID} WHERE project_id = ${oldID}`)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      if (projectID !== ProjectV2.ID.global && data.vcs?.type === "git") {
        yield* projectV2.remember({ store: data.vcs.store, id: data.id })
      }
