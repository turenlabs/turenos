import type { Session as SDKSession, Message, Part } from "@turenlabs/sdk/v2"
import { SessionV1 } from "@turenlabs/core/v1/session"
import { Session } from "@/session/session"
import { MessageV2 } from "../../session/message-v2"
import { CliError, effectCmd } from "../effect-cmd"
import { Database } from "@turenlabs/core/database/database"
import { SessionTable, MessageTable, PartTable } from "@turenlabs/core/session/sql"
...
/** Extract a share ID from an explicitly configured legacy share URL. */
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    ...
    if (!shouldAttachShareAuthHeaders(file, req.baseUrl)) {
      process.stdout.write(`Share URL must match the configured legacy share endpoint: ${req.baseUrl}`)
      process.stdout.write(EOL)
      return
    }
