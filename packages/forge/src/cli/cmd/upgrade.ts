...
export const UpgradeCommand = {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (requestedMethod === "unknown") {
      prompts.log.error(`forge is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (InstallationVersion === target) {
      prompts.log.warn(`forge upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }
