...
export class Service extends Context.Service<Service, Interface>()("@forge/Permission") {}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: PermissionV1.AskInput) {
      ...
      for (const pattern of request.patterns) {
        ...
        if (!(yield* checks.enforced())) continue
        needsAsk = true
      }
      ...
      const disabled = yield* Effect.raceFirst(
        Deferred.await(deferred).pipe(Effect.as(false)),
        checks.untilDisabled().pipe(Effect.as(true)),
      ).pipe(
        Effect.onInterrupt(() =>
      ...
      yield* completion.withPermit(
        Effect.gen(function* () {
          ...
          yield* events.publish(Event.Replied, { sessionID: info.sessionID, requestID: info.id, reply: "once" })
    ...
    const reply = Effect.fn("Permission.reply")((input: PermissionV1.ReplyInput) =>
      completion.withPermit(
        Effect.gen(function* () {
          ...
          yield* tasks.authorizeMutation({ sessionID: existing.info.sessionID }).pipe(Effect.orDie)
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [EventV2Bridge.node, PermissionChecks.node, SessionTaskV2.node],
})
