...
import { RepoMapTool } from "./repo_map"
...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@turenlabs/plugin"
...
import { DefinitionTool, IncomingCallsTool, ReferencesTool, WorkspaceSymbolTool } from "./lsp-symbol"
...
export class Service extends Context.Service<Service, Interface>()("@forge/ToolRegistry") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const workspacesymboltool = yield* WorkspaceSymbolTool
    const referencestool = yield* ReferencesTool
    const incomingcallstool = yield* IncomingCallsTool
    const definitiontool = yield* DefinitionTool
    ...
    const repomaptool = yield* RepoMapTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          repo_map: Tool.init(repomaptool),
          edit: Tool.init(edit),
          ...
          workspace_symbol: Tool.init(workspacesymboltool),
          references: Tool.init(referencestool),
          incoming_calls: Tool.init(incomingcallstool),
          definition: Tool.init(definitiontool),
          plan: Tool.init(plan),
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            // Name-addressed structural tools. Agent-only: they gate on the
            // existing `lsp` permission key, add no config surface, and start
            // their language server lazily on first use.
    ...
    const describeCodeMode = Effect.fn("ToolRegistry.describeCodeMode")(function* (input: {
      ...
      const tools = Permission.visibleTools(input.mcpTools ?? {}, ruleset)
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const websearchProviders = {
        exa: yield* extensions.enabled(Extension.ID.make("turenlabs", "websearch-exa")),
        parallel: yield* extensions.enabled(Extension.ID.make("turenlabs", "websearch-parallel")),
      }
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return websearchProviders.exa || websearchProviders.parallel
        }
