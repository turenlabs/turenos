...
import { Module } from "@turenlabs/core/util/module"
...
/**
 * Look for an already-installed `gopls` in every place `go install` is known
 * to put it, before ever considering installing one.
 *
 * `go install` defaults to `$GOBIN`, falling back to `$(go env GOPATH)/bin`,
 * which itself defaults to `~/go/bin` — and none of those are reliably on
 * `PATH` (they are not exported by default on a stock Go install). Checking
 * `which("gopls")` alone therefore misses a perfectly good binary a prior
 * `go install golang.org/x/tools/gopls@latest` already produced, and used to
 * walk straight into reinstalling it every single time.
 */
async function resolveGoplsBinary(): Promise<string | undefined> {
  const ext = process.platform === "win32" ? ".exe" : ""

  const dirs: (string | undefined)[] = [process.env.GOBIN]
  if (which("go")) {
    const gopath = await output(["go", "env", "GOPATH"])
    const trimmed = gopath.text.trim()
    if (gopath.code === 0 && trimmed) dirs.push(path.join(trimmed, "bin"))
  }
  dirs.push(path.join(os.homedir(), "go", "bin"))

  for (const dir of dirs) {
    if (!dir) continue
    const candidate = path.join(dir, "gopls" + ext)
    if (await pathExists(candidate)) return candidate
  }

  // `which()` also searches Global.Path.bin, so this still covers a gopls a
  // previous version of this function may have installed there.
  return which("gopls") ?? undefined
}
...
export const Gopls: Info = {
  ...
  extensions: [".go"],
  async spawn(root) {
    const bin = await resolveGoplsBinary()
    if (!bin) {
      // This used to fall back to `go install golang.org/x/tools/gopls@latest`,
      // which can take minutes on a cold module cache and gave no indication
      // anything was happening — a structural query would just hang. Fail the
      // spawn immediately instead and say exactly what to run.
      console.error(
        "[lsp] gopls not found (checked $GOBIN, `go env GOPATH`/bin, ~/go/bin, and PATH). Install it with " +
          "`go install golang.org/x/tools/gopls@latest`, then make sure that install directory is on PATH.",
      )
      return
    }
    return {
