...
export function convertTool(
  ...
  client: Client,
  timeout?: number,
  transform?: (result: CallToolResult) => CallToolResult,
): Tool {
    ...
    execute: async (args: unknown, options) => {
      const called = await client.callTool(
        ...
        CallToolResultSchema,
        {
          ...
          timeout,
          // Progress resets are open-ended: without a total cap a server that
          // keeps emitting notifications/progress holds the call forever.
          maxTotalTimeout: Math.max((timeout ?? 0) * 10, 600_000),
