...
export class McpOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    return {
      ...
      client_name: "TurenOS",
      client_uri: "https://github.com/turenlabs/forge",
  ...
  async clientInformation(): Promise<OAuthClientInformation | undefined> {
    ...
    const entry = await Effect.runPromise(this.auth.getForUrl(this.mcpName, this.serverUrl, this.generation))
    if (entry?.clientInfo) {
...
/**
 * Provider for an ordinary connect, which can never complete an authorization.
 *
 * `connectRemote` passes a no-op `onRedirect`, so when the SDK decides to authorize,
 * every PKCE artefact it asks this provider to save is dead the moment the connect
 * fails with `UnauthorizedError`. Those artefacts live in a process-global map keyed
 * by server name (`McpAuth`'s `attempts`), shared with the interactive flow — and one
 * background connect fires per open directory. Keeping them on the instance instead
 * is what stops a background connect from overwriting the verifier and CSRF state
 * that an in-flight `startAuth` is waiting on. Tokens and client registration still
 * go through storage, because those are worth keeping.
 */
export class McpOAuthAutoProvider extends McpOAuthProvider {
  private attemptVerifier?: string
  private attemptState?: string

  override async saveCodeVerifier(codeVerifier: string): Promise<void> {
    this.attemptVerifier = codeVerifier
  }

  override async codeVerifier(): Promise<string> {
    if (!this.attemptVerifier) {
      throw new Error(`No code verifier saved for MCP server: ${this.mcpName}`)
    }
  ...
  override async state(): Promise<string> {
    this.attemptState ??= Array.from(crypto.getRandomValues(new Uint8Array(32)))
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("")
...
export class McpOAuthPendingProvider extends McpOAuthProvider {
  ...
  async commit(): Promise<void> {
    ...
    const current = await Effect.runPromise(this.auth.getForUrl(this.mcpName, this.serverUrl, this.generation))
    await Effect.runPromise(
      this.auth.set(
        this.mcpName,
        {
          tokens: {
            ...
            refreshToken: this.pendingTokens.refresh_token ?? current?.tokens?.refreshToken,
