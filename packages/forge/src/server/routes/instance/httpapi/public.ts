...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for TurenOS serve and web commands",
    ...
    for (const method of ["get", "post", "put", "delete", "patch"] as const) {
      ...
      if (operation.requestBody) {
        ...
        if (path === "/session/{sessionID}" && method === "patch") {
          // `time.archived` is Schema.NullOr — genuinely nullable, because null is
          // how a client unarchives a session. Re-add the null the strip removed,
          // otherwise the generated SDK types make archiving a one-way door.
          const ref = operation.requestBody.content?.["application/json"]?.schema?.$ref?.replace(
            "#/components/schemas/",
            "",
          )
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  for (const [name, fields] of [
    ["StorageGetResult", ["state"]],
    ["StorageSetInput", ["expectedRevision"]],
    ["StorageRevisionConflictError", ["expected", "actual"]],
    ["StorageReceiptResult", ["receipt"]],
    ["StorageMigrationReceipt", ["sourceFingerprint", "sourceVersion", "rowCount", "timeVerified"]],
    ["SessionGoalGetResponse", ["data"]],
  ] as const) {
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ForgeHttpApi)
...
export const PublicApi = ForgeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "TurenOS",
    version: "1.0.0",
    description: "TurenOS API",
