...
import * as ClientDisconnect from "@/server/shared/client-disconnect"
...
>()("@forge/ExperimentalHttpApiWorkspaceRouteContext") {}
...
>()("@forge/ExperimentalHttpApiWorkspaceRouting") {}
...
function configuredWorkspaceID(): WorkspaceV2.ID | undefined {
  return Flag.FORGE_WORKSPACE_ID ? WorkspaceV2.ID.make(Flag.FORGE_WORKSPACE_ID) : undefined
}
...
function defaultDirectory(request: HttpServerRequest.HttpServerRequest, url: URL): string {
  return url.searchParams.get("directory") || request.headers["x-forge-directory"] || process.cwd()
}
...
function proxyRemote(
  ...
  return Effect.gen(function* () {
    ...
    if (sync) {
      // Previously this read `request.source instanceof Request ? ... .signal`,
      // which is only ever true on the `Server.Default()` web-handler path — on
      // the real `Server.listen` listener `source` is a Node `IncomingMessage`,
      // so the fence always waited out its full timeout even after the client
      // hung up. `ClientDisconnect.use` reports a hangup on both transports.
      const syncFailure = yield* ClientDisconnect.use(request, (signal) => Fence.wait(workspace.id, sync, signal)).pipe(
        Effect.as(undefined),
