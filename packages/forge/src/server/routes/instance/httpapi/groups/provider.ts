  ...
  .add(
    HttpApiGroup.make("provider")
      .add(
        ...
        HttpApiEndpoint.get("usage", `${root}/usage`, {
          query: WorkspaceRoutingQuery,
          success: described(Provider.UsageResult, "Locally observed provider usage"),
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "provider.usage",
            summary: "Get provider usage",
            description: "Get token usage observed by TurenOS during the last seven days, grouped by provider.",
          }),
        ),
