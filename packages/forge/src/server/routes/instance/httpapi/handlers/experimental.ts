...
export const experimentalHandlers = HttpApiBuilder.group(InstanceHttpApi, "experimental", (handlers) =>
  Effect.gen(function* () {
    ...
    const tool = Effect.fn("ExperimentalHttpApi.tool")(function* (ctx: { query: typeof ToolListQuery.Type }) {
      ...
      const found = ctx.query.sessionID
        ? yield* sessionsV2.get(ctx.query.sessionID).pipe(Effect.catch(() => Effect.succeed(undefined)))
        : undefined
      if (
        ctx.query.sessionID &&
        (!found || found.location.directory !== directory || found.location.workspaceID !== workspaceID)
      )
      ...
      const snapshot = yield* Effect.gen(function* () {
        ...
        return yield* snapshots.materialize({
          sessionID: ctx.query.sessionID ?? SessionV2.ID.make("ses_tool_diagnostic"),
          directory: locationRef.directory,
          model: ModelV2.Ref.make({ providerID: ctx.query.provider, id: ctx.query.model }),
          agent: agent.id,
          permissions: agent.info?.permissions,
          parentID: session?.parentID,
          taskOwned: session?.parentID !== undefined,
          harnessSessionID: session?.parentID ?? session?.id,
