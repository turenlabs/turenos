...
import { CorsConfig, isAllowedRequestOrigin, type CorsOptions } from "@turenlabs/server/cors"
import {
...
export const ptyHandlers = HttpApiBuilder.group(InstanceHttpApi, "pty", (handlers) =>
  Effect.gen(function* () {
    ...
    const create = Effect.fn("PtyHttpApi.create")(function* (ctx: { payload: typeof Pty.CreateInput.Type }) {
      ...
      const launch = resolvePtyCommand(ctx.payload.command, ctx.payload.args ? [...ctx.payload.args] : undefined, cwd)
      return yield* pty(
