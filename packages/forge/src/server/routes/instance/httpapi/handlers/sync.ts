...
export const syncHandlers = HttpApiBuilder.group(InstanceHttpApi, "sync", (handlers) =>
  Effect.gen(function* () {
    ...
    const replay = Effect.fn("SyncHttpApi.replay")(function* (ctx: { payload: typeof ReplayPayload.Type }) {
      ...
      const protectedAggregate = (yield* Effect.forEach(
        [...new Set(payload.map((event) => event.aggregateID))],
        (aggregateID) => SessionTaskV2.isProtectedAggregate(db, aggregateID),
      )).some(Boolean)
      if (payload.some((event) => SessionTaskV2.isDurableEventType(event.type)) || protectedAggregate)
    ...
    const steal = Effect.fn("SyncHttpApi.steal")(function* (ctx: { payload: typeof SessionPayload.Type }) {
      ...
      if (yield* SessionTaskV2.isProtectedAggregate(db, ctx.payload.sessionID))
        return yield* new HttpApiError.BadRequest({})
