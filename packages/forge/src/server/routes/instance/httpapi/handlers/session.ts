...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const assertMutation = (sessionID: SessionID) =>
      tasks.authorizeMutation({ sessionID }).pipe(
        Effect.catchTag(
    ...
    const cancelDescendants = Effect.fn("SessionHttpApi.cancelDescendants")(function* (sessionID: SessionID) {
      yield* tasks
        .cancelRootWithInterrupt({
          ...
          interrupt: (sessions) =>
            Effect.forEach(
              sessions,
              (childSessionID) =>
                Effect.all([execution.interrupt(childSessionID), runState.cancel(childSessionID)], {
                    ...
                    orElse: () =>
                      Effect.fail(
                        new ServiceUnavailableError({
                          message: `Session execution did not stop within 5 seconds: ${childSessionID}`,
    ...
    const remove = Effect.fn("SessionHttpApi.remove")(function* (ctx: { params: { sessionID: SessionID } }) {
      ...
      yield* SessionError.mapStorageNotFound(
        session.removeCoordinated({
          ...
          beforeRemove: Effect.gen(function* () {
            yield* promptSvc.cancel(ctx.params.sessionID)
            if (!current.shared) return
            yield* shareSvc.unshare(ctx.params.sessionID).pipe(
    ...
    const update = Effect.fn("SessionHttpApi.update")(function* (ctx: {
      ...
      // A number archives, an explicit null unarchives, an omitted field is a no-op.
    ...
    const permissionRespond = Effect.fn("SessionHttpApi.permissionRespond")(function* (ctx: {
      ...
      if (request && request.sessionID !== ctx.params.sessionID)
        return yield* new InvalidRequestError({
          kind: "session_permission_owner",
          message: `Permission request ${ctx.params.permissionID} does not belong to Session ${ctx.params.sessionID}`,
        })
      if (request) yield* assertMutation(request.sessionID)
