...
import { registerLocalPluginSdk } from "@turenlabs/core/plugin/sdk"
  ...
  async function attempt<R>(
    ...
    if (!resolved.ok) {
      if (resolved.stage === "missing") {
        // Missing entrypoints are handled separately so callers can inspect package metadata.
        if (missing) {
