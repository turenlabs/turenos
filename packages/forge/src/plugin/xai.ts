...
const XAI_CLI_PROXY_PROTOCOL = "https:"
const XAI_CLI_PROXY_HOST = "cli-chat-proxy.grok.com"
const XAI_CLI_CLIENT_IDENTIFIER = "grok-shell"
const XAI_CLI_TOKEN_AUTH = "xai-grok-cli"
const XAI_CLI_USER_AGENT = "xai-grok-cli"
// The proxy rejects missing or obsolete client versions. Operators with a
// locally installed Grok CLI can override this without a rebuild.
...
interface XaiAuthPluginOptions {
  ...
  replaceOAuth?: (expected: XaiOAuth, info: XaiOAuth) => Promise<boolean>
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=forge` lets xAI attribute TurenOS-originated
  ...
  if (
    ...
    (tokens.expires_in !== undefined &&
      (typeof tokens.expires_in !== "number" ||
        ...
        tokens.expires_in <= 0 ||
        tokens.expires_in > Math.floor((Number.MAX_SAFE_INTEGER - Date.now()) / 1000)))
  ) {
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            if (!currentAuth) throw new Error("xAI credentials were removed while a request was in flight")
            // Auth can flip from oauth to api mid-session (user re-runs
            // /connect with a pasted key). The loaded SDK still holds the OAuth
            // dummy key, so replace it with the new live key ourselves.
            ...
            const refreshNow = async (expected: XaiOAuth) => {
              if (!refreshPromise) {
                const refreshToken = expected.refresh
                refreshPromise = refreshAccessToken(refreshToken, options)
                  .then(async (tokens) => {
                    ...
                    // Never let a refresh finishing after disconnect or reconnect overwrite
                    // the newer credential. Production supplies the atomic replacement;
                    // direct plugin consumers still get the refreshed token for this request.
                    if (options.replaceOAuth && !(await options.replaceOAuth(expected, next))) {
                      throw new Error("xAI credentials changed while an access token was refreshing")
                    }
                    if (!options.replaceOAuth) {
                      await input.client.auth.set({ providerID: "xai", auth: next }).catch(() => {})
                    }
              ...
              if (options.replaceOAuth) {
                ...
                if (!latest || latest.type !== "oauth" || !sameXaiOAuth(latest, { ...expected, ...refreshed })) {
                  throw new Error("xAI credentials changed while an access token was refreshing")
                }
                return latest
              }
              return { ...expected, ...refreshed }
            }
            ...
            if (expiresSoon) currentAuth = await refreshNow(currentAuth)
            ...
            const dispatch = async (oauth: XaiOAuth) => {
              ...
              // and custom-gateway requests must stay on their original endpoint.
              if (requestUrl?.origin !== XAI_API_ORIGIN) return fetch(requestInput, { ...init, headers })

              const model = await xaiRequestModel(requestInput, init)
              headers.set("X-XAI-Token-Auth", XAI_CLI_TOKEN_AUTH)
              headers.set("x-grok-client-identifier", XAI_CLI_CLIENT_IDENTIFIER)
              headers.set("x-grok-client-version", process.env.GROK_CLI_VERSION?.trim() || XAI_CLI_CLIENT_VERSION)
              headers.set("User-Agent", XAI_CLI_USER_AGENT)
              if (model) headers.set("x-grok-model-override", model)
              else headers.delete("x-grok-model-override")

              requestUrl.protocol = XAI_CLI_PROXY_PROTOCOL
              requestUrl.host = XAI_CLI_PROXY_HOST
              const fetchInput = requestInput instanceof Request ? new Request(requestUrl, requestInput) : requestUrl
              return fetch(fetchInput, { ...init, headers })
            }

            const response = await dispatch(currentAuth)
            if (response.status !== 401) return response
            return dispatch(await refreshNow(currentAuth))
          },
        }
      },
      methods: [
...
function xaiRequestHeaders(requestInput: RequestInfo | URL, init?: RequestInit) {
...
function xaiRequestUrl(requestInput: RequestInfo | URL): URL | undefined {
  try {
    return new URL(requestInput instanceof Request ? requestInput.url : requestInput.toString())
  } catch {
    return undefined
  }
}

async function xaiRequestModel(requestInput: RequestInfo | URL, init?: RequestInit): Promise<string | undefined> {
  const body = init?.body
  if (typeof body === "string") return parseXaiRequestModel(body)
  if (body !== undefined || !(requestInput instanceof Request)) return undefined

  try {
    return parseXaiRequestModel(await requestInput.clone().text())
  } catch {
    return undefined
  }
}

function parseXaiRequestModel(body: string): string | undefined {
  try {
    const parsed: unknown = JSON.parse(body)
    if (typeof parsed !== "object" || parsed === null || !("model" in parsed)) return undefined
...
function sameXaiOAuth(left: XaiOAuth, right: XaiOAuth) {
  return (
