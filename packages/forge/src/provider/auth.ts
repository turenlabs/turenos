...
import type { AuthOAuthResult, Hooks } from "@turenlabs/plugin"
import { serviceUse } from "@turenlabs/core/effect/service-use"
import { Auth } from "@/auth"
import { InstanceState } from "@/effect/instance-state"
import { optional } from "@turenlabs/core/schema"
...
export class Service extends Context.Service<Service, Interface>()("@forge/ProviderAuth") {}
...
const layer: Layer.Layer<Service, never, Auth.Service | Plugin.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const authorize = Effect.fn("ProviderAuth.authorize")(function* (
      ...
      // Both lookups are caller-controlled: the provider may declare no auth hook at all (unknown
      // provider, or a plugin that failed to load) and the method index arrives straight off the
      // wire. Neither may escape as a defect, or the request answers 500 instead of its declared
      // error.
