...
import type { DeepMutable } from "@turenlabs/core/schema"
import { InvalidError, JsonError } from "@turenlabs/core/v1/config/error"
...
export function schema<S extends EffectSchema.Decoder<unknown, never>>(
  ...
  data: unknown,
  source: string,
  // Top-level keys this schema does not declare but the document may legitimately carry, because
  // another reader owns them. The guard below exists to catch typos, and a key a sibling schema
  // defines is not one.
  options?: { readonly allow?: Iterable<string> },
): DeepMutable<S["Type"]> {
  const extra = topLevelExtraKeys(schema, data, options?.allow)
  if (extra.length) {
...
function topLevelExtraKeys(schema: EffectSchema.Top, data: unknown, allow?: Iterable<string>) {
  ...
  if (allow) for (const key of allow) known.add(key)
