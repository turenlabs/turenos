...
const MANAGED_PLIST_DOMAIN = "com.turenlabs.forge.managed"

// Keys injected by macOS/MDM into the managed plist that are not TurenOS config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    ...
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "Forge")
    default:
      return "/etc/forge"
  }
}
...
export function managedConfigDir() {
  return process.env.FORGE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
