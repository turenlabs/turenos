...
it.instance(
  "ask - allows ask actions by default and preserves deny",
  () =>
    Effect.gen(function* () {
      const checks = yield* PermissionChecks.Service
      const permission = yield* Permission.Service
      yield* checks.set(false)
...
it.instance(
  "ask - releases a pending action when checks are disabled",
  () =>
    Effect.gen(function* () {
      const events = yield* EventV2Bridge.Service
      const replies: { requestID: PermissionV1.ID; reply: PermissionV1.Reply }[] = []
      ...
      const id = PermissionV1.ID.make("per_disable_wins")
      const fiber = yield* ask({
        id,
        sessionID: SessionID.make("session_test"),
        permission: "bash",
      ...
      const matched = replies.filter((item) => item.requestID === id)
      expect(matched).toHaveLength(1)
      expect(matched[0]).toMatchObject({ requestID: id, reply: "once" })
      expect(yield* fail(reply({ requestID: id, reply: "once" }))).toBeInstanceOf(PermissionV1.NotFoundError)
    }),
  { git: true },
)

it.instance(
  "reply - wins exactly once before a later permission-check disable",
  () =>
    Effect.gen(function* () {
      const events = yield* EventV2Bridge.Service
      const replies: { requestID: PermissionV1.ID; reply: PermissionV1.Reply }[] = []
      ...
      yield* Effect.addFinalizer(() => unsubscribe)
      const id = PermissionV1.ID.make("per_reply_wins")
      const fiber = yield* ask({
        id,
        sessionID: SessionID.make("session_test"),
        permission: "bash",
