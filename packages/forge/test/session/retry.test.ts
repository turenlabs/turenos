...
describe("session.retry.retryable", () => {
  ...
  test.each([
    ...
    "stream_read_error: response terminated",
    "fetch failed: ECONNRESET",
    "socket hang up",
    "response timed out",
    "upstream connection refused",
  ])("retries transient provider text: %s", (message) => {
    expect(SessionRetry.retryable(wrap(message), retryProvider)).toEqual({ message })
  })

  test("retries numeric OpenRouter-style upstream status codes", () => {
    const message = JSON.stringify({ code: 502, message: "Network connection lost." })
    expect(SessionRetry.retryable(wrap(message), retryProvider)).toEqual({ message })
  })

  test("does not retry exhausted quota even when the provider marks it retryable", () => {
  ...
  test("does not retry a rate-limit error envelope", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
  ...
  test("does not retry deterministic 4xx errors even when marked retryable", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
...
describe("session.message-v2.fromError", () => {
  ...
  test("retries prefixed OpenAI server Error objects", () => {
    const message =
      "server_error: An error occurred while processing your request. You can retry your request, or contact us through our help center at help.openai.com if the error persists. Please include the request ID cd7f128c-6183-41dc-9200-70edd62e94d1 in your message."
    const result = MessageV2.fromError(new Error(message), { providerID: ProviderV2.ID.make("openai") })

    expect(SessionV1.APIError.isInstance(result)).toBe(true)
    if (!SessionV1.APIError.isInstance(result)) throw new Error("expected APIError")
    expect(result.data.isRetryable).toBe(true)
    expect(SessionRetry.retryable(result, retryProvider)).toEqual({ message })
  })

  test("retries code-only server errors without retrying permanent error codes", () => {
    const server = MessageV2.fromError({ type: "error", error: { code: "server_error" } }, { providerID })
    const authentication = MessageV2.fromError(
      {
        type: "error",
        error: { code: "authentication_error", message: "Invalid key, please try again later." },
      },
      { providerID },
    )

    expect(SessionV1.APIError.isInstance(server)).toBe(true)
    if (!SessionV1.APIError.isInstance(server)) throw new Error("expected APIError")
    expect(SessionRetry.retryable(server, retryProvider)).toEqual({ message: "server_error" })
    expect(SessionRetry.retryable(authentication, retryProvider)).toBeUndefined()
  })
})
