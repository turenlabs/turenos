...
describe("Session.Info", () => {
  ...
  test("does not expose legacy share URLs", () => {
    const decoded = decode({
      ...
      version: "1.0.0",
      share: { url: "https://share.example.com/s/secret" },
      shared: true,
      time: { created: 1, updated: 2 },
    }) as Record<string, unknown>

    expect(decoded.shared).toBe(true)
    expect(decoded).not.toHaveProperty("share")
    expect(JSON.stringify(decoded)).not.toContain("share.example.com")
  })
