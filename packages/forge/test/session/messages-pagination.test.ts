...
import { SessionMessageTable, SessionTable, SessionTranscriptAdoptionTable } from "@turenlabs/core/session/sql"
...
const it = testEffect(
  LayerNode.compile(LayerNode.group([EventV2.node, SessionNs.node, MessageV2.node, SessionProjector.node])),
)
...
describe("MessageV2.page", () => {
  ...
  it.instance("fails closed across legacy readers after Session V2 adopts the transcript", () =>
    withSession(({ session, sessionID }) =>
      Effect.gen(function* () {
        const messageID = yield* addUser(sessionID, "legacy content")
        const before = yield* MessageV2.get({ sessionID, messageID })
        const partID = before.parts[0]!.id
        const { db } = yield* Database.Service
        const count = yield* Database.primary(db).$count(SessionTable).pipe(Effect.orDie)
        yield* Database.primary(db)
          ...
          .run()
          .pipe(Effect.orDie)
        const v2OnlyID = MessageID.make("msg_v2_only_after_adoption")
        const currentID = SessionMessage.ID.make(v2OnlyID)
        yield* (yield* EventV2.Service).publish(SessionEvent.Synthetic, {
          sessionID,
          messageID: currentID,
          text: "current-only",
        ...
        expect(
          yield* Database.primary(db)
            ...
            .where(eq(SessionMessageTable.id, currentID))
            .get()
            .pipe(Effect.orDie),
        ).toEqual({ id: currentID })
        const expected = `Legacy transcript is unavailable after Session V2 adoption: ${sessionID}; use the V2 context or history endpoint`

        const page = yield* MessageV2.page({ sessionID, limit: 10 }).pipe(Effect.flip)
        const stream = yield* MessageV2.stream(sessionID).pipe(Effect.flip)
        const message = yield* MessageV2.get({ sessionID, messageID }).pipe(Effect.flip)
        const parts = yield* MessageV2.parts(messageID).pipe(Effect.catchDefect(Effect.succeed))
        const currentParts = yield* MessageV2.parts(v2OnlyID).pipe(Effect.catchDefect(Effect.succeed))
        const messages = yield* session.messages({ sessionID }).pipe(Effect.flip)
        const found = yield* session.findMessage(sessionID, () => true).pipe(Effect.flip)
        const part = yield* session.getPart({ sessionID, messageID, partID }).pipe(Effect.catchDefect(Effect.succeed))
        // `fork` is deliberately absent: it reads the Session V2 transcript, not
        // the legacy tables, and must keep working after adoption. See fork.test.ts.

        for (const error of [page, stream, message, parts, currentParts, messages, found, part]) {
          expect(error).toBeInstanceOf(NotFoundError)
          expect(error instanceof Error ? error.message : "").toBe(expected)
        }
        expect(yield* Database.primary(db).$count(SessionTable).pipe(Effect.orDie)).toBe(count)
      }),
    ),
  )
...
describe("MessageV2.filterCompacted", () => {
  ...
  // A `fork remaps compaction tail_start_id` test lived here. It forked a session
  // assembled from V1 `message`/`part` rows — a shape production stopped producing —
  // and so it passed against a fork that returned 404 for every real session. Forks
  // now copy the V2 transcript, which has no `tail_start_id` to remap: a V2
  // `compaction` message carries its tail inline and history cuts positionally on
  // `seq`. Fork coverage lives in fork.test.ts, against a production-shaped session.
