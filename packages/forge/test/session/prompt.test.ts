...
import { SessionMessageTable } from "@turenlabs/core/session/sql"
...
const localProvider = ProviderTest.openAICompatible("http://localhost:1/v1", providerModel).layer
const httpProvider = Layer.effect(
  Provider.Service,
  Effect.gen(function* () {
    return ProviderTest.openAICompatible((yield* TestLLMServer).url, providerModel).service
  }),
).pipe(Layer.orDie)
const httpProviderNode = LayerNode.make({
...
function makePrompt(input?: { processor?: "blocking"; source?: Layer.Layer<McpTool.Source> }) {
  ...
  if (input?.source && input.processor === "blocking") {
    return LayerNode.compile(promptRoot, [
      ...replacements,
      [McpTool.sourceNode, input.source],
      [SessionProcessor.node, blockingProcessor],
    ] as const)
  }
  if (input?.source) {
    return LayerNode.compile(promptRoot, [...replacements, [McpTool.sourceNode, input.source]] as const)
  }
  if (input?.processor === "blocking") {
    return LayerNode.compile(promptRoot, [...replacements, [SessionProcessor.node, blockingProcessor]] as const)
  }
  return LayerNode.compile(promptRoot, replacements)
}

function makeHttp(input?: { processor?: "blocking"; source?: Layer.Layer<McpTool.Source> }) {
  const root = LayerNode.group([promptRoot, testLLMServerNode])
  const replacements = [
    [SessionSummary.node, summary],
    [LSP.node, lsp],
    [MCP.node, makeMcp()],
    [Provider.node, httpProviderNode],
    [RuntimeFlags.node, runtimeFlags],
  ] as const
  if (input?.source && input.processor === "blocking") {
  ...
  if (input?.source) {
    return LayerNode.compile(root, [...replacements, [McpTool.sourceNode, input.source]] as const)
  }
  if (input?.processor === "blocking") {
    return LayerNode.compile(root, [...replacements, [SessionProcessor.node, blockingProcessor]] as const)
  }
...
const legacyBrokerCalls = { search: 0, load: [] as string[][] }
const legacySource = Layer.succeed(
  McpTool.Source,
  McpTool.Source.of({
    list: () => Effect.succeed([]),
    ...
    search: () =>
      Effect.sync(() => {
        legacyBrokerCalls.search += 1
        return { matches: [], selected: [], available: 0 }
      }),
    load: (input) =>
      Effect.sync(() => {
        legacyBrokerCalls.load.push([...input.tools])
        return { loaded: [...input.tools], selected: [...input.tools], message: "loaded" }
      }),
    touch: () => Effect.void,
  }),
)
const legacyBroker = testEffect(makeHttp({ source: legacySource }))
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://github.com/turenlabs/forge/config.json", ...config }),
  )
})

const useServerConfig = Effect.fn("test.useServerConfig")(function* (config: Partial<ConfigV1.Info>) {
...
it.instance("loop exits without an LLM request for interrupted orphan tool calls", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(testConfig)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Pinned" })
...
legacyBroker.instance("legacy prompt delegates MCP search and load to the V2 source", () =>
  Effect.gen(function* () {
    legacyBrokerCalls.search = 0
    legacyBrokerCalls.load = []
    const { llm } = yield* useServerConfig(testConfig)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    ...
    yield* prompt.prompt({
      ...
      agent: "build",
      noReply: true,
      parts: [{ type: "text", text: "search and load the connected MCP capability" }],
    })
    yield* llm.tool(McpTool.SEARCH_TOOL_NAME, {})
    yield* llm.tool(McpTool.LOAD_TOOL_NAME, { tools: ["notion_notion-search"] })
    yield* llm.text("done")

    const result = yield* prompt.loop({ sessionID: session.id })
    expect(result.info.role).toBe("assistant")
    expect(legacyBrokerCalls.search).toBe(1)
    expect(legacyBrokerCalls.load).toEqual([["notion_notion-search"]])
  }),
)
...
it.instance("glob tool keeps instance context during prompt runs", () =>
  Effect.gen(function* () {
    const { dir, llm } = yield* useServerConfig(testConfig)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const session = yield* sessions.create({
      title: "Glob context",
...
noLLMServer.instance(
  ...
  // Without a pinned provider this fell through to ambient provider discovery and only
  // passed on machines with the `claude` CLI installed; anywhere else — every CI runner —
  // model resolution raised NoProvidersError.
  { config: testConfig },
)
...
it.instance("command execution loads an installed Extension skill", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(testConfig)
    ...
    yield* extensions.update(manifest.id, { enabled: true, manifest })
    const { prompt, chat } = yield* boot()
    yield* llm.text("done")
    ...
    expect(messages).toContain("Corroborate indicators and report confidence")
    expect(messages).toContain("8.8.8.8 include confidence")
  }),
)
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/forge/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("other"), modelID: ModelV2.ID.make("other-model") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
