...
import { LLMError, TransportReason } from "@turenlabs/llm"
...
describe("session.message-v2.fromError", () => {
  test("serializes native stream transport failures as retryable APIError", () => {
    const message =
    ...
    const result = MessageV2.fromError(
      new LLMError({
        ...
        method: "stream",
        reason: new TransportReason({ kind: "Stream", message }),
      }),
      { providerID },
    )
  ...
  test("normalizes validation-wrapped transient stream errors", () => {
    const body = {
      type: "error",
      error: {
        type: "upstream_error",
    ...
    const result = MessageV2.fromError(new Error(`Type validation failed: Value: ${JSON.stringify(body)}`), {
  ...
  test("keeps quota and rate-limit stream envelopes non-retryable", () => {
    const quota = {
      type: "error",
      error: { code: "insufficient_quota", message: "Quota exceeded, please try again later." },
    }
    const rateLimit = {
      error: { type: "rate_limit_error", message: "Concurrency limit exceeded, please try again later." },
    }
    const quotaResult = MessageV2.fromError(new Error(`Provider validation failed: ${JSON.stringify(quota)}`), {
      providerID,
    })
    const rateLimitResult = MessageV2.fromError(JSON.stringify(rateLimit), { providerID })

    expect(SessionV1.APIError.isInstance(quotaResult)).toBe(true)
    expect(SessionV1.APIError.isInstance(rateLimitResult)).toBe(true)
    if (!SessionV1.APIError.isInstance(quotaResult) || !SessionV1.APIError.isInstance(rateLimitResult))
      throw new Error("expected APIErrors")
    expect(quotaResult.data.isRetryable).toBe(false)
    expect(rateLimitResult.data.isRetryable).toBe(false)
  })

  test("extracts status from a plain provider error object", () => {
    const error = Object.assign(new Error("Network connection lost."), { statusCode: 502 })
    const result = MessageV2.fromError(error, { providerID })

    expect(SessionV1.APIError.isInstance(result)).toBe(true)
    if (!SessionV1.APIError.isInstance(result)) throw new Error("expected APIError")
    expect(result.data).toMatchObject({ message: "Network connection lost.", statusCode: 502, isRetryable: true })
  })

  test("does not retry a 400 even when the SDK marks it retryable", () => {
    const result = MessageV2.fromError(
      new APICallError({
        message: "Bad Request",
        ...
        responseBody: JSON.stringify({
          error: { code: "invalid_request_error", message: "Please try your request again" },
        }),
      }),
      { providerID },
    )
  ...
  test("retries a bodyless generic 429 but not a rate-limit envelope", () => {
    const transient = MessageV2.fromError(
      new APICallError({
    ...
    const terminal = MessageV2.fromError(
      new APICallError({
    ...
    expect(SessionV1.APIError.isInstance(transient)).toBe(true)
    expect(SessionV1.APIError.isInstance(terminal)).toBe(true)
    if (!SessionV1.APIError.isInstance(transient) || !SessionV1.APIError.isInstance(terminal))
      throw new Error("expected APIErrors")
    expect(transient.data.isRetryable).toBe(true)
    expect(terminal.data.isRetryable).toBe(false)
  })
  ...
  test("a fresh compaction-user newer than the latest summary surfaces in tasks", () => {
    const newCompactionUser: SessionV1.WithParts = {
      ...
      parts: [
        ...
        {
          ...basePart(NEW_COMPACTION_USER, "p2"),
          type: "text",
          text: "not a task",
        },
        {
          ...basePart(NEW_COMPACTION_USER, "p3"),
          type: "subtask",
    ...
    expect(state.tasks).toMatchObject([
      ...
      { type: "subtask", prompt: "inspect the cache" },
    ])
  })
})
