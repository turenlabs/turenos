...
import { BlockAnchorReplacer, EditTool } from "../../src/tool/edit"
...
describe("BlockAnchorReplacer", () => {
  test("matches long middle lines with unequal lengths", () => {
    const content = ["function configure() {", `  const value = "${"a".repeat(960)}${"b".repeat(32)}"`, "}"].join("\n")
    const find = ["function configure() {", `  const value = "${"a".repeat(1_024)}"`, "}"].join("\n")

    expect([...BlockAnchorReplacer(content, find)]).toEqual([content])
  })
})
