...
function build(
  ...
  McpBroker.clear(ctx.sessionID)
  const available = Object.entries(mcpTools)
  const selected = available.length <= 12 ? available : [...available.slice(0, 11), available.at(-1)!]
  const capabilities = available.map(([key, entry]) => ({
    key,
    server: entry.server,
    name: entry.def.name,
  ...
  McpBroker.beginTurn(ctx.sessionID, capabilities)
  McpBroker.load(
    ctx.sessionID,
    capabilities,
...
describe("code mode execute", () => {
  ...
  test("redacts 1Password results before a code mode program can observe them", async () => {
    const onepassword = {
      ...mcpTool("list_variables", () => ({
        content: [{ type: "text", text: JSON.stringify({ name: "API_KEY", value: "text-canary" }) }],
        structuredContent: { variables: [{ name: "DATABASE_URL", value: "structured-canary" }] },
      })),
      server: "onepassword",
    }
    const tool = await build({ onepassword_list_variables: onepassword }, ["onepassword"])

    const output = await Effect.runPromise(
      tool.execute({ code: "return await tools.onepassword.list_variables({})" }, ctx),
    )

    expect(output.output).not.toContain("text-canary")
    expect(output.output).not.toContain("structured-canary")
    expect(output.output).toContain("[REDACTED]")
    expect(output.output).toContain("DATABASE_URL")
  })
  ...
  test("omits unsupported MCP attachment types before they enter code mode", async () => {
    ...
    expect(out.output).toContain("MCP attachment omitted: audio/wav")
    expect(out.attachments).toBeUndefined()
  })

  test("omits MCP attachments larger than the established 10 MB media limit", async () => {
    const oversized = "A".repeat(Math.ceil(((10 * 1024 * 1024 + 1) * 4) / 3))
    const tool = await build({
      media_image: mcpTool("image", () => ({
        content: [{ type: "image", data: oversized, mimeType: "image/png" }],
      })),
    })
    const out = await Effect.runPromise(tool.execute({ code: "return await tools.media.image({})" }, ctx))
    expect(out.output).toContain("MCP attachment omitted: image/png, 11 MB")
    expect(out.attachments).toBeUndefined()
  })
  ...
  test("leaves oversized results to Forge's native tool-output truncation", async () => {
    const tool = await build({})
  ...
  test("bounds the number of MCP calls admitted by one code-mode execution", async () => {
    const tool = await build({ host_touch: mcpTool("touch", () => ({ content: [] })) })
    const calls = Array.from({ length: 33 }, () => "await tools.host.touch({})").join("\n")
    const error = await failure(tool.execute({ code: `${calls}\nreturn 'done'` }, ctx))
    expect(error.message).toContain("tool-call limit of 32")
  })

  test("bounds model-facing output produced by one code-mode execution", async () => {
    const tool = await build({})
    const output = await Effect.runPromise(tool.execute({ code: "return 'x'.repeat(300000)" }, ctx))
    expect(output.output.length).toBeLessThan(300_000)
