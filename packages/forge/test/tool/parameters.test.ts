...
import { SymbolParameters, WorkspaceSymbolParameters, DefinitionParameters } from "../../src/tool/lsp-symbol"
...
describe("tool parameters", () => {
  describe("JSON Schema (wire shape)", () => {
    ...
    // Structural tools. The snapshots exist to catch a line/character parameter
    // ever creeping back in: these tools are name-addressed on purpose.
    test("references / incoming_calls", () => expect(toJsonSchema(SymbolParameters)).toMatchSnapshot())
    test("workspace_symbol", () => expect(toJsonSchema(WorkspaceSymbolParameters)).toMatchSnapshot())
    test("definition", () => expect(toJsonSchema(DefinitionParameters)).toMatchSnapshot())

    test("structural tools never ask the model for an offset", () => {
      for (const schema of [SymbolParameters, WorkspaceSymbolParameters, DefinitionParameters]) {
        const properties = Object.keys((toJsonSchema(schema) as { properties?: object }).properties ?? {})
        expect(properties).not.toContain("line")
        expect(properties).not.toContain("character")
        expect(properties).not.toContain("offset")
        expect(properties).not.toContain("column")
      }
    })
  ...
  describe("definition", () => {
    test("accepts symbol + path", () => {
      const parsed = parse(DefinitionParameters, { symbol: "OpenRepository", path: "repo.go" })
      expect(parsed).toEqual({ symbol: "OpenRepository", path: "repo.go" })
    })
    test("rejects missing path", () => {
      // path is the only reason to call this tool instead of workspace_symbol;
      // it is required, not optional.
      expect(accepts(DefinitionParameters, { symbol: "OpenRepository" })).toBe(false)
    })
    test("rejects missing symbol", () => {
      expect(accepts(DefinitionParameters, { path: "repo.go" })).toBe(false)
    })
  })
