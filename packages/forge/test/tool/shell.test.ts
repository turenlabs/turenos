...
describe("tool.shell", () => {
  ...
  each("advertises exact process targeting and recursive-delete policy", () =>
    Effect.gen(function* () {
      ...
      yield* runIn(
        ...
        Effect.gen(function* () {
          ...
          expect(tool.description).toContain("PID or process group")
          expect(tool.description).toContain("pkill -f")
          expect(tool.description).toContain("Recursive deletion")
        }),
      )
    }),
  )

  each("hard-denies unbounded recursive deletion before permission", (item) =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped()
      yield* runIn(
        tmp,
        Effect.gen(function* () {
          ...
          const sentinel = new Error("permission should not be reached")
          const command = PS.has(item.label)
            ? "Remove-Item -Recurse -Force $HOME"
            : item.label === "cmd"
              ? "rmdir /s /q %USERPROFILE%"
              : 'rm -rf "$HOME"'
          const error = yield* fail({ command }, capture(requests, sentinel))
          expect(error.message).toContain("Blocked dangerous recursive deletion")
          expect(error.message).not.toBe(sentinel.message)
  ...
  it.live("canonicalizes a symlink workdir before recursive-delete ancestry checks", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return
      const tmp = yield* tmpdirScoped()
      const facade = path.join(tmp, "facade")
      const victim = path.join(tmp, "victim")
      const child = path.join(victim, "child")
      yield* Effect.promise(() => Promise.all([fs.mkdir(facade), fs.mkdir(child, { recursive: true })]))
      yield* Effect.promise(() => fs.symlink(child, path.join(facade, "link")))
      yield* runIn(
        tmp,
        Effect.gen(function* () {
          const requests: Array<Omit<PermissionV1.Request, "id" | "sessionID" | "tool">> = []
          ...
          const error = yield* fail(
            { command: `rm -rf ${quote(victim)}`, workdir: path.join(facade, "link") },
            capture(requests, sentinel),
          )
          expect(error.message).toContain("Blocked dangerous recursive deletion")
          expect(error.message).not.toBe(sentinel.message)
          expect(requests).toEqual([])
  ...
  it.live("rejects a recursive target with a symlink intermediate before permission", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return
      const tmp = yield* tmpdirScoped()
      const cache = path.join(tmp, "cache")
      const protectedDirectory = path.join(tmp, "protected")
      yield* Effect.promise(() =>
        Promise.all([fs.mkdir(cache), fs.mkdir(path.join(protectedDirectory, "child"), { recursive: true })]),
      )
      yield* Effect.promise(() => fs.symlink(protectedDirectory, path.join(cache, "link")))
      yield* runIn(
        tmp,
        Effect.gen(function* () {
          const requests: Array<Omit<PermissionV1.Request, "id" | "sessionID" | "tool">> = []
          const sentinel = new Error("permission should not be reached")
          const error = yield* fail({ command: "rm -rf ./cache/link/child" }, capture(requests, sentinel))
          expect(error.message).toContain("Blocked dangerous recursive deletion")
          expect(error.message).not.toBe(sentinel.message)
          expect(requests).toEqual([])
        }),
...
describe("tool.shell permissions", () => {
  ...
  each("redirects workspace writes to a specialized edit tool before permission", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped()
      yield* runIn(
        tmp,
        Effect.gen(function* () {
          const requests: Array<Omit<PermissionV1.Request, "id" | "sessionID" | "tool">> = []
          expect(yield* fail({ command: "echo test > output.txt" }, capture(requests))).toMatchObject({
            message: expect.stringContaining("Use the edit tool"),
          })
          expect(requests).toEqual([])
        }),
      )
