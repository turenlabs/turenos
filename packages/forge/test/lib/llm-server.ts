...
namespace TestLLMServer {
  export interface Service {
    ...
    readonly pushFactory: (match: Match, factory: (hit: Hit) => Item | Reply) => Effect.Effect<void>
    ...
    readonly aborts: Effect.Effect<number>
  }
}
...
export class TestLLMServer extends Context.Service<TestLLMServer, TestLLMServer.Service>()("@test/LLMServer") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // Disconnect watchers for in-flight `hang` responses, keyed by the TCP
      // front's upstream local port (which the Effect server observes as
      // `incoming.socket.remotePort`). See `tcpFront` below for why the front
      // exists at all.
      ...
      const queueFactory = (match: Match, factory: (hit: Hit) => Item | Reply) => {
        list = [...list, { item: (hit) => item(factory(hit)), match }]
      }
      ...
      const handle = Effect.fn("TestLLMServer.handle")(function* (mode: "chat" | "responses") {
        ...
        if (next.hang) {
          const incoming = NodeHttpServerRequest.toIncomingMessage(req)
          const response = NodeHttpServerRequest.toServerResponse(req)
          ...
          // These are the events a real Node server would fire. They work on
          // Node, so keep them, but on Bun they never fire for this case -- see
          // the watcher registration below.
          incoming.once("aborted", recordDisconnect)
          incoming.socket.once("close", recordDisconnect)
          response.once("close", () => {
            if (!response.writableFinished) recordDisconnect()
          })
          // Bun 1.3.14's node:http *server* stops watching a connection for
          // peer disconnects once the request body has been fully consumed
          // (which `req.json` above always does). After that, no amount of
          // `socket.resume()` or response writes will surface the disconnect --
          // verified with a raw net.Socket client that sends a POST with a body
          // and then destroys itself: the server observes nothing at all, while
          // the identical teardown on a GET or a bodyless POST is reported
          // instantly. That makes `aborted`/`close` useless here, so the socket
          // lifecycle is instead observed on the TCP front (see `tcpFront`),
          // which owns a real net.Socket and does see the client's FIN/RST.
          const remotePort = incoming.socket.remotePort
          if (remotePort !== undefined) {
            watchers.set(remotePort, recordDisconnect)
            response.once("close", () => watchers.delete(remotePort))
          }
        }
      ...
      // A transparent TCP front sits between the client and the Effect HTTP
      // server. Its only job is to give us a `net.Socket` we own, so a client
      // disconnect is observable as a real socket lifecycle event; Bun's
      // node:http server cannot report one once a request body has been read
      // (see the `hang` branch above). Each client connection is paired with an
      // upstream connection, and the upstream's local port is what the Effect
      // server reports as `incoming.socket.remotePort`, so a disconnect is
      // attributed to exactly the request that was hanging on it.
      const tcpFront = yield* Effect.acquireRelease(
        Effect.sync(() => {
          const upstream = server.address._tag === "TcpAddress" ? server.address.port : 0
          const front = Net.createServer((client) => {
            const up = Net.connect(upstream, "127.0.0.1")
            let upPort: number | undefined
            let gone = false
            up.once("connect", () => (upPort = up.localPort))
            client.pipe(up)
            up.pipe(client)
            const clientGone = () => {
              if (gone) return
              gone = true
              if (upPort !== undefined) watchers.get(upPort)?.()
              up.destroy()
            }
            client.once("end", clientGone)
            client.once("close", clientGone)
            client.once("error", clientGone)
            up.once("close", () => client.destroy())
            up.once("error", () => client.destroy())
          })
          front.on("error", () => {})
          return front
        }),
        (front) => Effect.sync(() => front.close()),
      )
      yield* Effect.callback<void>((resume) => {
        tcpFront.listen(0, "127.0.0.1", () => resume(Effect.void))
      })
      const frontAddress = tcpFront.address() as Net.AddressInfo

      return TestLLMServer.of({
        url:
          server.address._tag === "TcpAddress"
            ? `http://127.0.0.1:${frontAddress.port}/v1`
        ...
        pushFactory: Effect.fn("TestLLMServer.pushFactory")(function* (
          match: Match,
          factory: (hit: Hit) => Item | Reply,
        ) {
          queueFactory(match, factory)
        }),
