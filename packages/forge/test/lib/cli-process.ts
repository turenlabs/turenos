...
//   - FORGE_CONFIG_CONTENT      : minimal config inline, no files to find
//   - FORGE_TEST_HOME           : pins os.homedir() → tmpdir
//   - FORGE_DISABLE_PROJECT_CONFIG : skip walking up for forge.json
//   - FORGE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
// different return shape — see the TODO at the bottom of ForgeCli.
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    FORGE_CONFIG_CONTENT: configJson,
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const configJson = JSON.stringify({ formatter: false, lsp: false })
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          FORGE_CONFIG_CONTENT: JSON.stringify({
            formatter: false,
            lsp: false,
    ...
    const opencode: ForgeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }
