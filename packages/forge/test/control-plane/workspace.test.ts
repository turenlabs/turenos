...
import { EventSequenceTable } from "@turenlabs/core/event/sql"
...
const originalEnv = {
  FORGE_EXPERIMENTAL_WORKSPACES: process.env.FORGE_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(recorded.calls.create[0].env.FORGE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.FORGE_EXPERIMENTAL_WORKSPACES).toBe("true")
