...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    test("checks the start directory when it is also the stop boundary", async () => {
      await using tmp = await tmpdir()
      const marker = path.join(tmp.path, "marker")
      await fs.writeFile(marker, "root", "utf-8")

      const result = await Filesystem.findUp("marker", tmp.path, tmp.path)
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "forge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "forge.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "forge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "forge.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["forge.json", "forge.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "forge.json"),
        path.join(project, "forge.jsonc"),
      ])
    })
  })
