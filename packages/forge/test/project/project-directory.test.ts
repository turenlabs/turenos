...
describe("Project directory persistence", () => {
  ...
  it.live("keeps the active directory under its claimed id when an origin appears", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped({ git: true })
      const project = yield* Project.Service
      ...
      // The remote points at a project that already exists, but the directory
      // was claimed on first open and does not defect to it.
  ...
  it.live("keeps recorded directories when an origin appears", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped({ git: true })
      const project = yield* Project.Service
      const original = yield* project.fromDirectory(tmp)
      ...
      expect(yield* directories(remoteID)).toEqual([])
      expect(new Set(yield* directories(original.project.id))).toEqual(
        new Set([
