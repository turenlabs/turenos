...
const grokCliVersion = process.env.GROK_CLI_VERSION?.trim() || "0.2.103"
...
describe("plugin.xai", () => {
  ...
  describe("buildAuthorizeUrl", () => {
    ...
    test("includes required OAuth + PKCE + OIDC params", () => {
      ...
      expect(params.get("referrer")).toBe("forge")
    })
  ...
  describe("loader", () => {
    test("returns no options unless stored auth is OAuth and exposes methods in order", async () => {
      ...
      expect(await hooks.auth!.loader!(async () => undefined as any, {} as any)).toEqual({})
    ...
    test("routes OAuth requests for the default API origin through the Grok CLI proxy", async () => {
      ...
      try {
        ...
        await opts.fetch!("https://api.x.ai/v1/chat/completions?stream=true", {
          headers: { Authorization: `Bearer ${OAUTH_DUMMY_KEY}`, "x-keep": "yes" },
      ...
      expect(captured[0].url).toBe("https://cli-chat-proxy.grok.com/v1/chat/completions?stream=true")
      expect(captured[0].headers.get("authorization")).toBe("Bearer live-token")
      expect(captured[0].headers.get("x-keep")).toBe("yes")
      expect(captured[0].headers.get("x-xai-token-auth")).toBe("xai-grok-cli")
      expect(captured[0].headers.get("x-grok-client-identifier")).toBe("grok-shell")
      expect(captured[0].headers.get("x-grok-client-version")).toBe(grokCliVersion)
      expect(captured[0].headers.get("user-agent")).toBe("xai-grok-cli")
    })

    test("sets the Grok model override from the JSON request body", async () => {
      const { input } = makeInput()
      const { captured, restore } = captureFetch()
      ...
      expect(captured).toHaveLength(1)
      expect(captured[0].headers.get("x-grok-model-override")).toBe("grok-3-mini")
    })

    test("keeps API-key requests on their configured endpoints without proxy headers", async () => {
      const { input } = makeInput()
      let auth: any = { type: "oauth", access: "oauth-token", refresh: "rt", expires: Date.now() + 3600_000 }
      const { captured, restore } = captureFetch()

      try {
        const opts = await (await XaiAuthPlugin(input)).auth!.loader!(async () => auth, {} as any)
        auth = { type: "api", key: "sk-live" }

        await opts.fetch!("https://api.x.ai/v1/chat/completions?stream=true", { headers: {} })
        await opts.fetch!("https://gateway.example/v1/chat/completions?stream=true", { headers: {} })
      } finally {
        restore()
      }

      expect(captured.map((request) => request.url)).toEqual([
        "https://api.x.ai/v1/chat/completions?stream=true",
        "https://gateway.example/v1/chat/completions?stream=true",
      ])
    ...
    test("uses the live API key when stored auth flips from oauth to api", async () => {
      const { input } = makeInput()
      const captured: Headers[] = []
      using server = makeServer((request) => {
        captured.push(request.headers)
      ...
      await opts.fetch!(new URL("/chat/completions", server.url), {
        headers: { Authorization: `Bearer ${OAUTH_DUMMY_KEY}`, "x-keep": "v" },
      })
      expect(captured[0].get("authorization")).toBe("Bearer sk-new")
      expect(captured[0].get("x-keep")).toBe("v")
    })

    test("fails cleanly when auth is removed before an in-flight request reads it", async () => {
      const { input } = makeInput()
      let connected = true
      const opts = await (
        await XaiAuthPlugin(input)
      ).auth!.loader!(async () => {
        if (!connected) return undefined as any
        return { type: "oauth", access: "tok", refresh: "rt", expires: Date.now() + 3600_000 }
      }, {} as any)
      connected = false

      await expect(opts.fetch!("http://127.0.0.1:9/chat/completions", { headers: {} })).rejects.toThrow(
        "xAI credentials were removed while a request was in flight",
      )
    })

    test("does not restore OAuth credentials when they change during refresh", async () => {
      ...
      await expect(opts.fetch!(new URL("/chat/completions", server.url), { headers: {} })).rejects.toThrow(
        "xAI credentials changed while an access token was refreshing",
      )
      expect(replacements).toEqual([
        {
          expected: { type: "oauth", access: "old", refresh: "rt-old", expires: 0 },
          info: {
            type: "oauth",
            access: "new-access",
            refresh: "rt-new",
    ...
    test("does not dispatch when auth is removed after refresh replacement", async () => {
      const { input } = makeInput()
      let auth: any = { type: "oauth", access: "old", refresh: "rt-old", expires: 0 }
      let apiRequests = 0
      using server = makeServer((_, url) => {
        if (url.pathname === "/oauth2/token") {
          return Response.json({ access_token: "new-access", refresh_token: "rt-new", expires_in: 3600 })
    ...
    test("preserves OAuth account metadata during atomic refresh replacement", async () => {
      const { input } = makeInput()
      let auth: any = {
        type: "oauth",
        access: "old",
    ...
    test("retries once after a 401 from a still-unexpired access token", async () => {
      const { input, setCalls } = makeInput()
      let tokenRequests = 0
      const apiRequests: string[] = []
      using server = makeServer((_, url) => {
        ...
        apiRequests.push(_.headers.get("authorization") ?? "")
      ...
      expect(apiRequests[1]).toBe("Bearer new-access")
      expect((setCalls[0].auth as any).access).toBe("new-access")
    })
    ...
    test("rejects malformed successful refresh responses", async () => {
      const { input, setCalls } = makeInput()
      using server = makeServer((_, url) => {
        if (url.pathname === "/oauth2/token") return Response.json({ refresh_token: "rt-new", expires_in: 3600 })
        return new Response("{}", { status: 200 })
      })
      const opts = await (
        await XaiAuthPlugin(input, serverOptions(server))
      ...
      await expect(opts.fetch!(new URL("/chat/completions", server.url), { headers: {} })).rejects.toThrow(
        "xAI token refresh response is invalid",
      )
      expect(setCalls).toEqual([])
    })
  })
