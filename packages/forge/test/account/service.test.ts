...
it.live("orgsByAccount groups orgs per account", () =>
  Effect.gen(function* () {
    ...
    expect(rows.map((row) => [row.account.id, row.orgs.map((org) => org.id)]).map(([id, orgs]) => [id, orgs])).toEqual([
      [one, [OrgID.make("org-1")]],
      [two, [OrgID.make("org-2"), OrgID.make("org-3")]],
    ])
...
it.live("poll stores the account and first org on success", () =>
  Effect.gen(function* () {
    ...
    // The control plane's user id is stored as a lookup attribute, never as
    // the identity the row and its sealed tokens are keyed on.
    const row = yield* AccountRepo.use.getRow(Option.getOrThrow(active).id)
    expect(Option.getOrThrow(row).remote_id).toBe(RemoteAccountID.make("user-1"))
    expect(Option.getOrThrow(row).id).not.toBe("user-1")
  }),
)
