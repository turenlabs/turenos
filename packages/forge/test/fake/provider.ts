...
export namespace ProviderTest {
  ...
  export function fake(override: Partial<Provider.Interface> & { model?: Provider.Model; info?: Provider.Info } = {}) {
    ...
    const service = Provider.Service.of({
    ...
    return {
      ...
      info: row,
      service,
      layer: Layer.succeed(Provider.Service, service),
    }
  }

  /** A fake provider whose language model really talks to `url` over the OpenAI-compatible protocol. */
  export function openAICompatible(url: string, override: Partial<Provider.Model> = {}) {
    const mdl = model({
      id: ModelV2.ID.make("test-model"),
      providerID: ProviderV2.ID.make("test"),
      ...override,
      api: override.api ?? {
        id: override.id ?? ModelV2.ID.make("test-model"),
        url,
        npm: "@ai-sdk/openai-compatible",
      },
    })
    const sdk = createOpenAICompatible({ name: String(mdl.providerID), apiKey: "test-key", baseURL: url })
    return fake({
      model: mdl,
      getLanguage: Effect.fn("TestProvider.getLanguage")(() => Effect.succeed(sdk.languageModel(mdl.api.id))),
    })
  }
}
