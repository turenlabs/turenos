...
describe("session.list", () => {
  it.instance(
    "excludes internal lobby sessions",
    () =>
      Effect.gen(function* () {
        const internal = yield* withSession({ title: "lobby", metadata: { "forge.internal": true } })
        const visible = yield* withSession({ title: "visible" })
        const sessions = yield* SessionNs.use.list()

        expect(sessions.map((session) => session.id)).toContain(visible.id)
        expect(sessions.map((session) => session.id)).not.toContain(internal.id)
        expect(yield* SessionNs.use.get(internal.id)).toMatchObject({ id: internal.id })
      }),
    { git: true },
  )
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "forge", "src", "deep"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))

        const parent = yield* withSession({ title: "parent" }).pipe(
          provideInstance(path.join(test.directory, "packages", "forge")),
        )
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "forge", "src")),
        )
        const deeper = yield* withSession({ title: "deeper" }).pipe(
          provideInstance(path.join(test.directory, "packages", "forge", "src", "deep")),
        )
        const sibling = yield* withSession({ title: "sibling" }).pipe(
          provideInstance(path.join(test.directory, "packages", "app")),
        )
        ...
        if (process.platform === "win32") {
          const windowsPathIDs = (yield* SessionNs.Service.use((session) =>
            session.list({ path: "packages\\forge\\src" }),
          )).map((session) => session.id)
