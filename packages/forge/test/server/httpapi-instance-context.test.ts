...
describe("HttpApi instance context middleware", () => {
  ...
  it.live("provides selected workspace id on control-plane routes", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.get(`/session?workspace=${workspace.id}`).pipe(
        HttpClientRequest.setHeader("x-forge-directory", dir),
        HttpClient.execute,
      )
  ...
  // The failure mode this replaced: an unreadable config surfaced as the bare string
  // "ConfigInvalidError", because NamedError puts its name in `Error.message` and every structured
  // issue sits on `.data`. The message has to name the file and the key or it is unactionable.
  it.live("names the offending config key when a directory cannot be opened", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      yield* Effect.promise(() => writeFile(path.join(dir, "forge.json"), JSON.stringify({ banana: true })))
      yield* serveProbe()
      ...
      expect(body).toContain("Unrecognized key: banana")
      expect(body).toContain(path.join(dir, "forge.json"))
    }),
  )
