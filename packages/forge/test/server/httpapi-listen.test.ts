...
const original = {
  ...
  envUsername: process.env.FORGE_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  else process.env.FORGE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.FORGE_SERVER_USERNAME
  else process.env.FORGE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.FORGE_SERVER_PASSWORD = auth.password
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-forge-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-forge-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // The route must resolve to a real 2xx endpoint: the Effect logger middleware
      // only emits "Sent HTTP response" on a success exit, so probing a missing path
      // would make this assertion vacuously true. GlobalPaths.health is referenced by
      // constant so renaming the route breaks the build instead of silently 404ing.
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const directoryScoped = await fetch(
        ...
        {
          method: "POST",
          headers: { authorization: authorization(), "x-forge-ticket": "1" },
        },
      )
