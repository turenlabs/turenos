      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-forge-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://github.com/turenlabs/forge/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-forge-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-forge-workspace")
