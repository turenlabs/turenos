    ...
    modules.HttpApiApp.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ FORGE_SERVER_PASSWORD: password, FORGE_SERVER_USERNAME: username }),
        ),
      ),
    ),
...
function toAuthProbeRequest(scenario: ActiveScenario, credentials: "missing" | "valid", signal: AbortSignal) {
  ...
  const headers = {
    ...
    // Keep every auth probe inside the exerciser's scratch project. Without this,
    // workspace routing falls back to `process.cwd()` and the probes mutate the repo.
    "x-forge-directory": exerciseAuthDirectory,
    ...spec.headers,
    ...(credentials === "valid" ? { authorization: basic("forge", "secret") } : {}),
  }
  return new Request(new URL(spec.path, "http://localhost"), {
    method: scenario.method,
    headers,
