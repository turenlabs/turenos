...
import { isAllowedCorsOrigin } from "@turenlabs/server/cors"
...
describe("HttpApi CORS", () => {
  test("trusts only the exact Forge Desktop renderer origin", () => {
    expect(isAllowedCorsOrigin("forge-internal://renderer")).toBe(true)
    expect(isAllowedCorsOrigin("forge-internal://renderer.attacker.example")).toBe(false)
  ...
  it.live("does not trust OpenCode origins", () =>
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(
        HttpApiApp.createRoutes().pipe(
          Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ FORGE_SERVER_PASSWORD: "secret" }))),
        ),
        { disableLogger: true },
      ).handler
