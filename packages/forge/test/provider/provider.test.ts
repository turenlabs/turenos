...
const languageHeaders = async (language: unknown) =>
  await (
    language as {
      config: { headers: () => Record<string, string | undefined> | Promise<Record<string, string | undefined>> }
    }
  ).config.headers()
const languageFetch = (language: unknown) =>
  (
    language as {
      config: { fetch: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response> }
    }
  ).config.fetch
const languageFetchError = async (language: unknown) => {
  try {
    await languageFetch(language)("https://api.anthropic.com/v1/messages")
    return ""
  } catch (error) {
...
const it = testEffect(LayerNode.compile(LayerNode.group([Provider.node, Storage.node, Env.node, Plugin.node])))
const experimentalModels = testEffect(providerLayer({ enableExperimentalModels: true }))
const providerWithAuth = testEffect(providerLayer())
...
it.instance("connection policies use an isolated language cache without changing normal reuse", () =>
  Effect.gen(function* () {
    yield* setProcessEnv("ANTHROPIC_API_KEY", "test-api-key")
    const provider = yield* Provider.Service
    const model = yield* provider.getModel(ProviderV2.ID.anthropic, ModelV2.ID.make("claude-sonnet-4-6"))
    const plain = yield* provider.getLanguage(model)
    const plainAgain = yield* provider.getLanguage(model)
    const connectionPolicy = {
      id: "qualified-anthropic",
      origin: "https://api.anthropic.com",
      pathPrefix: "/v1",
      pinnedAddresses: ["203.0.113.10"],
      expiresAt: Date.now() + 60_000,
    }
    const qualified = yield* provider.getLanguage(model, connectionPolicy)
    const qualifiedAgain = yield* provider.getLanguage(model, connectionPolicy)
    const changedPins = yield* provider.getLanguage(model, {
      ...connectionPolicy,
      pinnedAddresses: ["203.0.113.11"],
    })

    expect(plainAgain).toBe(plain)
    expect(qualified).not.toBe(plain)
    expect(qualifiedAgain).toBe(qualified)
    expect(changedPins).not.toBe(qualified)
  }),
)

providerWithAuth.instance(
  "policy cache follows credential rotation, revocation, and instance restart",
  () =>
    Effect.gen(function* () {
      ...
      yield* auth.set("anthropic", new Auth.Api({ type: "api", key: "credential-a" }))
      const model = yield* provider.getModel(ProviderV2.ID.anthropic, ModelV2.ID.make("claude-sonnet-4-6"))
      const connectionPolicy = {
        id: "qualified-anthropic-rotation",
        origin: "https://api.anthropic.com",
        pathPrefix: "/v1",
        pinnedAddresses: ["203.0.113.10"],
        expiresAt: Date.now() + 60_000,
      }
      const credentialA = yield* provider.getLanguage(model, connectionPolicy)
      expect((yield* Effect.promise(() => languageHeaders(credentialA)))["x-api-key"]).toBe("credential-a")

      yield* auth.set("anthropic", new Auth.Api({ type: "api", key: "credential-b" }))
      const credentialB = yield* provider.getLanguage(model, connectionPolicy)
      expect(credentialB).not.toBe(credentialA)
      expect((yield* Effect.promise(() => languageHeaders(credentialB)))["x-api-key"]).toBe("credential-b")
      expect(yield* Effect.promise(() => languageFetchError(credentialA))).toBe("Connection policy transport retired")

      yield* store.reload({ directory: test.directory })
      const restarted = yield* store.provide(
        { directory: test.directory },
        Effect.gen(function* () {
          const restartedModel = yield* provider.getModel(ProviderV2.ID.anthropic, ModelV2.ID.make("claude-sonnet-4-6"))
          return yield* provider.getLanguage(restartedModel, connectionPolicy)
        }),
      )
      expect(restarted).not.toBe(credentialB)
      expect((yield* Effect.promise(() => languageHeaders(restarted)))["x-api-key"]).toBe("credential-b")
      expect(yield* Effect.promise(() => languageFetchError(credentialB))).toBe("Connection policy transport retired")

      yield* auth.remove("anthropic")
      const revoked = yield* store.provide(
        { directory: test.directory },
        provider.getLanguage(model, connectionPolicy).pipe(Effect.exit),
      )
      expect(revoked._tag).toBe("Failure")
      expect(yield* Effect.promise(() => languageFetchError(restarted))).toBe("Connection policy transport retired")
    }),
  { config: { provider: { anthropic: { options: { timeout: 30_000 } } } } },
)

providerWithAuth.instance(
  "policy cache makes late-added credentials sticky until explicit replacement",
  () =>
    Effect.gen(function* () {
      const auth = yield* Auth.Service
      const provider = yield* Provider.Service
      const model = yield* provider.getModel(ProviderV2.ID.anthropic, ModelV2.ID.make("claude-sonnet-4-6"))
      const connectionPolicy = {
        id: "qualified-anthropic-late-auth",
        origin: "https://api.anthropic.com",
        pathPrefix: "/v1",
        pinnedAddresses: ["203.0.113.10"],
        expiresAt: Date.now() + 60_000,
      ...
      yield* auth.set("anthropic", new Auth.Api({ type: "api", key: "late-credential-a" }))
      const credentialA = yield* provider.getLanguage(model, connectionPolicy)
      expect((yield* Effect.promise(() => languageHeaders(credentialA)))["x-api-key"]).toBe("late-credential-a")

      yield* auth.remove("anthropic")
      const revoked = yield* provider.getLanguage(model, connectionPolicy).pipe(Effect.exit)
      expect(revoked._tag).toBe("Failure")
      expect(yield* Effect.promise(() => languageFetchError(credentialA))).toBe("Connection policy transport retired")
    }),
  { config: { provider: { anthropic: { options: { apiKey: "configured-fallback" } } } } },
)
...
test("normalizes Ollama endpoints and only accepts loopback hosts", () => {
  expect(Provider.normalizeOllamaEndpoint("localhost:11434/v1")).toBe("http://localhost:11434")
  expect(Provider.normalizeOllamaEndpoint("https://[::1]:11434")).toBe("https://[::1]:11434")
  expect(Provider.normalizeOllamaEndpoint("http://192.168.1.10:11434")).toBeUndefined()
  expect(Provider.normalizeOllamaEndpoint("http://user:password@127.0.0.1:11434")).toBeUndefined()
})

test("converts Ollama tags into exact OpenAI-compatible model records", () => {
  const models = Provider.ollamaModelsFromTags(
    {
      models: [
        { name: "qwen3:8b", modified_at: "2026-08-01T00:00:00Z", details: { family: "qwen2" } },
        { model: "llama3.2:3b" },
        { name: "" },
        { name: "__proto__" },
        { name: "constructor" },
  ...
  expect(Object.keys(models)).toEqual(["qwen3:8b", "llama3.2:3b"])
  expect(models["qwen3:8b"]).toMatchObject({
    providerID: "ollama",
    name: "qwen3:8b",
    family: "qwen2",
    api: {
...
it.instance("discovers Ollama models from a local server", () =>
  Effect.acquireUseRelease(
    Effect.sync(() =>
      Bun.serve({
        port: 0,
        fetch: (request) => {
          if (new URL(request.url).pathname !== "/api/tags") return new Response("not found", { status: 404 })
          return Response.json({ models: [{ name: "qwen3:8b", details: { family: "qwen2" } }] })
        },
      }),
    ),
    (server) =>
      Effect.gen(function* () {
        yield* set("OLLAMA_HOST", server.url.origin)
        const providers = yield* list
        const ollama = providers[ProviderV2.ID.make("ollama")]
        expect(ollama).toBeDefined()
        expect(ollama.name).toBe("Ollama")
        expect(ollama.models[ModelV2.ID.make("qwen3:8b")]).toBeDefined()
        expect(ollama.options.baseURL).toBe(`${server.url.origin}/v1`)
      }),
...
it.instance(
  "keeps statically configured Ollama models on the loopback endpoint",
  Effect.gen(function* () {
    const providers = yield* list
    const ollama = providers[ProviderV2.ID.make("ollama")]
    expect(ollama).toBeDefined()
...
it.instance(
  "does not load Ollama models from a remote endpoint",
  Effect.gen(function* () {
    const providers = yield* list
  ...
  }),
  {
    config: {
      provider: {
        ollama: {
          npm: "@ai-sdk/openai-compatible",
          api: "http://192.168.1.10:11434/v1",
          models: { "qwen3:8b": { name: "Qwen 3 8B" } },
        },
      },
    },
...
it.instance("defaultModel observes desktop model state updates from Storage", () =>
  Effect.gen(function* () {
    yield* setProcessEnv("ANTHROPIC_API_KEY", "test-api-key")
    const initial = yield* Provider.use.defaultModel()
    const providers = yield* Provider.use.list()
    const target = Object.values(providers[initial.providerID].models).find((item) => item.id !== initial.modelID)
    expect(target).toBeDefined()

    const storage = yield* Storage.Service
    yield* storage.set({
      scope: Storage.Scope.make("desktop/store/model-state"),
      key: Storage.Key.make("model"),
      value: JSON.stringify({
        recent: [{ providerID: initial.providerID, modelID: target!.id }],
        favorite: [],
        variant: {},
      }),
    })

    const updated = yield* Provider.use.defaultModel()
    expect(updated).toEqual({ providerID: initial.providerID, modelID: target!.id })
  }),
)
...
it.instance(
  "model config preserves empty generated variants",
  Effect.gen(function* () {
  ...
  }),
  {
    config: {
      provider: {
        openai: { models: { "custom-gpt-chat": { id: "gpt-4o-mini", name: "Custom GPT Chat" } } },
      },
    },
  },
)
...
test("V2 catalog projection preserves models for the public provider list", () => {
  ...
  const models = ["daybreak-blue-latest", "daybreak-red-latest", "gpt-5.6-cyber"].map((id) =>
    ModelV2.Info.make({
      ...ModelV2.Info.empty(providerID, ModelV2.ID.make(id)),
      name: id,
      api: { id: ModelV2.ID.make(id), type: "aisdk", package: "@ai-sdk/openai" },
      capabilities: { tools: true, input: ["text", "image"], output: ["text"] },
      variants: [{ id: ModelV2.VariantID.make("medium"), headers: {}, body: { reasoningEffort: "medium" } }],
  ...
  const result = Provider.fromCatalogProvider(provider, models)
  expect(Object.keys(result.models)).toEqual(["daybreak-blue-latest", "daybreak-red-latest", "gpt-5.6-cyber"])
  expect(result.models["daybreak-blue-latest"]).toMatchObject({
    api: { id: "daybreak-blue-latest", npm: "@ai-sdk/openai", url: "https://api.openai.com/v1" },
    capabilities: {
      reasoning: true,
      toolcall: true,
...
test("public provider info omits credentials", () => {
  const provider = Provider.fromModelsDevProvider({
    id: "test",
    name: "Test",
    env: [],
  ...
  provider.options = {
    ...
    region: "us-east-1",
    nested: { auth: { token: "nested-secret" }, retries: 2 },
    aiGatewayHeaders: { Authorization: "Bearer nested-header-secret" },
    serviceKey: "sap-secret",
  ...
  provider.models.valid.options = { token: "model-secret", temperature: 0.5 }
  provider.models.valid.headers = { Authorization: "Bearer header-secret" }

  const result = Provider.toPublicInfo(provider)

  expect(JSON.stringify(result)).not.toContain("secret")
  expect(result.options).toEqual({ region: "us-east-1", nested: { retries: 2 } })
  expect(result.models.valid.options).toEqual({ temperature: 0.5 })
  expect(result.models.valid.headers).toEqual({})
})

test("Kimi catalog preserves the AI SDK Anthropic adapter base URL", () => {
  const provider = Provider.fromModelsDevProvider({
    id: "kimi-for-coding",
  ...
  expect(provider.models.k3.api).toEqual({
    id: "k3",
    npm: "@ai-sdk/anthropic",
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".forge")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
it.effect("retired opencode stays hidden while opencode-go loads with a credential", () =>
  Effect.gen(function* () {
    ...
    const keyedDir = yield* tmpdirScoped({
      config: {
        provider: {
          opencode: { options: { apiKey: "test-key" } },
          "opencode-go": { options: { apiKey: "test-key" } },
        },
      },
    })
