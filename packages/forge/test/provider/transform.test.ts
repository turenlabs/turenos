...
describe("ProviderTransform.providerOptions", () => {
  ...
  test("uses the xAI SDK key for custom provider IDs", () => {
    const model = createModel({
      providerID: "custom-xai",
      api: {
        id: "grok-4.6",
    ...
    expect(ProviderTransform.providerOptions(model, { store: false, promptCacheKey: "session-123" })).toEqual({
      xai: { store: false, promptCacheKey: "session-123" },
    })
  })
...
describe("ProviderTransform.schema - moonshot $ref siblings", () => {
  ...
  test("also runs for the Kimi For Coding K3 alias", () => {
    const result = ProviderTransform.schema(
      {
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".forge/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    expect(result[0].content[0].providerOptions?.compatible?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.compatible?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          compatible: { itemId: "msg_compatible" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              compatible: { itemId: "msg_compatible_part" },
    ...
    expect(result[0].providerOptions?.compatible?.itemId).toBe("msg_compatible")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.compatible?.itemId).toBe("msg_compatible_part")
...
describe("ProviderTransform.variants", () => {
  ...
  describe("@ai-sdk/openai", () => {
    ...
    for (const testCase of [
      ...
      { id: "gpt-5.6", releaseDate: "2026-07-09", efforts: ["none", "low", "medium", "high", "xhigh", "max"] },
