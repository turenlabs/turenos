...
test("release workflow requires every signing identity before building", () => {
  ...
  expect(workflow).toContain("script/build.ts ${{ matrix.settings.build_flags || '--skip-install --platform' }}")
...
test("release workflow signs and requires every published desktop format", () => {
  ...
  expect(workflow).toContain('gh api "repos/$GH_REPO/releases/$release_id"')
})
