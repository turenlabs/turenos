...
import { ConfigToolOutput } from "@turenlabs/core/config/tool-output"
...
describe("ToolOutputStore", () => {
  it.live("writes binary artifacts without UTF-8 conversion", () =>
    withStore(({ store, fs }) =>
      Effect.gen(function* () {
        const bytes = Uint8Array.from([0, 255, 1, 254])
        const file = yield* store.writeBytes(bytes)
        expect(yield* fs.readFile(file)).toEqual(bytes)
      }),
    ),
  )

  it.live("preserves Unicode code points at the tail byte boundary", () =>
    Effect.sync(() => {
      const marker = "[cut]"
      const result = ToolOutputStore.boundedPreview(`${"A".repeat(100_000)}🙂${"B".repeat(7)}`, marker, 10, 31)
      expect(result).toBe(`${"A".repeat(11)}\n\n${marker}\n\n🙂${"B".repeat(7)}`)
      expect(Buffer.byteLength(result, "utf-8")).toBe(31)
    }),
  )
