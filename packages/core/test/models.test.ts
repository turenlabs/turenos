...
import { InstallationChannel, InstallationVersion } from "@turenlabs/core/installation/version"
import { ModelsDev } from "@turenlabs/core/models-dev"
import { it } from "./lib/effect"
import { readFile, rm, writeFile, utimes, mkdir } from "fs/promises"
import path from "path"

// test/preload.ts pins FORGE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.FORGE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.FORGE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.FORGE_MODELS_PATH = undefined
  Flag.FORGE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.FORGE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.FORGE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.FORGE_DISABLE_MODELS_FETCH = false
        }),
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toBe(`forge/${InstallationChannel}/${InstallationVersion}/${Flag.FORGE_CLIENT}`)
    }),
  )
