...
/**
 * Defects open at 2df5e268 that the safe fuzz run steers around. Remove an entry
 * when its fix lands; the strict run exercises all of them.
 * - resume-quota: never resume a terminal orchestrator.
 * - reconcile-conflict: restart only when no active orchestrator has an active worker.
 * - queued-spawn-op: tolerate `pending-spawn` violations; never retry a spawn that did not start.
 * - unstarted-send: resume only tasks that started.
 * - grant-widening: spawn workers only within their orchestrator's grant.
 */
...
export type KnownIssue = (typeof knownIssues)[number]
 ...
 */
const fuzzSeed = Effect.fnUntraced(function* (seed: number, steps: number, known: ReadonlySet<KnownIssue>) {
  let state = seed
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const operation: Operation = yield* Effect.gen(function* () {
      ...
      if (roll < 0.45 && orchestrators.length > 0) {
        ...
        const widen = !known.has("grant-widening") && random() < 0.3
        ...
        return {
          ...
          // A fixed admission may reject widening with a typed error.
      ...
      const resumable = all.filter(
        (task) =>
          ...
          !(known.has("resume-quota") && task.authority.orchestrate === true) &&
          !(known.has("unstarted-send") && task.time.started === undefined),
      )
      if (roll < 0.86 && resumable.length > 0) {
        ...
        return {
          ...
          allowed: ["SessionTask.ActiveLimitError", "SessionTask.OrchestrateError"],
        }
      }
      const retryable = spawned.filter(
        (item) => !(known.has("queued-spawn-op") && byID.get(item.taskID)?.time.started === undefined),
      )
      if (roll < 0.94 && retryable.length > 0) {
        const item = pick(retryable)
        return {
          name: "retry spawn",
          run: tasks
            .spawn(item.input)
      ...
      const conflicted = all.some(
        (task) => isActive(task) && task.depth === 2 && isActive(byID.get(task.parentTaskID!)!),
      )
      if (known.has("reconcile-conflict") && conflicted) return { name: "idle", run: Effect.succeed("") }
    ...
    problems.push(
      ...(yield* violations(rootSessionID, limit)).filter(
        (problem) => !(known.has("queued-spawn-op") && problem.startsWith("pending-spawn:")),
      ),
    )
 ...
 */
export const fuzz = (input: {
  readonly seeds: ReadonlyArray<number>
  readonly steps: number
  readonly known: ReadonlyArray<KnownIssue>
}) =>
  Effect.forEach(input.seeds, (seed) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        return yield* fuzzSeed(seed, input.steps, new Set(input.known)).pipe(Effect.provide(context))
      }),
    ),
