...
describe("ProjectDirectories", () => {
  ...
  it.effect("finds the SQL project that owns a directory", () =>
    Effect.gen(function* () {
      yield* setup()
      const service = yield* ProjectDirectories.Service
      yield* service.create({ projectID, directory, strategy: "git_worktree" })

      expect(yield* service.find(directory)).toEqual({ projectID, directory, strategy: "git_worktree" })
      expect(yield* service.find(AbsolutePath.make("/tmp/missing-project-directory"))).toBeUndefined()
    }),
  )
