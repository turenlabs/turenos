...
import { AnthropicPlugin } from "@turenlabs/core/plugin/provider/anthropic"
...
describe("AnthropicPlugin", () => {
  it.effect("preserves provider and model-specific beta headers", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) => {
        const item = ProviderV2.Info.make({
          ...
          request: { headers: { Existing: "1", "anthropic-beta": "context-1m-2025-08-07" }, body: {} },
        })
        catalog.provider.update(item.id, (draft) => {
          draft.api = item.api
          draft.request = item.request
        })
        catalog.model.update(item.id, ModelV2.ID.make("claude-opus-5-fast"), (model) => {
          model.api = { id: ModelV2.ID.make("claude-opus-5"), type: "aisdk", package: "@ai-sdk/anthropic" }
          model.request.headers["anthropic-beta"] = "fast-mode-2026-02-01"
        })
      })
      yield* addPlugin()
      ...
      expect(
        required(yield* catalog.model.get(ProviderV2.ID.anthropic, ModelV2.ID.make("claude-opus-5-fast"))).request
          .headers["anthropic-beta"],
      ).toBe("fast-mode-2026-02-01")
    }),
  )
