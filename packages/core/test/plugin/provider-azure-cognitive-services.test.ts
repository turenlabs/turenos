...
import { AzureCognitiveServicesPlugin } from "@turenlabs/core/plugin/provider/azure"
...
describe("AzureCognitiveServicesPlugin", () => {
  ...
  it.effect("maps the current models.dev Azure package to the Cognitive Services endpoint", () =>
    withEnv({ AZURE_COGNITIVE_SERVICES_RESOURCE_NAME: "current-cognitive" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          catalog.provider.update(ProviderV2.ID.make("azure-cognitive-services"), (item) => {
        ...
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("azure-cognitive-services"))).api).toEqual({
          type: "aisdk",
          package: "@ai-sdk/azure",
          url: "https://current-cognitive.cognitiveservices.azure.com/openai",
        })
      }),
    ),
  )
