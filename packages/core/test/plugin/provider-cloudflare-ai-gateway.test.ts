...
import { CloudflareAIGatewayPlugin } from "@turenlabs/core/plugin/provider/cloudflare-ai-gateway"
...
describe("CloudflareAIGatewayPlugin", () => {
  it.effect("qualifies the SessionRunner bridge and derives the configured Cloudflare gateway", () =>
    withEnv(
      cloudflareEnv({
        ...
        Effect.gen(function* () {
          ...
          const resolved = yield* SessionRunnerModel.fromCatalogModelWithAISDK(
            ...
            Credential.Key.make({
              type: "key",
              key: "bridge-token",
              metadata: { accountId: "bridge-account", gatewayId: "bridge-gateway", tenant: "must-not-project" },
            }),
          )
          ...
          expect(JSON.stringify(aiGatewayCalls[0])).not.toContain("must-not-project")
          expect(unifiedCalls).toEqual(["openai/gpt-5"])
        }),
    ),
  )
