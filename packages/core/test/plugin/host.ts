...
import type { IntegrationEnvMethod, IntegrationKeyMethod, IntegrationOAuthMethod } from "@turenlabs/sdk/v2/types"
        ...
        callback({
          ...
          update: (id, update) =>
            draft.update(AgentV2.ID.make(id), (value) => {
              ...
              update(current)
              // batou:ignore prototype_pollution -- test double; `current` is a shallow copy of a decoded AgentV2.Info made two lines above, and `request.headers` is Forge config, not an HTTP request
          ...
          provider: {
            ...
            update: (id, update) =>
              draft.provider.update(ProviderV2.ID.make(id), (value) => {
                ...
                update(current)
                // batou:ignore prototype_pollution -- test double; `current` is a shallow copy of a decoded ProviderV2.MutableInfo made two lines above, and `request.headers` is Forge config, not an HTTP request
          ...
          model: {
            ...
            update: (providerID, modelID, update) =>
              draft.model.update(ProviderV2.ID.make(providerID), ModelV2.ID.make(modelID), (value) => {
                ...
                update(current)
                // batou:ignore prototype_pollution -- test double; `current` is a shallow copy of a decoded ModelV2.Info made two lines above, and `request.headers` is Forge config, not an HTTP request
