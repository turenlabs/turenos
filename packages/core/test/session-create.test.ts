...
describe("SessionV2.create", () => {
  ...
  it.effect("filters archived sessions before applying the page limit", () =>
    Effect.gen(function* () {
      const session = yield* SessionV2.Service
      ...
      expect(yield* session.list({ archived: false, limit: 1, roots: true })).toEqual([active])
      expect(yield* session.list({ archived: true, limit: 1, roots: true })).toMatchObject([{ id: archived.id }])
    }),
  )

  it.effect("filters inactive sessions by updated time before applying the page limit", () =>
    Effect.gen(function* () {
      const session = yield* SessionV2.Service
      const { db } = yield* Database.Service
      const active = yield* session.create({ location })
      const boundary = yield* session.create({ location })
      const older = yield* session.create({ location })
      ...
      expect(
        (yield* session.list({ inactive: false, inactivityThreshold: 1_000, limit: 1, roots: true })).map(
          (item) => item.id,
        ),
      ).toEqual([active.id])
      expect(
        (yield* session.list({ inactive: true, inactivityThreshold: 1_000, limit: 1, roots: true })).map(
          (item) => item.id,
        ),
      ).toEqual([boundary.id])
    }),
  ...
  it.effect("persists metadata and excludes internal sessions from lists", () =>
    Effect.gen(function* () {
      const session = yield* SessionV2.Service
      const internal = yield* session.create({ location, metadata: { "forge.internal": true, origin: "lobby" } })
      const legacyLobby = yield* session.create({ id: SessionV2.ID.make("ses_lobby_legacy"), location })
      const visible = yield* session.create({ location })

      expect((yield* session.get(internal.id)).metadata).toEqual({ "forge.internal": true, origin: "lobby" })
      expect((yield* session.get(legacyLobby.id)).id).toBe(legacyLobby.id)
      expect((yield* session.list()).map((item) => item.id)).toEqual([visible.id])
      expect((yield* session.list({ internal: "lobby" })).map((item) => item.id)).toEqual(
        expect.arrayContaining([visible.id, legacyLobby.id, internal.id]),
      )
    }),
  )
  ...
  it.effect("reports the unfinished skill operation as unavailable", () =>
    Effect.gen(function* () {
      const session = yield* SessionV2.Service
      const created = yield* session.create({ location })
      const unavailable = (
