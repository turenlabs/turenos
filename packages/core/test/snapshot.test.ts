...
describe("Snapshot", () => {
  ...
  testEffect(Layer.empty).live("keeps the shadow repository when the worktree moves", () =>
    Effect.acquireUseRelease(
      Effect.promise(() => tmpdir()),
      (tmp) =>
        Effect.gen(function* () {
          ...
          const database = Database.layerFromPath(path.join(tmp.path, "snapshot.sqlite"))
          const before = yield* Effect.gen(function* () {
            return yield* (yield* Snapshot.Service).capture()
          }).pipe(Effect.provide(snapshotLayer(tmp.path, project, database)))
          expect(before).toBeDefined()
          if (!before) return

          yield* Effect.promise(async () => {
            await fs.rename(project, renamed)
            await fs.writeFile(path.join(renamed, "tracked.txt"), "two\n")
          })
          ...
          const projectID = yield* Effect.gen(function* () {
            ...
            Effect.provide(
              AppNodeBuilder.build(Location.boundNode(Location.Ref.make({ directory: AbsolutePath.make(renamed) })), [
          ...
          }).pipe(Effect.provide(snapshotLayer(tmp.path, renamed, database)))

          expect(yield* Effect.promise(() => fs.readdir(path.join(tmp.path, "snapshot", projectID)))).toEqual([
            Hash.fast(project),
          ])
          expect(yield* read(path.join(renamed, "tracked.txt"))).toBe("one\n")

          yield* Effect.promise(() => fs.writeFile(path.join(renamed, "tracked.txt"), "three\n"))
          yield* Effect.gen(function* () {
            const snapshot = yield* Snapshot.Service
            yield* snapshot.restore({ files: new Map([[RelativePath.make("tracked.txt"), before]]) })
          }).pipe(Effect.provide(snapshotLayer(tmp.path, renamed, database)))
...
function snapshotLayer(data: string, directory: string, database?: Layer.Layer<Database.Service>) {
  ...
  if (database) replacements.push([Database.node, database])
  return AppNodeBuilder.build(Snapshot.node, replacements)
}
