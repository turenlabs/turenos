...
describe("SystemContext", () => {
  ...
  it.effect("renders removals in stable key order with one previous-key enumeration", () =>
    Effect.gen(function* () {
      const observations = { enumerations: 0 }
      const previous = new Proxy(
        {
      ...
      expect(yield* SystemContext.reconcile(SystemContext.empty, previous)).toMatchObject({
        _tag: "Updated",
        text: "Removed a\n\nRemoved z",
      })
      expect(observations.enumerations).toBe(1)
    }),
  )
