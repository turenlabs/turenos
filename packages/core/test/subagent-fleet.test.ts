...
describe("subagent fleet", () => {
  ...
  // Settling an orchestrator retires its whole subtree (decided on PR #93): a
  // running worker would hold a slot while reporting to a settled parent.
  it.effect("cancels the whole worker subtree when an orchestrator settles", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* tasks.getMany(workers.map((item) => item.task.id))).map((task) => task.status),
      ).toEqual(["cancelled", "cancelled", "cancelled", "cancelled"])
      expect(yield* tasks.get(workers[0].task.id)).toMatchObject({
        status: "cancelled",
        error: "Owning orchestrator settled before this subagent finished.",
      })
...
describe("subagent fleet fuzzing", () => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        expect(yield* fuzz({ seeds, steps })).toEqual([])
      }),
    timeout,
  )
})

// Regression tests for the defects the PR #93 review found at 2df5e268.
describe("subagent fleet regression", () => {
  it.effect("holds a resumed terminal orchestrator to the orchestrator quota", () =>
    Effect.gen(function* () {
  ...
  it.effect("recovers orchestrators with running workers in one reconcile pass", () =>
    Effect.gen(function* () {
  ...
  it.effect("denies a worker the commands its orchestrator was not granted", () =>
    Effect.gen(function* () {
  ...
  it.effect("keeps a worker's write roots within its orchestrator's", () =>
    Effect.gen(function* () {
  ...
  it.effect("settles the spawn operation when a queued task is cancelled", () =>
    Effect.gen(function* () {
  ...
  it.effect("does not promote a queued task that has a pending interrupt", () =>
    Effect.gen(function* () {
  ...
  it.effect("rejects a send to a worker that recovery interrupted before it started", () =>
    Effect.gen(function* () {
  ...
  it.effect("cancels tasks that recovery interrupted before they started", () =>
    Effect.gen(function* () {
      const rootSessionID = yield* setup("unstarted_cancel")
      const tasks = yield* SessionTaskV2.Service
      const store = yield* SessionStore.Service
      const orchestrator = yield* spawnFrom(rootSessionID, "unstarted_cancel_o", 2, { authority: orchestrating })
      yield* spawnFrom(rootSessionID, "unstarted_cancel_t", 2)
      const first = yield* spawnFrom(orchestrator.task.childSessionID, "unstarted_cancel_w1", 2)
      const second = yield* spawnFrom(orchestrator.task.childSessionID, "unstarted_cancel_w2", 2)
      expect([first.task.status, second.task.status]).toEqual(["queued", "queued"])

      // Recovery interrupts the orchestrator, and its subtree pass interrupts
      // both queued workers before either created a child Session.
      yield* tasks.reconcile()
      for (const item of [first.task, second.task]) {
        expect(yield* tasks.get(item.id)).toMatchObject({ status: "interrupted" })
        expect(yield* store.get(item.childSessionID)).toBeUndefined()
      }

      // A direct cancel on a never-started interrupted task has no child
      // Session to place.
      yield* cancel(first.task)
      expect(yield* tasks.get(first.task.id)).toMatchObject({ status: "cancelled" })

      // Resume the orchestrator, then cancel it: the subtree pass retires the
      // never-started worker without demanding a child Session.
      yield* tasks.send({
        actor: yield* actor(rootSessionID, "unstarted_cancel_send", "send_agent"),
        taskID: orchestrator.task.id,
        prompt: Prompt.make({ text: "Resume" }),
        activeLimit: 2,
      })
      yield* cancel(orchestrator.task)
      expect(yield* tasks.get(second.task.id)).toMatchObject({ status: "cancelled" })
      yield* expectInvariants(rootSessionID, 2)
    }),
  )

  it.effect("charges orchestrator-spawned workers to the @swarm budget", () =>
    Effect.gen(function* () {
  ...
  it.live("keeps promoting other roots when one root's limit cannot be resolved", () =>
    Effect.gen(function* () {
      const tasks = yield* SessionTaskV2.Service
