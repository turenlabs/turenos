      ...
      ([active, outside]) =>
        Effect.gen(function* () {
          ...
          expect(
            ...
            type: "error",
            value: "Unable to edit [redacted path]",
          })
            ...
            withTool(tmp.path, (registry) =>
              Effect.gen(function* () {
                ...
                const missing = yield* executeTool(
                  registry,
                  call({ path: "secret.txt", oldString: "not present", newString: "replacement" }, "call-edit-missing"),
                )
            ...
            withTool(tmp.path, (registry) =>
              Effect.gen(function* () {
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "same", newString: "same" }, "call-edit-noop"),
                  ),
                ).toEqual({
                  type: "error",
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "", newString: "after" }, "call-edit-empty"),
                  ),
                ).toEqual({
                  type: "error",
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "missing", newString: "after" }, "call-edit-absent"),
                  ),
                ).toEqual({
                  type: "error",
                  value:
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "same", newString: "after" }, "call-edit-ambiguous"),
                  ),
                ).toEqual({
                  type: "error",
                  value:
