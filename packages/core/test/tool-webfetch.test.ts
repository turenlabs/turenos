...
import { WebFetchTool } from "@turenlabs/core/tool/webfetch"
...
describe("WebFetchTool registration", () => {
  ...
  it.effect("rejects non-HTTP schemes before permission or transport", () =>
    Effect.gen(function* () {
      ...
      expect(yield* executeTool(registry, call({ url: "file:///etc/passwd", format: "text" }))).toEqual({
        type: "error",
        value: "Unable to fetch [redacted path]",
      })
  ...
  it.effect("converts HTML to requested markdown and text", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1", format: "markdown" }, "call-webfetch-markdown")),
      ).toEqual({
        type: "text",
        value: "# Hello\n\nworld",
      })
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1", format: "text" }, "call-webfetch-text")),
      ).toEqual({
        type: "text",
        value: "Helloworld",
      })
  ...
  it.effect("rejects declared and streamed oversized bodies", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* executeTool(
          registry,
          call({ url: "https://1.1.1.1/declared", format: "text" }, "call-webfetch-declared"),
        ),
      ).toEqual({
        type: "error",
      ...
      expect(
        yield* executeTool(
          registry,
          call({ url: "https://1.1.1.1/streamed", format: "text" }, "call-webfetch-streamed"),
        ),
      ).toEqual({
        type: "error",
  ...
  it.effect("keeps images and files unsupported until typed settlement can carry attachments", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/image", format: "html" }, "call-webfetch-image")),
      ).toEqual({
        type: "error",
      ...
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/file", format: "html" }, "call-webfetch-file")),
      ).toEqual({
        type: "error",
