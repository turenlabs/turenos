...
describe("ConfigAgentPlugin.Plugin", () => {
  ...
  it.effect("lets configuration deterministically override or disable built-in specialist IDs", () =>
    Effect.gen(function* () {
      const agents = yield* AgentV2.Service
      yield* agents.transform((editor) => {
        editor.update(AgentV2.ID.make("worker"), (agent) => {
          agent.system = "Built-in worker."
          agent.description = "Built-in description"
          agent.mode = "subagent"
          agent.permissions.push({ action: "edit", resource: "*", effect: "allow" })
        })
        editor.update(AgentV2.ID.make("research"), (agent) => {
          agent.mode = "subagent"
        })
      })
      const config = Config.Service.of({
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                agents: {
                  worker: {
                    system: "Custom worker.",
                    description: "Organization worker",
                    mode: "primary",
                    permissions: [{ action: "edit", resource: "*", effect: "deny" }],
                  },
                  research: { disabled: true },
                  "security-review": {
                    description: "Organization review specialist",
                    mode: "subagent",
                  },
                },
              }),
      ...
      expect((yield* agents.all()).filter((agent) => agent.id === AgentV2.ID.make("worker"))).toHaveLength(1)
      expect(yield* agents.get(AgentV2.ID.make("worker"))).toMatchObject({
        system: "Custom worker.",
        description: "Organization worker",
        mode: "primary",
        permissions: [
      ...
      expect(yield* agents.get(AgentV2.ID.make("research"))).toBeUndefined()
      expect(yield* agents.get(AgentV2.ID.make("security-review"))).toMatchObject({
        description: "Organization review specialist",
        mode: "subagent",
      })
    }),
...
Use supported v2 fields.`,
          ...
          expect(yield* agents.get(AgentV2.ID.make("native"))).toMatchObject({
            system: "Use supported v2 fields.",
            permissions: [{ action: "edit", resource: "*", effect: "deny" }],
          })
