...
const forgeClient = process.env.FORGE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (forgeClient === undefined) delete process.env.FORGE_CLIENT
  else process.env.FORGE_CLIENT = forgeClient
})
...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["forge.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.FORGE_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES = "forge.client=web,service.instance.id=override,service.namespace=anomalyco"
    ...
    expect(resource().attributes["forge.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})
