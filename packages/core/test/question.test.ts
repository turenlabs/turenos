...
describe("QuestionV2", () => {
  ...
  it.effect("publishes rejection when a pending ask is interrupted", () =>
    Effect.gen(function* () {
      const service = yield* QuestionV2.Service
      const events = yield* EventV2.Service
      const published: EventV2.Payload[] = []
      ...
      expect(published.map((event) => [event.type, event.data])).toEqual([
        [QuestionV2.Event.Asked.type, request],
        [QuestionV2.Event.Rejected.type, { sessionID, requestID: request.id }],
      ])
    }),
  )

  it.effect("allows only one concurrent terminal operation to claim a request", () =>
    Effect.gen(function* () {
      const service = yield* QuestionV2.Service
      const events = yield* EventV2.Service
      const published: EventV2.Payload[] = []
      ...
      yield* Effect.addFinalizer(() => unsubscribe)
      const { fiber, request } = yield* waitForAsk(service, { sessionID, questions: [question] })
      const replyFiber = yield* service.reply({ requestID: request.id, answers: [["One"]] }).pipe(Effect.forkScoped)
      const rejectFiber = yield* service.reject(request.id).pipe(Effect.forkScoped)

      const exits = yield* Effect.all([Fiber.await(replyFiber), Fiber.await(rejectFiber)])
      yield* Fiber.await(fiber)

      expect(exits.filter(Exit.isSuccess)).toHaveLength(1)
      expect(published.filter((event) => event.type !== QuestionV2.Event.Asked.type)).toHaveLength(1)
      expect(yield* service.list()).toEqual([])
    }),
  )
