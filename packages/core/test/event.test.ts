...
describe("EventV2", () => {
  ...
  it.effect("reads the aggregate tail sequence without decoding history", () =>
    Effect.gen(function* () {
      const events = yield* EventV2.Service
      const { db } = yield* Database.Service
...
test("file-backed reader pools serialize durable publishes and reconcile exact concurrent replays", async () => {
  await using tmp = await tmpdir()
  const database = Database.layerFromPath(path.join(tmp.path, "event.sqlite"))
  const app = AppNodeBuilder.build(LayerNode.group([Database.node, EventV2.node]), [[Database.node, database]])

  await Effect.runPromise(
    Effect.gen(function* () {
      const events = yield* EventV2.Service
      const { db } = yield* Database.Service
      const aggregateID = Session.ID.make("ses_reader_pool")
      expect(isWithReplicas(db)).toBe(true)

      const published = yield* Effect.forEach(
        Array.from({ length: 32 }, (_, index) => index),
        (index) => events.publish(DurableMessage, durableData(aggregateID, `concurrent-${index}`)),
        { concurrency: "unbounded" },
      )
      expect(published.map((event) => event.durable?.seq).toSorted((a, b) => (a ?? -1) - (b ?? -1))).toEqual(
        Array.from({ length: 32 }, (_, index) => index),
      )
      ...
      const replayed = {
        ...
        seq: 32,
        aggregateID,
        data: durableData(aggregateID, "exact-replay"),
      }
      yield* Effect.all([events.replay(replayed), events.replay(replayed)], { concurrency: "unbounded" })
      ...
      expect(rows.filter((row) => row.id === replayed.id)).toHaveLength(1)
      expect(rows.map((row) => row.seq).toSorted((a, b) => a - b)).toEqual(
