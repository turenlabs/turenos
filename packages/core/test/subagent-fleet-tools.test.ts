...
describe("subagent fleet tools", () => {
  ...
  // Known defect from the PR #93 review at 2df5e268, reproduced through the tools a
  // model calls. Asserts the fixed behavior; drop `.failing` when the fix lands.
  it.effect.failing("holds an orchestrator resumed through send_agent to the orchestrator quota", () =>
    Effect.gen(function* () {
