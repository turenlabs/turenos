...
import { ReadTool } from "@turenlabs/core/tool/read"
import { ReadToolFileSystem } from "@turenlabs/core/tool/read-filesystem"
...
describe("ReadTool", () => {
  ...
  it.effect("registers, authorizes, and reads through the location filesystem", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        // The model gets the file body as bare text; the uri/name/encoding/mime
        // envelope stays in `structured` for the desktop and never hits the wire.
  ...
  it.effect("lists a bounded directory page through read", () =>
    Effect.gen(function* () {
      ...
      ).toEqual({ type: "text", value: "[empty directory]" })
  ...
  it.effect("forwards pagination and returns bounded text pages with continuation", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        // Body plus only the load-bearing paging state — the mime/offset envelope
        // stays durable-only.
