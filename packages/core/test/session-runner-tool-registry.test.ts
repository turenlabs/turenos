...
describe("ToolRegistry", () => {
  ...
  it.effect("keeps provider-turn session overlays isolated and above Location tools", () =>
    Effect.gen(function* () {
      const service = yield* ToolRegistry.Service
      yield* service.register({ echo: make() })
      const sessionTool = (value: string) =>
        Tool.make({
          description: "Session-local echo",
          input: Schema.Struct({ text: Schema.String }),
          output: Schema.Struct({ text: Schema.String }),
          execute: () => Effect.succeed({ text: value }),
          toModelOutput: ({ output }) => [{ type: "text", text: output.text }],
        })
      const first = yield* service.materialize({ session: { echo: sessionTool("first") } })
      const second = yield* service.materialize({ session: { echo: sessionTool("second") } })

      expect((yield* first.settle(call("echo", "call-overlay-first"))).result).toEqual({ type: "text", value: "first" })
      expect((yield* second.settle(call("echo", "call-overlay-second"))).result).toEqual({
        type: "text",
        value: "second",
      })
      expect(
        (yield* service
          .materialize()
          .pipe(Effect.flatMap((tools) => tools.settle(call("echo", "call-overlay-location"))))).result,
      ).toEqual({ type: "text", value: "echo" })
    }),
  )

  it.effect("executes one durable settlement for exact and concurrent retries", () =>
    Effect.gen(function* () {
      const service = yield* ToolRegistry.Service
      ...
      yield* service.register({
        durable: Tool.make({
          ...
          execute: ({ text }) =>
            Effect.sync(() => executions.push(text)).pipe(
      ...
      const input = call("durable", "call-durable-once")
      const first = yield* tools.settle(input).pipe(Effect.forkChild)
      yield* Deferred.await(started)
      const second = yield* tools.settle(input).pipe(Effect.forkChild)
      yield* Deferred.succeed(release, undefined)

      expect((yield* Fiber.join(first)).result).toEqual({ type: "text", value: "durable" })
      expect((yield* Fiber.join(second)).result).toEqual({ type: "text", value: "durable" })
      expect((yield* tools.settle(input)).result).toEqual({ type: "text", value: "durable" })
      expect(executions).toEqual(["durable"])

      const conflict = yield* tools.settle({
        ...input,
        call: { ...input.call, input: { text: "changed" } },
      })
      expect(conflict.result).toEqual({
        type: "error",
        value: "Tool call identity was reused with a different request",
      })
      expect(executions).toEqual(["durable"])
    }),
  )

  it.effect("marks interrupted execution indeterminate instead of replaying it", () =>
    Effect.gen(function* () {
      const service = yield* ToolRegistry.Service
      const started = yield* Deferred.make<void>()
      let executions = 0
      yield* service.register({
        ambiguous: Tool.make({
          description: "Ambiguous execution",
          input: Schema.Struct({ text: Schema.String }),
          output: Schema.String,
          execute: () =>
      ...
      const tools = yield* service.materialize()
      const input = call("ambiguous", "call-ambiguous")
      const running = yield* tools.settle(input).pipe(Effect.forkChild)
      yield* Deferred.await(started)
      yield* Fiber.interrupt(running)

      expect((yield* tools.settle(input)).result).toEqual({
        type: "error",
        value: "Tool call outcome is indeterminate and cannot be safely replayed",
      })
      expect(executions).toBe(1)
    }),
  )

  it.effect("filters session overlays without leaking them into later materializations", () =>
    Effect.gen(function* () {
      const service = yield* ToolRegistry.Service
      const overlay = yield* service.materialize({
        permissions: [{ action: "spawn_agent", resource: "*", effect: "deny" }],
        session: { spawn_agent: make(), list_agents: make() },
      })

      expect(overlay.definitions.map((tool) => tool.name)).toEqual(["list_agents"])
      const inheritedDeny = yield* service.materialize({
        permissionSets: [
          [{ action: "*", resource: "*", effect: "allow" }],
          [{ action: "spawn_agent", resource: "*", effect: "deny" }],
        ],
        session: { spawn_agent: make(), list_agents: make() },
      })
      expect(inheritedDeny.definitions.map((tool) => tool.name)).toEqual(["list_agents"])
  ...
  it.effect("returns model errors without swallowing interruption or defects", () =>
    Effect.gen(function* () {
            ...
            Effect.fail(
              new Tool.Failure({
                message: [
                  ...
                  "/Users/private/work",
                  "/private/tmp/forge.sock",
                  "sk-abcdefghijklmnopqrstuv",
                  "https://example.test/result?access_token=query-secret",
                  "-----BEGIN PRIVATE KEY-----\nprivate-key-secret\n-----END PRIVATE KEY-----",
                  "x".repeat(5_000),
                ].join(" "),
              }),
      ...
      const redacted = yield* executeTool(service, {
        sessionID,
        ...identity,
        call: { type: "tool-call", id: "redacted", name: "redacted", input: {} },
      })
      expect(redacted.type).toBe("error")
      if (redacted.type !== "error") return
      expect(typeof redacted.value).toBe("string")
      if (typeof redacted.value !== "string") return
      expect(redacted.value.length).toBeLessThanOrEqual(4_096)
      expect(redacted.value).toContain("authorization=[redacted]")
      expect(redacted.value).toContain("$HOME/work")
      expect(redacted.value).toContain("[redacted path]")
      expect(redacted.value).toContain("[redacted credential]")
      expect(redacted.value).toContain("[redacted credential URL]")
      expect(redacted.value).not.toContain("super-secret")
      expect(redacted.value).not.toContain("/Users/private")
      expect(redacted.value).not.toContain("/private/tmp")
      expect(redacted.value).not.toContain("sk-abcdefghijklmnopqrstuv")
      expect(redacted.value).not.toContain("query-secret")
      expect(redacted.value).not.toContain("private-key-secret")
      expect(
        yield* executeTool(service, {
          sessionID,
          ...identity,
