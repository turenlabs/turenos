...
import { Message, Model } from "@turenlabs/llm"
import * as OpenAIChat from "@turenlabs/llm/protocols/openai-chat"
import { ModelV2 } from "@turenlabs/core/model"
import { ProviderV2 } from "@turenlabs/core/provider"
import { SessionMessage } from "@turenlabs/core/session/message"
import { AgentAttachment, FileAttachment, TextPart } from "@turenlabs/core/session/prompt"
...
describe("toLLMMessages", () => {
  test("lowers structured user text in order while omitting ignored parts", () => {
    const messages = toLLMMessages(
      [
        SessionMessage.User.make({
          id: id("structured-user"),
          type: "user",
          text: "Aggregate text must not replace structured parts",
          parts: [
  ...
  test("omits an ignored-only user turn without losing system updates or valid attachments", () => {
    const file = FileAttachment.make({
      uri: "data:image/png;base64,aGVsbG8=",
      mime: "image/png",
      name: "evidence.png",
    })
    const agent = AgentAttachment.make({ name: "review" })
    const messages = toLLMMessages(
      [
        SessionMessage.User.make({
          id: id("ignored-only"),
          ...
          parts: [TextPart.make({ id: "prt_ignored_only", text: "Ignored", ignored: true })],
          metadata: { forge: { legacy: { system: "Preserve this system update" } } },
          time: { created },
        }),
        SessionMessage.User.make({
          id: id("ignored-with-file"),
          type: "user",
          text: "Aggregate text must not leak",
          parts: [TextPart.make({ id: "prt_ignored_with_file", text: "Ignored", ignored: true })],
          files: [file],
          agents: [agent],
          time: { created },
        }),
    ...
    expect(messages).toEqual([
      Message.system("Preserve this system update"),
      Message.make({
        id: id("ignored-with-file"),
        role: "user",
        content: [
    ...
    expect(JSON.stringify(messages)).not.toContain("Aggregate text must not leak")
  })
  ...
  test("maps every top-level V2 Session message type", () => {
    ...
    expect(messages.map((message) => message.role)).toEqual(["system", "user", "user", "user", "user", "user"])
...
Recent work
      ...
      [{ type: "text", text: expect.stringContaining("Resume immediately") }],
    ])
  })

  test("keeps the memory handoff actionable after manual compaction", () => {
    const messages = toLLMMessages(
      [
        SessionMessage.Compaction.make({
          id: id("manual_compaction"),
          type: "compaction",
          reason: "manual",
          summary: "## Durable Memories\n- Preserve the diagnosed failure",
          recent: "",
          time: { created },
        }),
        SessionMessage.User.make({
          id: id("after_manual_compaction"),
          type: "user",
          text: "Continue",
          time: { created },
        }),
  ...
  test("ships the durable-fact ledger alongside the summary and the resume directive", () => {
    const messages = toLLMMessages(
      [
        SessionMessage.Compaction.make({
          id: id("ledger_compaction"),
          type: "compaction",
          reason: "auto",
          summary: "## Objective\n- Finish the port",
          recent: "Recent work",
          ledger: [
            "- packages/core/src/session/compaction.ts owns the checkpoint",
            "- the user requires the ledger to stay append-only",
          ],
          time: { created },
        }),
      ],
    ...
    const checkpoint = messages[0]?.content
...
Recent work
    ...
    // The ledger must not displace the continuation directive: a request that ends on a
    // checkpoint is still the resumable post-compaction turn.
    expect(messages.at(-1)?.content).toEqual([{ type: "text", text: expect.stringContaining("Resume immediately") }])
  })

  test("a checkpoint written before the ledger existed lowers to byte-identical text", () => {
    // Old rows carry no `ledger` at all. They must not gain an empty `<durable-facts>` block --
    // that would change the prompt prefix of every adopted session and invalidate its cache.
    const withoutField = SessionMessage.Compaction.make({
      id: id("legacy_compaction"),
      type: "compaction",
      reason: "auto",
      summary: "Earlier work",
      recent: "Recent work",
    ...
    const lowered = toLLMMessages([withoutField], model)[0]?.content
...
Recent work
    ...
    // An empty ledger is the same case: a compaction whose extraction returned nothing.
    expect(toLLMMessages([{ ...withoutField, ledger: [] }], model)[0]?.content).toEqual(lowered)
  })

  test("replays settled local tool results in order without structured base64", () => {
    const messages = toLLMMessages(
      [
        SessionMessage.Assistant.make({
          id: id("assistant"),
