...
describe("SessionRunCoordinator", () => {
  ...
  it.effect("claims pending Automation work without forcing a provider turn", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const forces: boolean[] = []
        const coordinator = yield* SessionRunCoordinator.make<string, never>({
          drain: (_key, force) => Effect.sync(() => forces.push(force)),
        })

        const join = yield* coordinator.claimPending("automation-session")
        yield* join
        expect(forces).toEqual([false])
      }),
    ),
  ...
  it.effect("returns from wake before starting the drain prelude", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const started = yield* Deferred.make<void>()
        const gate = yield* Deferred.make<void>()
        ...
        const coordinator = yield* SessionRunCoordinator.make({
          drain: () =>
            Effect.sync(() => {
              running = true
            }).pipe(Effect.andThen(Deferred.succeed(started, undefined)), Effect.andThen(Deferred.await(gate))),
        })
  ...
  it.effect("passes only process-local active, wake, and interrupt controls into drains", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const sibling = yield* Deferred.make<void>()
        const seen: string[][] = []
        const coordinator = yield* SessionRunCoordinator.make<string, never>({
          drain: (key, _force, control) =>
            Effect.gen(function* () {
              seen.push(Array.from(yield* control.active).sort())
              if (key === "parent") yield* control.wake("child")
              if (key === "child") yield* Deferred.succeed(sibling, undefined)
            }),
        })

        yield* coordinator.wake("parent")
        yield* Deferred.await(sibling)
        expect(seen.some((active) => active.includes("parent"))).toBeTrue()
        expect(seen.some((active) => active.includes("child"))).toBeTrue()
        expect(seen.flat().every((key) => key === "parent" || key === "child")).toBeTrue()
      }),
    ),
  )
  ...
  it.effect("queues a forced follow-up behind an active execution", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const firstStarted = yield* Deferred.make<void>()
        const firstGate = yield* Deferred.make<void>()
        ...
        yield* Deferred.await(firstStarted)
        yield* coordinator.wakeForced("session")
        yield* Deferred.succeed(firstGate, undefined)
        yield* Deferred.await(secondStarted)
  ...
  it.effect("starts a forced provider turn while idle", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const drained = yield* Deferred.make<void>()
        const forces: boolean[] = []
  ...
  it.effect("does not start an idle adoption wake", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const forces: boolean[] = []
        const coordinator = yield* SessionRunCoordinator.make<string, never>({
        ...
        yield* coordinator.wakeForced("session", false)
        yield* Effect.yieldNow
  ...
  it.effect("does not force an adoption retry after a failed drain", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const firstStarted = yield* Deferred.make<void>()
        const firstGate = yield* Deferred.make<void>()
