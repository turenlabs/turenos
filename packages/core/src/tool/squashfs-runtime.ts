export * as SquashfsRuntime from "./squashfs-runtime"
...
export interface Request {
  readonly op: "squashfs_list" | "squashfs_extract"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/SquashfsRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("squashfs operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`squashfs worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "squashfs", "squashfs-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "squashfs-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./squashfs-worker.${extension}`, import.meta.url)
}
