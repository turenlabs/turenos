export * as CapaMatchRuntime from "./capa-match-runtime"
...
export type Request =
  | {
      readonly op: "capa_match" | "capa_features"
      readonly bytes: Uint8Array
      readonly options: Readonly<Record<string, unknown>>
    }
  | {
      readonly op: "capa_ruleset"
      readonly options: Readonly<Record<string, unknown>>
    }
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/CapaMatchRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("capa match operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`capa match worker exited with code ${code}`) })
                })
                ...
                if (request.op === "capa_ruleset") return worker.postMessage(request)
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "capa-match", "capa-match-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "capa-match-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./capa-match-worker.${extension}`, import.meta.url)
}
