...
export function selectProvider(
  sessionID: string,
  enabled: { readonly enableExa: boolean; readonly enableParallel: boolean },
): Provider | undefined {
  if (!enabled.enableExa && !enabled.enableParallel) return undefined
  if (!enabled.enableParallel) return "exa"
  if (!enabled.enableExa) return "parallel"
...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const parallel = Extension.ID.make("turenlabs", "websearch-parallel")
        ...
        [name]: Tool.make({
          ...
          execute: (input, context) => {
            return Effect.gen(function* () {
              const enabled = {
                enableExa: yield* extensions.enabled(exa),
                enableParallel: yield* extensions.enabled(parallel),
              }
              const provider = selectProvider(context.sessionID, enabled)
              if (!provider) return yield* new ToolFailure({ message: "No web search extension is enabled" })
              ...
              const apiKey = yield* extensions.secret(
                provider === "exa" ? exa : parallel,
                provider === "exa" ? "EXA_API_KEY" : "PARALLEL_API_KEY",
              )
              const text =
                provider === "exa"
                  ? yield* callMcp(http, exaUrl(apiKey), "web_search_exa", ExaArgs, {
                      query: input.query,
...
export const node = makeLocationNode({
  name: "tool/websearch",
  layer,
  deps: [ToolRegistry.node, PermissionV2.node, LayerNodePlatform.httpClient, ExtensionRuntime.node],
})
