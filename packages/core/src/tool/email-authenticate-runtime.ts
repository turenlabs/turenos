export * as EmailAuthenticateRuntime from "./email-authenticate-runtime"
...
export interface Interface {
  readonly authenticate: (input: Input) => Effect.Effect<Result, Error>
}

export class Service extends Context.Service<Service, Interface>()("@forge/v2/EmailAuthenticateRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("email authentication timed out after 5000ms") }),
                  5_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`email authentication worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "email-authenticate", "email-authenticate-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "email-authenticate-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./email-authenticate-worker.${extension}`, import.meta.url)
}
