export * as JsonQueryRuntime from "./json-query-runtime"
...
export type Request =
  | {
      readonly op: "json_query" | "json_paths"
      readonly bytes: Uint8Array
      readonly options: Readonly<Record<string, unknown>>
    }
  | {
      readonly op: "json_validate" | "json_stats"
      readonly bytes: Uint8Array
    }
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/JsonQueryRuntime") {}
          ...
          Effect.tryPromise({
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                // jaq has no fuel limiter: the 60s wall-clock bound and worker
                // termination on abort are the only guard against hostile filters.
                const timer = setTimeout(
                  () => finish({ error: new Error("json query operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`json query worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "json-query", "json-query-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "json-query-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./json-query-worker.${extension}`, import.meta.url)
}
