export * as PdfInspectRuntime from "./pdf-inspect-runtime"
...
export interface Request {
  readonly op: "pdf_inspect" | "pdf_objects" | "pdf_stream_decode" | "pdf_text"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/PdfInspectRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("pdf inspect operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`pdf inspect worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "pdf-inspect", "pdf-inspect-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "pdf-inspect-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./pdf-inspect-worker.${extension}`, import.meta.url)
}
