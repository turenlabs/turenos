export * as SqliteInspectRuntime from "./sqlite-inspect-runtime"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/SqliteInspectRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("sqlite inspect operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`sqlite inspect worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "sqlite-inspect", "sqlite-inspect-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "sqlite-inspect-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./sqlite-inspect-worker.${extension}`, import.meta.url)
}
