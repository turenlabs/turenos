export * as ImageInspectRuntime from "./image-inspect-runtime"
...
export interface Request {
  readonly op: "image_inspect" | "image_exif" | "image_pixel_stats" | "image_text_chunks"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/ImageInspectRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("image inspect operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`image inspect worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "image-inspect", "image-inspect-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "image-inspect-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./image-inspect-worker.${extension}`, import.meta.url)
}
