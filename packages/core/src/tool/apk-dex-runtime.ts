export * as ApkDexRuntime from "./apk-dex-runtime"
...
export interface Request {
  readonly op: "apk_inspect" | "axml_decode" | "dex_inspect"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/ApkDexRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("apk dex operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`apk dex worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "apk-dex", "apk-dex-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "apk-dex-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./apk-dex-worker.${extension}`, import.meta.url)
}
