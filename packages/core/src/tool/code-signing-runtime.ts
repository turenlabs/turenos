export * as CodeSigningRuntime from "./code-signing-runtime"
...
export interface Request {
  readonly op: "cert_inspect" | "crl_inspect" | "macho_codesign" | "pe_authenticode" | "pkcs7_inspect"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/CodeSigningRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("code signing operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`code signing worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "code-signing", "code-signing-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "code-signing-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./code-signing-worker.${extension}`, import.meta.url)
}
