export * as MacosArtifactsRuntime from "./macos-artifacts-runtime"
...
export interface Request {
  readonly op: "plist_parse" | "fsevents_parse" | "unified_log_parse" | "ds_store_parse" | "analyze"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/MacosArtifactsRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("macos artifacts operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`macos artifacts worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "macos-artifacts", "macos-artifacts-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "macos-artifacts-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./macos-artifacts-worker.${extension}`, import.meta.url)
}
