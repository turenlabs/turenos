export * as DebugSymbolsRuntime from "./debug-symbols-runtime"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/DebugSymbolsRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("debug symbol inspection timed out after 15000ms") }),
                  15_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`debug symbols worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "debug-symbols", "debug-symbols-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "debug-symbols-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./debug-symbols-worker.${extension}`, import.meta.url)
}
