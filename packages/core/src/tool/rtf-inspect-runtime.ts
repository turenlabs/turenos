export * as RtfInspectRuntime from "./rtf-inspect-runtime"
...
export interface Request {
  readonly op: "rtf_inspect" | "rtf_objects" | "rtf_audit" | "rtf_text"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/RtfInspectRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("rtf inspect operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`rtf inspect worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "rtf-inspect", "rtf-inspect-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "rtf-inspect-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./rtf-inspect-worker.${extension}`, import.meta.url)
}
