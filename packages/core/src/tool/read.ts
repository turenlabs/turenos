    ...
    yield* tools
      .register({
        [name]: Tool.make({
          ...
          input: Input,
          output: Output,
          // Text reads used to leave `content` empty, so the provider received the whole
          // structured envelope as an escaped JSON string — `uri`, `name`, `encoding`, a
          // `mime` field that mislabels `.ts` as `video/mp2t`, and the file body paying a
          // second escaping pass. ~40% of every read's wire cost was envelope, re-sent on
          // every turn. The envelope stays in `structured` for the desktop; the model gets
          // the text plus only the load-bearing paging state.
          toModelOutput: ({ input, output }) => {
            ...
            const paging = (page: { truncated: boolean; next?: number }) =>
              page.truncated && page.next !== undefined ? `\n[truncated — continue with offset ${page.next}]` : ""
            if ("entries" in output) {
              const listing = output.entries.map((entry) => entry.path).join("\n")
              return [{ type: "text", text: (listing || "[empty directory]") + paging(output) }]
            }
            const body = output.content || "[empty file]"
            return [{ type: "text", text: "truncated" in output ? body + paging(output) : body }]
          },
