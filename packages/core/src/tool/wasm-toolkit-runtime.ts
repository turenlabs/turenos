export * as WasmToolkitRuntime from "./wasm-toolkit-runtime"
...
export interface Request {
  readonly op: "wasm_analyze" | "wasm_metadata" | "wasm_print" | "wat_compile"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/WasmToolkitRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("wasm toolkit operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`wasm toolkit worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "wasm-toolkit", "wasm-toolkit-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "wasm-toolkit-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./wasm-toolkit-worker.${extension}`, import.meta.url)
}
