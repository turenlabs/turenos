export * as InstallerInspectRuntime from "./installer-inspect-runtime"
...
export interface Request {
  readonly op: "msi_inspect" | "msi_stream_read" | "cab_list" | "cab_extract"
...
export type Result =
  ...
  | { readonly type: "bytes"; readonly bytes: Uint8Array; readonly report: Readonly<Record<string, unknown>> }
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/InstallerInspectRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("installer inspect operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`installer inspect worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources
    ? path.join(resources, "installer-inspect", "installer-inspect-worker.js")
    : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "installer-inspect-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./installer-inspect-worker.${extension}`, import.meta.url)
}
