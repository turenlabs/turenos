export * as FuzzyHashRuntime from "./fuzzy-hash-runtime"
...
export type Request =
  | { readonly op: "hash_all"; readonly bytes: Uint8Array }
  | { readonly op: "fuzzy_hash"; readonly bytes: Uint8Array; readonly algorithm: "ssdeep" | "tlsh" }
  | {
      readonly op: "fuzzy_compare"
      readonly algorithm: "ssdeep" | "tlsh"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/FuzzyHashRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("fuzzy hash operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`fuzzy hash worker exited with code ${code}`) })
                })
                ...
                if (request.op === "fuzzy_compare") return worker.postMessage(request)
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "fuzzy-hash", "fuzzy-hash-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "fuzzy-hash-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./fuzzy-hash-worker.${extension}`, import.meta.url)
}
