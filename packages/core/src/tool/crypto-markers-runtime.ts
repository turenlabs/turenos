export * as CryptoMarkersRuntime from "./crypto-markers-runtime"
...
export interface Request {
  readonly op: "byte_stats" | "crypto_constants" | "entropy_map" | "xor_probe"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/CryptoMarkersRuntime") {}
            ...
            try: (signal) =>
              new Promise<Result>((resolve, reject) => {
                ...
                const timer = setTimeout(
                  () => finish({ error: new Error("crypto markers operation timed out after 60000ms") }),
                  60_000,
                )
                worker.once("error", (error) => finish({ error }))
                worker.once("exit", (code) => {
                  if (code !== 0) finish({ error: new Error(`crypto markers worker exited with code ${code}`) })
                })
...
function workerURL() {
  ...
  const packaged = resources ? path.join(resources, "crypto-markers", "crypto-markers-worker.js") : undefined
  if (packaged && existsSync(packaged)) return pathToFileURL(packaged)
  const executable = path.join(path.dirname(process.execPath), "crypto-markers-worker.js")
  if (existsSync(executable)) return pathToFileURL(executable)
  const extension = import.meta.url.endsWith(".ts") ? "ts" : "js"
  return new URL(`./crypto-markers-worker.${extension}`, import.meta.url)
}
