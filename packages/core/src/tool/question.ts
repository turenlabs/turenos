        ...
        [name]: Tool.make({
          ...
          execute: (input, context) =>
            permission
              ...
              .pipe(
                // A correction carries the user's own words back to the model;
                // flattening it to a generic denial would discard that feedback.
                Effect.mapError((error) =>
                  error._tag === "PermissionV2.CorrectedError"
                    ? new ToolFailure({ message: error.feedback })
                    : new ToolFailure({ message: "Permission denied: question" }),
                ),
                Effect.andThen(
                  question
                    .ask({
