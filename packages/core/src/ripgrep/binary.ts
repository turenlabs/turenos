...
export namespace RipgrepBinary {
  ...
  export class Service extends Context.Service<Service, Interface>()("@forge/RipgrepBinary") {}
        ...
        filepath: yield* Effect.cached(
          Effect.gen(function* () {
            ...
            return yield* Effect.scoped(
              Effect.gen(function* () {
                yield* Flock.effect("ripgrep-install", {
                  dir: path.join(Global.Path.bin, "locks"),
                  staleMs: 5 * 60_000,
                  timeoutMs: 5 * 60_000,
                })

                // Another process may have completed the installation while
                // this process waited for the cross-process lock.
                if (yield* fs.isFile(target).pipe(Effect.orDie)) return target
