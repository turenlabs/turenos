...
function renderFleet(invocation: Swarm.Ready) {
  ...
  return [
    ...
    `3. Tell each orchestrator to: claim its lane; split its slice into at most ${share} bounded, non-overlapping worker assignments; dispatch them with one spawn_agents call under one wave; wait on that wave; then return one evidence-backed synthesis of its slice that names every failed or incomplete worker. Its workers finish and report without parking — parked workers would hold slots their queued siblings need. Orchestrators post slice status to the room; workers post only material findings.`,
