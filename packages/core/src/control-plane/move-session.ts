...
export class Service extends Context.Service<Service, Interface>()("@forge/ControlPlaneMoveSession") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const moveSession = Effect.fn("MoveSession.moveSession")(function* (input: Input) {
      yield* tasks.authorizeRelocation(input.sessionID)
      ...
      // The destination is the payload; the envelope is the routing key, and it
      // has to be the placement the Session is leaving. This service is global
      // and its HTTP route carries no Location middleware, so without an
      // explicit envelope the frame publishes unlocated and no per-instance
      // stream delivers it — least of all the source instance, which is the one
      // still listing a Session that is no longer there.
      // The move only ever changes `directory` (Destination carries no
      // workspace of its own, and nothing here can resolve a directory back
      // to a workspace), so the session's workspace association is carried
      // forward unchanged. Dropping it here previously nulled it out on
      // every move — see move-session.test.ts.
...
export const node = makeGlobalNode({
  service: Service,
  layer,
  deps: [Git.node, EventV2.node, ProjectV2.node, SessionStore.node, SessionTaskV2.node],
})
