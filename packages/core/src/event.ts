...
import { Durable } from "@turenlabs/schema/durable-event-manifest"
import { isWithReplicas } from "@turenlabs/effect-drizzle-sqlite"

export const ID = Event.ID
export type ID = import("@turenlabs/schema/event").ID
export type { Data, Definition, Payload } from "@turenlabs/schema/event"
...
export interface Interface {
  ...
  /** Reads the currently committed durable aggregate events without subscribing to future wakes. */
  readonly durableSnapshot: (input: {
    readonly aggregateID: string
    readonly after?: number
...
export class Service extends Context.Service<Service, Interface>()("@forge/Event") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return Service.of({
        ...
        all: streamAll,
        durable,
        durableSnapshot: (input) => readAfter(input.aggregateID, input.after ?? -1),
        listen,
        project,
        replay,
        replayAll,
