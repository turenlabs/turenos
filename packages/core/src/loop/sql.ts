...
export const LoopTable = sqliteTable(
  "loop",
  {
    ...
    schedule_type: text({ enum: ["interval", "cron"] }).notNull(),
    interval_seconds: integer().notNull(),
    timezone: text().notNull(),
    cron_expression: text(),
    trigger_type: text({ enum: ["scheduled", "file-change", "session-end"] }),
    trigger_config: text({ mode: "json" }).$type<Loop.EventTriggerConfig>(),
...
export const LoopRunTable = sqliteTable(
  "loop_run",
  {
    ...
    trigger: text({ enum: ["scheduled", "manual", "file-change", "session-end"] }).notNull(),
    trigger_payload: text({ mode: "json" }).$type<Record<string, unknown>>(),
