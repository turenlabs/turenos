...
import type { Plugin as PluginRuntime } from "@turenlabs/plugin/v2/effect"
...
export class Service extends Context.Service<Service, Interface>()("@forge/v2/Plugin") {}
          ...
          Effect.andThen(
            State.batch(
              ...
              Effect.flatMap((child) =>
                Effect.gen(function* () {
                  // State.batch commits registered transforms after its inner effect
                  // returns. Publish readiness only after that commit, otherwise a
                  // waiter can snapshot the pre-plugin agent set on the first turn.
                  active.set(id, child)
...
export const locationLayer = layer.pipe(
  ...
  Layer.provideMerge(ToolInterceptor.locationLayer),
)
