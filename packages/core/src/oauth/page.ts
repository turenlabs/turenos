...
// etc.).
//
// The visual language mirrors the TurenOS app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `TurenOS is now connected to ${escapeHtml(provider)}.` : "TurenOS is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `TurenOS couldn't finish connecting to ${escapeHtml(provider)}.`
        : "TurenOS couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from TurenOS.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · TurenOS</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("TurenOS couldn't finish connecting to "+PROVIDER+"."):"TurenOS couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from TurenOS."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("TurenOS is now connected to "+PROVIDER+"."):"TurenOS is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
