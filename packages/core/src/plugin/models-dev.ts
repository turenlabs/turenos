...
function anthropicOmitsThinking(modelID: string) {
  ...
  if (/sonnet-(?:[5-9]|\d{2,})(?:[.@-]|$)/.test(id)) return true
  if (/opus-(?:[5-9]|\d{2,})(?:[.@-]|$)/.test(id)) return true
  return /opus-4[.-](?:[7-9]|\d{2,})(?:[.@-]|$)/.test(id)
}
...
function reasoningVariants(model: ModelsDev.Model, packageName: string | undefined): ModelV2Info["variants"] {
  const efforts =
    model.reasoning_options
      ...
      ?.values.flatMap((value) => (typeof value === "string" ? [value] : value === null ? ["none"] : [])) ?? []
  if (packageName === "@ai-sdk/openai")
    return efforts.map((effort) => ({ id: effort, headers: {}, body: { reasoningEffort: effort } }))
  if (packageName === "@ai-sdk/openai-compatible")
    return efforts.map((effort) => ({ id: effort, headers: {}, body: { reasoning_effort: effort } }))
  if (packageName !== "@ai-sdk/anthropic") return []
  if (efforts.length > 0)
    return efforts.map((effort) => ({
      id: effort,
      headers: {},
      body: {
        ...(["claude-opus-4-5", "claude-opus-4.5"].some((id) => model.id.includes(id))
          ? {}
          : {
              thinking: {
                type: "adaptive",
                ...(anthropicOmitsThinking(model.id) ? { display: "summarized" } : {}),
              },
            }),
        effort,
      },
    }))
  if (!model.reasoning_options?.some((option) => option.type === "budget_tokens")) return []
  return [
    {
      id: "high",
      headers: {},
      body: { thinking: { type: "enabled", budgetTokens: Math.min(16_000, Math.floor(model.limit.output / 2 - 1)) } },
    },
    {
      id: "max",
      headers: {},
      body: { thinking: { type: "enabled", budgetTokens: Math.min(31_999, model.limit.output - 1) } },
    },
  ]
}
...
function applyModel(
  ...
  draft.capabilities = {
    ...
    ...(typeof model.interleaved === "object" ? { interleaved: { field: model.interleaved.field } } : {}),
  }
