...
import type { PluginContext as Interface } from "@turenlabs/plugin/v2/effect"
...
export const make = Effect.fn("PluginHost.make")(function* (plugin: PluginV2.Interface) {
  ...
  const toolInterceptors = yield* ToolInterceptor.Service
  ...
  return {
    ...
    // Pass-through, unlike `aisdk` above: the interceptor event is already the plugin-facing
    // shape, with no branded or `readonly`-deep core value to widen. `satisfies Interface` is
    // what keeps the two definitions from drifting apart.
    tool: {
      execute: {
        before: toolInterceptors.hook.before,
        after: toolInterceptors.hook.after,
      },
    },
  } satisfies Interface
})
