...
import type { Plugin, PluginContext, Registration } from "@turenlabs/plugin/v2/promise"
      ...
      Effect.gen(function* () {
        ...
        const context2: PluginContext = {
          ...
          tool: {
            execute: {
              before: (callback) =>
                register(host.tool.execute.before((event) => Effect.promise(() => Promise.resolve(callback(event))))),
              after: (callback) =>
                register(host.tool.execute.after((event) => Effect.promise(() => Promise.resolve(callback(event))))),
            },
          },
        }
