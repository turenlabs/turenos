...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServerV1.Server).annotate({
    description: "Server configuration for TurenOS serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommandV1.Info)).annotate({
    description: "Command configuration, see https://github.com/turenlabs/forge/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://github.com/turenlabs/forge/docs/agents" }),
  ...
  retention: Schema.optional(
    Schema.Struct({
      archivedSessionDays: Schema.optional(NonNegativeInt).annotate({
        description:
          "Days after a session is archived before its tool payloads and shell output are reduced to previews. Titles, user prompts, and assistant text are always kept (default: 30, 0 disables)",
      }),
      toolOutputDays: Schema.optional(NonNegativeInt).annotate({
        description:
          "Days a stored tool payload is kept in full, in any session, before it is reduced to its preview (default: 14, 0 disables)",
      }),
    }),
  ).annotate({
    description:
      "Disk retention policy. Applies only to stored transcript payloads; the durable event log is never rewritten.",
  }),
  ...
  semantic_memory: Schema.optional(ConfigSemanticMemory.Info).annotate({
  ...
  experimental: Schema.optional(
    Schema.Struct({
      harness_self_modification: Schema.optional(Schema.Boolean).annotate({
