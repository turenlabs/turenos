...
const valueDiffs = (expected: unknown, received: unknown, base = "$", limit = 8): ReadonlyArray<string> => {
  if (Object.is(expected, received) || limit <= 0) return []
  if (isRecord(expected) && isRecord(received)) {
    const diffs: string[] = []
    const keys = [...new Set([...Object.keys(expected), ...Object.keys(received)])].toSorted()
    for (const key of keys) {
      diffs.push(...valueDiffs(expected[key], received[key], `${base}.${key}`, limit - diffs.length))
  ...
  if (Array.isArray(expected) && Array.isArray(received)) {
    ...
    for (let index = 0; index < length && diffs.length < limit; index++) {
      diffs.push(...valueDiffs(expected[index], received[index], `${base}[${index}]`, limit - diffs.length))
    }
    return diffs
  }
