...
import { type UiI18n, useI18n } from "@turenlabs/ui/context/i18n"
...
import { Checkbox } from "@turenlabs/ui/checkbox"
import { DiffChanges } from "@turenlabs/ui/diff-changes"
import { Markdown } from "./markdown"
import { ImagePreview } from "@turenlabs/ui/image-preview"
import { getDirectory as _getDirectory, getFilename } from "@turenlabs/core/util/path"
...
export function UserMessageDisplay(props: {
  ...
  // Colour for this turn's left rule \u2014 see the [data-slot="user-message-text"]
  // block in message-part.css. Same resolver the rest of the file uses, so a
  // custom agent colour wins over the built-in palette here too.
  const ruleTone = createMemo(() => taskAgent(props.message.agent, data.store.agent).v2Color)
  ...
  return (
    <div
      ...
      style={ruleTone() ? { "--rule-tone": ruleTone()! } : undefined}
    >
...
/*
 * Tools that render a body of their own — shell output, a diff, a todo list, a
 * set of answers. The TUI drew these as `BlockTool`: a filled block with a rule
 * down its left edge toned to the page background, so the block read as a notch
 * cut into the transcript rather than as a card floating on it. Everything else
 * was a single flush line with no rule at all, and that contrast is what kept a
 * run of Read/Grep/Glob tight while a shell block still announced itself.
 *
...
PART_MAPPING["tool"] = function ToolPartDisplay(props) {
  ...
  // A failed tool is already ruled — Card draws it as a ::before in the error
  // tone (card.css). It opts out of the notch so a block never carries two marks.
  const failed = createMemo(() => part().state.status === "error" && !!(part().state as any).error)
  const rule = createMemo(() => (!failed() && BLOCK_TOOLS.has(part().tool) ? "notch" : undefined))
  // The mark hangs in the margin rather than sitting on the box edge, so a tool
  // body is exactly as wide ruled as unruled and lands in the one rule column
  // the user turn and the failure rule use. See rule.css.
  const ruleHang = createMemo(() => (rule() ? "" : undefined))

  // Compaction's pruner cleared this result out of the model's context; the transcript still
  // holds it. The body is left intact — it is the user's own history and it is not wrong, only
  // no longer shared with the model — and a note says so, so the two views cannot diverge
  // silently. See tool-cleared.ts.
  const cleared = createMemo(() => toolResultCleared(part()))
  ...
  return (
    <Show when={!hideQuestion()}>
      <div
        ...
        data-rule={rule()}
        data-rule-hang={ruleHang()}
        data-tool-failed={failed() ? "" : undefined}
        data-tool-cleared={cleared() ? "" : undefined}
      ...
      >
        <div data-component="task-tool-surface">
          <div data-slot="basic-tool-tool-info-structured">
            <div data-slot="basic-tool-tool-info-main">
              <Show
                when={running() && !props.hideTaskThinking}
                fallback={
                  <Show when={newLayout()}>
              ...
              >
                <span data-component="task-tool-thinking" role="status" aria-live="polite">
                  <Thinking state={taskThinkingState(props.input.subagent_type)} size={20} aria-hidden="true" />
                  <span class="sr-only">{i18n.t("ui.sessionTurn.status.delegating")}</span>
                </span>
              </Show>
