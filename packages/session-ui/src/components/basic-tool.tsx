              ...
              {(title) => (
                <div data-slot="basic-tool-tool-info-structured">
                  <div data-slot="basic-tool-tool-info-main">
                    ...
                    {/*
                      A tool's arguments settle at `tool.called` — the moment the row goes
                      `running` — so a running row already holds its complete input. Hiding
                      the subtitle and the argument chips until `completed` made a running
                      call render as a bare `Called `X``, indistinguishable from a call that
                      took no arguments at all. Only a `pending` row genuinely has nothing:
                      its input is still streaming and the projection carries `{}` until the
                      call lands, and its shimmering title is what says so.
                    */}
...
export function GenericTool(props: {
  ...
  const args = createMemo(() => {
    const preview = toolArgs(props.input, label()?.key)
    if (preview.omitted === 0) return preview.args
    // An argument the row cannot preview, or cannot fit, is named rather than
    // dropped. A row holding something back must not read as a call that took
    // nothing. See basic-tool-args.ts.
    if (preview.args.length === 0) return [i18n.t("ui.basicTool.argsUnavailable")]
    return [...preview.args, i18n.t("ui.basicTool.argsMore", { count: preview.omitted })]
  })
