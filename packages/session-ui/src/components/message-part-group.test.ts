...
import { groupParts, sameGroups } from "./message-part-group"
...
test("failed reads and coordination are individual error rows between routine groups", () => {
  for (const name of ["read", "reflection_state", "board_post"]) {
...
test("questions, writes, shell calls, and unknown tools are never quieted by a name substring", () => {
  const parts = ["question", "write", "bash", "custom_reflection_state", "board_post_external"].map((name) =>
    tool(name, name),
  )
