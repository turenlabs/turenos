...
export interface ToastV2Options {
  ...
  description?: string
  /**
   * Prefer a thunk. `showToastV2` is called from plain event handlers, so an eagerly-built element
   * would create computations outside any root and Solid would never dispose them. `children()`
   * below resolves a zero-arg function inside the toast's own root.
   */
...
export function showToastV2(options: ToastV2Options | string) {
  ...
  return toaster.show((props) => {
    // This project's JSX.Element maps to the DOM Element, so it cannot express Solid's
    // function-element form; `children()` resolves a zero-arg thunk at runtime regardless.
    const resolvedIcon = children(() => opts.icon as JSX.Element)
    return (
