...
describe("SerializeAddon", () => {
  ...
  describe("terminal modes", () => {
    test("should restore mouse tracking and SGR encoding across a snapshot cycle", async () => {
      const { term, addon } = createTerminal()
      await writeAndWait(term, "\x1b[?1049h\x1b[?1000h\x1b[?1006h")
      ...
      expect(term2.hasMouseTracking()).toBe(true)
      expect(term2.getMode(1000)).toBe(true)
      expect(term2.getMode(1006)).toBe(true)
    })

    test("should restore bracketed paste and application cursor keys", async () => {
      const { term, addon } = createTerminal()
      await writeAndWait(term, "\x1b[?1h\x1b[?2004h")

      const { term: term2 } = createTerminal()
      await writeAndWait(term2, addon.serialize())

      expect(term2.getMode(1)).toBe(true)
      expect(term2.getMode(2004)).toBe(true)
    })

    test("should not emit mode sequences for a terminal in its default state", async () => {
      const { term, addon } = createTerminal()
      await writeAndWait(term, "plain shell output")
    ...
    test("should omit modes when excludeModes is set", async () => {
      const { term, addon } = createTerminal()
      await writeAndWait(term, "\x1b[?1000h\x1b[?1006h")

      const serialized = addon.serialize({ excludeModes: true })

      expect(serialized).not.toContain("\x1b[?1000h")
      expect(serialized).not.toContain("\x1b[?1006h")
    })
  })
})
