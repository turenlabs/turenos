...
type ModeQuery = (mode: number, isAnsi?: boolean) => boolean

// ghostty-web's Terminal exposes getMode(mode, isAnsi) but the ITerminalCore
// addon interface does not declare it, so duck-type it like `buffer` above.
const getModeQuery = (value: ITerminalCore): ModeQuery | undefined => {
  if (!isRecord(value)) return
  const candidate = value.getMode
  if (typeof candidate !== "function") return
  return (mode, isAnsi) => {
    try {
      return candidate.call(value, mode, isAnsi) === true
    } catch {
      return false
    }
  }
...
// DEC private modes that are reset by default; each currently-set mode is
// restored with a DECSET. The load-bearing ones are mouse tracking
// (9/1000/1002/1003) and its encodings (1005/1006/1015/1016): without them a
// restored terminal running a full-screen terminal app falls back to translating wheel
// input into arrow keys, which terminal applications interpret as
// prompt-history navigation instead of scrolling. The rest mirror the xterm.js
// addon-serialize mode list: application cursor keys (1), reverse wraparound
// (45), application keypad (66), focus reporting (1004), and bracketed paste
// (2004). Origin mode (6) is deliberately not restored because DECSET 6 homes
// the cursor after the snapshot has already positioned it.
const DEC_MODES_DEFAULT_RESET = [1, 9, 45, 66, 1000, 1002, 1003, 1004, 1005, 1006, 1015, 1016, 2004]

// DEC private modes that are set by default; a cleared mode is restored with a
// DECRST (autowrap 7, cursor visibility 25).
const DEC_MODES_DEFAULT_SET = [7, 25]

const serializeModes = (getMode: ModeQuery): string => {
  let content = ""
  for (const mode of DEC_MODES_DEFAULT_RESET) {
    if (getMode(mode, false)) content += `\u001b[?${mode}h`
  }
  for (const mode of DEC_MODES_DEFAULT_SET) {
    if (!getMode(mode, false)) content += `\u001b[?${mode}l`
  }
  return content
}
...
export class SerializeAddon implements ITerminalAddon {
  ...
  public serialize(options?: ISerializeOptions): string {
    ...
    if (!options?.excludeModes) {
      const getMode = getModeQuery(this._terminal)
      if (getMode) content += serializeModes(getMode)
    }
