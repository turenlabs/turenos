import type { FilePart, Project, UserMessage, VcsFileDiff } from "@turenlabs/sdk/v2"
...
import { isScrollKeyTarget, scrollKey, scrollKeyOwner } from "@turenlabs/ui/scroll-view"
...
import { SessionReviewEmptyChangesV2 } from "@turenlabs/session-ui/v2/session-review-empty-changes-v2"
import { SessionReviewEmptyNoGitV2 } from "@turenlabs/session-ui/v2/session-review-empty-no-git-v2"
import { SessionReviewV2SidebarToggle } from "@turenlabs/session-ui/v2/session-review-v2"
...
import {
  createSessionV2TimelineController,
  sessionTurnActivity,
  sessionTurnStatus,
} from "./session/goal/session-v2-timeline-controller"
import { createSessionSubagentController } from "./session/subagent/session-subagent-controller"
import { sessionSwarmProgress, sessionSwarmRequest, sessionTaskActive } from "./session/subagent/session-subagent"
import { SessionSubagentDock } from "./session/subagent/session-subagent-dock"
import {
  SessionLiveDock,
  SessionLivePending,
  SessionLivePrototype,
  type SessionLiveToolCall,
...
function SessionErrorFallback(props: { error: unknown; sessionID?: string; serverKey?: ServerConnection.Key }) {
  ...
  // Self-heal the home session index when the route classifies the session as gone
  // server-side (hard delete → session.get 404 / empty lineage). A missed
  // session.deleted event can otherwise leave a permanently stale left-nav row for a
  // session that only ever renders "not found". Emit the synthetic removal the index
  // already knows how to apply, targeting this route's server (the ServerSyncProvider
  // above resolves to serverKey). Gated by the same not-found classification as the
  // fallback UI below, so archived sessions — which resolve and render normally, never
  // reaching this fallback — cannot misfire it. Fires only on the not-found
  // classification, never for transient network errors (which render ErrorPage).
  createEffect(() => {
    const sessionID = props.sessionID
    if (!sessionID) return
    if (!isCurrentSessionNotFoundError(props.error, sessionID)) return
    serverSync().homeSessions.apply(removedHomeSessionEvent(sessionID))
  })
  ...
  const closeTab = () => {
    ...
    const open = tabs.store.some(
      (tab) => tab.type === "session" && tab.server === serverKey && tab.sessionId === sessionID,
    )
    tabs.removeSessionTab({ server: serverKey, sessionId: sessionID })
    if (!open) navigate("/")
  }
          ...
          <Show when={props.sessionID}>
            {(sessionID) => (
              <div class="max-w-full flex flex-col items-center gap-1">
                ...
                <code class="max-w-full rounded-control px-1 py-0.5 font-mono text-xs font-medium leading-4 text-text-base break-all bg-[color-mix(in_oklch,var(--v2-text-text-base)_8%,transparent)]">
                  {sessionID()}
                </code>
              </div>
            )}
...
function ResolvedTargetSessionRoute() {
  ...
  const [activation, setActivation] = createSignal(layout.activation.current())
  const serverKey = createMemo(() => requireServerKey(params.serverKey))
  const current = createSessionLineage(
    () => params.id,
    () => sync().session.lineage,
  )
  const cachedTitle = createMemo(
    () => tabs.info[tabKey({ type: "session", server: serverKey(), sessionId: params.id })]?.title,
  )
  const directory = createMemo(() => current()?.session.directory)
  const targetDirectory = () => directory()!
  const directoryState = createMemo(() => {
    const value = directory()
    if (!value) return
    return sync().directoryState(value)
  })
  const loading = createMemo(() => {
    const state = directoryState()
    return !directory() || state?.status === "loading" || state?.bootstrapping === true
  })
  const loadingPhase = () => (!directory() ? "Resolving session" : "Loading workspace data")
  const targetActivation = createMemo(() => {
    const value = activation()
    if (!value) return
    if (value.scope !== serverSDK().scope) return
    if (value.sessionID !== params.id) return
    return value
  })
  ...
  const sessionShell = () => (
    <SessionPanelFrame newLayout raised>
      ...
      <SessionLivePending view={tabs.liveView} onViewChange={tabs.setLiveView} />
    </SessionPanelFrame>
  )
  ...
  createEffect(() => {
    ...
    const next = nextSessionActivation({
      current,
      pending: layout.activation.current(),
      sessionID: id,
      scope: serverSDK().scope,
    })
    if (current !== next) setActivation(next)
  })

  createEffect(() => {
    const current = activation()
    if (!current) return
    const remaining = SESSION_ACTIVATION_MAX_AGE_MS - (Date.now() - current.startedAt)
    if (remaining <= 0) {
      setActivation(undefined)
      return
    }
    const timer = window.setTimeout(() => setActivation(undefined), remaining)
  ...
  createEffect(() => {
    ...
    if (!session) return
    const tab = { server: serverKey(), sessionId: session.root.id }
    // Closing a tab must not retrigger this route effect through addSessionTab's store read.
  ...
  return (
    ...
    <div data-session-destination class="contents">
      <Show when={loading()}>
        <div
          data-session-destination-loading
          class="pointer-events-none fixed right-4 top-12 z-40 rounded-md border border-v2-border-border-base bg-v2-background-bg-layer-01 px-2.5 py-1 shadow-sm"
        >
          <DestinationLoading detail="phase" label="Loading session" phase={loadingPhase()} />
        </div>
      </Show>
      <Show when={workspaceIdentity()} keyed fallback={sessionShell()}>
        {(_identity) => session()}
      </Show>
    </div>
  )
...
export default function Page() {
  ...
  const liveView = tabContext.liveView
  createEffect(() => tabContext.setLiveView(liveView()))
  ...
  const subagents = createSessionSubagentController({
    sessionID: () => params.id,
  })
  const harness = createSessionHarnessController({
    sessionID: () => params.id,
  })
  ...
  const [ui, setUi] = createStore({
    ...
    pendingPromptBaseMessageID: undefined as string | undefined,
  ...
  onMount(() => {
    ...
    update()
    onCleanup(makeEventListener(document, "visibilitychange", update))
  })
  ...
  createEffect(
    on(
      ...
      (id, prev) => {
        ...
        const pending = layout.activation.current()
        if (!pending) return
        if (Date.now() - pending.startedAt > SESSION_ACTIVATION_MAX_AGE_MS) {
          layout.activation.clear(pending.sessionID)
          return
        }
  ...
  const sessionTools = createQuery(() => {
    const selected = local.model.current()
    const saved = info()?.model
    const provider = selected?.provider.id ?? saved?.providerID
    const model = selected?.id ?? saved?.id
    const directory = info()?.directory
    return {
      queryKey: ["session-tools", sdk().url, params.id, directory, provider, model],
      enabled: Boolean(settings.general.newLayoutDesigns() && params.id && directory && provider && model),
      queryFn: async () => {
        if (!directory || !provider || !model) return undefined
      ...
      refetchInterval: liveView() === "activity" && documentVisible() ? 5_000 : false,
    }
  })
  ...
  const v2Timeline = createSessionV2TimelineController({
    sessionID: () => params.id,
    sessionKey,
    agent: () => local.agent.current()?.name ?? info()?.agent ?? "build",
    model: () => {
      const selected = local.model.current()
      if (selected)
        return {
  ...
  const timeline = createTimelineModel({
    sessionID: () => params.id,
    revertMessageID,
    hydrate: v2Timeline.hydrate,
    loadOlder: v2Timeline.loadOlder,
    hasOlder: v2Timeline.hasOlder,
  })
  // The transcript's live status line: durable-stream activity folded by the timeline
  // controller, joined with the two renderer-only signals it cannot see — the question
  // dock derivation and the Subagents panel's active count.
  const turnStatus = createMemo(() =>
    sessionTurnStatus({
      questionPending: !!composer.questionRequest(),
      agentsActive: subagents.taskIDs().reduce((count, id) => {
        const task = subagents.task(id)
        return task && sessionTaskActive(task) ? count + 1 : count
      }, 0),
      activity: sessionTurnActivity.get(params.id ?? ""),
    }),
  )
  ...
  createEffect(() => {
    const pending = ui.pendingPrompt
    if (!pending) return
    const message = lastUserMessage()
    if (!message || message.id === ui.pendingPromptBaseMessageID) return
    setUi({ pendingPrompt: undefined, pendingPromptBaseMessageID: undefined })
  })
  ...
  createEffect(() => {
    ...
    if (restoredModelSession !== id) {
      ...
      // Prompt state must win before the last-message fallback can materialize session state.
  ...
  const liveChanges = createMemo(() => newSessionDesign() && liveView() === "changes")
    ...
    on(
      ...
      () => {
        ...
        setUi({
          ...
          pendingPromptBaseMessageID: undefined,
  ...
  useComposerCommands({ goal: { toggle: goal.toggleMode } })
  useSurfaceCommands({ focusInput })
  useSessionCommands({
  ...
  useCommandPalette(() => command.trigger("file.open", "palette"))
  ...
  const reviewPanelV2Props = () => ({
    ...
    onFocusedCommentChange: (focus: { file: string; id: string } | null) => {
      ...
      if (!focus) {
        ...
        if (current && reviewDiffs().some((diff) => diff.file === current.file)) selectReviewDiff(current.file)
      }
  ...
  const selectReviewDiff = (path: string) => {
  ...
  const historyVisible = () => sessionTranscriptVisible(settings.general.newLayoutDesigns(), liveView())
  const setLiveView = (next: SessionLiveView) => {
    if (next === "history" && timeline.error()) timeline.refetch()
    view().liveView.set(next)
    tabContext.setLiveView(next)
  }
  const setLiveDockView = (view: SessionLiveView) => {
    setLiveView(view)
    if (view === "terminal" && params.id) void terminal.shared(params.id, { focus: true })
  }
  const loadOlder = async () => {
    ...
    // Pin the topmost visible row before the prepend and re-pin it after.
    //
    // Widening the window inserts rows *above* the viewport, and a virtualised list measures
    // lazily: without this the newly inserted height lands under the scroll offset and the
    // transcript jumps away from whatever the user was reading. The anchor machinery in
    // `message-timeline.tsx` was written for exactly this and has been dormant only because
    // V2 had no older page to load.
    captureHistoryAnchor()
    try {
      await timeline.history.loadOlder()
    } finally {
      historyRequests.delete(owner.key)
      restoreHistoryAnchor(true)
    }
  ...
  createEffect(
    on(
      ...
      ([id, ready, more, loading, scrolled, visible]) => {
        if (!visible || !id || !ready || loading || scrolled) return
        if (!more) return
        fill()
      },
      { defer: true },
  ...
  const roll = (sessionID: string, next: NonNullable<ReturnType<typeof info>>["revert"], target = sync()) => {
    ...
    if (!session) return
    target.session.remember(applySessionV2Revert(session, next))
  }
  ...
  const revertMutation = useMutation(() => ({
    mutationFn: async (input: { sessionID: string; messageID: string }) => {
      ...
      await runPromptRollbackMutation({
        ...
        request: () => stageSessionV2Revert(client, input),
        complete: (revert) => roll(input.sessionID, revert, target),
  ...
  const restoreMutation = useMutation(() => ({
    mutationFn: async (id: string) => {
      ...
      await runPromptRollbackMutation({
        ...
        request: () =>
          next
            ? stageSessionV2Revert(client, { sessionID, messageID: next.id })
            : clearSessionV2Revert(client, sessionID).then(() => undefined),
        complete: (revert) => roll(sessionID, revert, target),
  ...
  const liveMessages = createMemo(() => {
    const id = revertMessageID()
    if (!id) return messages()
    return messages().filter((message) => message.id < id)
  })
  const liveChangedFiles = createMemo(() => [
    ...new Set(
      liveMessages().flatMap((message) =>
        (sync().data.part[message.id] ?? []).flatMap((part) => {
          ...
          if (typeof path === "string" && ["edit", "write"].includes(part.tool)) return [path]
          if (part.tool !== "apply_patch" || typeof part.state.input.patchText !== "string") return []
          return [...part.state.input.patchText.matchAll(/^\*\*\* (?:Add|Update|Delete) File: (.+)$/gm)].map(
            (match) => match[1],
          )
        }),
      ),
  ...
  const liveToolCalls = createMemo<SessionLiveToolCall[]>((previous = []) => {
      ...
      .flatMap((message) =>
        (sync().data.part[message.id] ?? []).flatMap((part) =>
          ...
          (part.tool !== "question" || (part.state.status !== "pending" && part.state.status !== "running"))
            ? [{ id: `${message.id}:${part.id}`, message, part }]
            : [],
        ),
      )
      .map((item) => {
        const existing = prior.get(item.id)
        if (existing?.message === item.message && existing.part === item.part) return existing
        return item
      })
      .reverse()
  }, [])
  const liveToolCounts = createMemo(() => {
    const tools = liveMessages().flatMap((message) =>
      (sync().data.part[message.id] ?? []).flatMap((part) => (part.type === "tool" ? [part] : [])),
    )
    return {
      shell: tools.filter((part) => part.tool === "bash").length,
      total: tools.length,
    }
  })
  const liveTasks = createMemo(() =>
    subagents.taskIDs().flatMap((id) => {
      const task = subagents.task(id)
      return task ? [task] : []
    }),
  )
  const liveAgents = () => {
    ...
    return {
      active: tasks.filter(sessionTaskActive).length,
      completed: tasks.filter((task) => task.status === "completed").length,
      failed: tasks.filter((task) => task.status === "failed").length,
      total: tasks.length,
    }
  }
  const liveSwarmRequest = createMemo(() => {
    return sessionSwarmRequest(
      liveMessages().flatMap((message) =>
        message.role === "user" ? [{ text: line(message.id), time: message.time.created }] : [],
      ),
      ui.pendingPrompt,
    )
  })
  const liveSwarm = createMemo(() => {
  ...
  const liveTimeline = createMemo<SessionTimelineItem[]>(() =>
    liveMessages()
      .flatMap((message): SessionTimelineItem[] => {
        const time = message.time.created
        const tools: SessionTimelineItem[] = (sync().data.part[message.id] ?? []).flatMap((part) => {
          if (part.type !== "tool") return []
          const toolTime = "time" in part.state && part.state.time?.start ? part.state.time.start : time
          return [{ id: `${message.id}:${part.id}`, time: toolTime, kind: "tool" as const, label: part.tool }]
        })
        if (message.role === "user")
          return [{ id: message.id, time, kind: "prompt" as const, label: line(message.id) }, ...tools]
  ...
  const { clearMessageHash, scrollToMessage } = useSessionHashScroll({
    ...
    historyVisible,
    visibleUserMessages,
    historyVersion: timeline.history.version,
    historyMore,
    historyLoading,
    // A hash target older than the loaded window is not missing, only unloaded. This effect
    // re-runs as the window widens, so it walks back a page at a time until the message appears
    // or history runs out — which is what keeps jump-to-message working now that hydration takes
    // a window instead of the whole transcript.
    loadMore: async (_sessionID, intent) => {
      await timeline.history.loadOlder(intent)
    },
  ...
  // Background work owns a tab in the new layout, so the composer stops stacking it
  // above the prompt; the legacy layout keeps the dock.
  const subagentDock = () => (
    <SessionSubagentDock
      controller={subagents}
      swarm={liveSwarm}
      variant={settings.general.newLayoutDesigns() ? "panel" : "dock"}
      onOpenSession={openSubagentSession}
      onOpenParent={openSubagentSession}
      onRetryFromParent={(task) => {
        if (params.id === task.parentSessionID) {
          inputRef?.focus()
          return
        }
        openSubagentSession(task.parentSessionID)
      }}
    />
  )

  const sessionGoalControls = () => {
    ...
    return {
      ...
      editRequest: goal.editRequest(),
      onEdit: (value: { objective: string }) => {
        void goal.edit({ sessionID, goal: current, ...value }).catch(failed)
      },
      onPause: () => {
        void goal.pause(sessionID).catch(failed)
      },
      onResume: () => {
        void goal.resume(sessionID).catch(failed)
      },
      onClear: () => {
        void goal.clear(sessionID).catch(failed)
      },
    }
  }
        ...
        promptInput={
          <PromptInput
            ...
            onPendingPrompt={(value) => {
              ...
              setUi({
                pendingPrompt: promptLine(value),
                pendingPromptBaseMessageID: lastUserMessage()?.id,
              })
            }}
    ...
    <>
      <div class="flex flex-1 min-h-0 min-w-0 flex-col overflow-hidden">
        <Switch>
          ...
          <Match when={params.id && settings.general.newLayoutDesigns()}>
            <SessionLivePrototype
              view={liveView}
              ...
              todosReady={() => !params.id || serverSync().session.data.todo[params.id] !== undefined}
              timeline={liveTimeline}
              toolCounts={liveToolCounts}
              toolCalls={liveToolCalls}
              availableTools={() => sessionTools.data?.visible ?? []}
              mcpServers={() => sessionTools.data?.mcpServers ?? []}
              mcpCapabilities={() => sessionTools.data?.broker.capabilities ?? []}
              toolExclusions={() => sessionTools.data?.exclusions ?? []}
              ...
              harness={() => (
                <SessionHarnessPanel
                  snapshot={() => {
                    const snapshot = harness.state(params.id)?.snapshot
                    return snapshot
                      ? {
                          version: snapshot.version,
                          label: snapshot.source,
                          updatedAt: new Date(snapshot.timestamps.updated).toLocaleString(),
                        }
                      : undefined
                  }}
                  mode={() => "Session overlay"}
                  status={() => harness.state(params.id)?.snapshot?.status}
                  proposals={() =>
                    // Applied proposals are shown too: auto-apply means nothing ever sits in
                    // pending, so filtering them out left the section permanently empty.
                    (harness.state(params.id)?.proposals ?? [])
                      .slice()
                      .reverse()
                      .map((proposal) => ({
                        id: proposal.id,
                        ...
                        createdAt: new Date(proposal.timestamps.created).toLocaleString(),
                      }))
                  }
                  changes={() =>
                    (harness.state(params.id)?.snapshot?.changes ?? []).map((change) => ({
                      path: change.path,
                      summary: change.summary ?? `${change.operation} harness file`,
                      kind:
                  ...
                  tools={() =>
                    (harness.state(params.id)?.snapshot?.tools ?? []).map((tool) => ({
                      name: tool.name,
                      description: tool.description,
                      status: tool.enabled && tool.readOnly ? "Available" : "Disabled",
                    }))
                  }
                  validation={() => {
                    const validation = harness.state(params.id)?.snapshot?.validation
                      ...
                      summary:
                        validation.errors.length > 0
                          ...
                          : (validation.warnings[0] ??
                            (validation.status === "passed"
                              ? "Declarative harness snapshot is ready."
                              : validation.status === "failed"
                                ? "Harness validation failed."
                                : "Harness validation is pending.")),
                      details: [...validation.errors, ...validation.warnings],
                    }
                  }}
                  guidance={() =>
                    (harness.state(params.id)?.snapshot?.guidance ?? []).map((item) => ({
                      directive: item.directive,
                      appliesTo: item.appliesTo,
                    }))
                  }
                  reviewerRuns={() =>
                    (harness.state(params.id)?.reviewerRuns ?? [])
                      .slice()
                      .reverse()
                      .map((run, index) => ({
                        id: `${run.timestamp}-${index}`,
                        outcome: run.outcome,
                        label: reviewerRunLabel(run.outcome),
                        detail: run.detail,
                        reviewerSessionID: run.reviewerSessionID,
                        at: new Date(run.timestamp).toLocaleTimeString(),
                      }))
                  }
                  error={() => harness.error(params.id)}
                  canRollback={() => (harness.state(params.id)?.snapshot?.version ?? 0) > 1}
                  onApply={(proposal) => {
                    const sessionID = params.id
                    const current = harness.state(sessionID)?.proposals.find((item) => item.id === proposal.id)
                    if (sessionID && current) void harness.apply(sessionID, current)
                  }}
                  onApprove={(proposal) => {
                    const sessionID = params.id
                    const current = harness.state(sessionID)?.proposals.find((item) => item.id === proposal.id)
                    if (sessionID && current) void harness.approve(sessionID, current)
                  }}
                  onReject={(proposal) => {
                    const sessionID = params.id
                    const current = harness.state(sessionID)?.proposals.find((item) => item.id === proposal.id)
                    if (sessionID && current) void harness.reject(sessionID, current)
                  }}
                  onReload={() => {
                    const sessionID = params.id
                    const version = harness.state(sessionID)?.snapshot?.version
                    if (sessionID && version !== undefined) void harness.reload(sessionID, version)
                  }}
                  onRollback={() => {
                    const sessionID = params.id
                    const version = harness.state(sessionID)?.snapshot?.version
                    if (sessionID && version !== undefined) void harness.rollback(sessionID, version)
                  }}
                />
              )}
              subagents={() => (
                  ...
                  fallback={
                    <div class="flex min-h-full items-center justify-center rounded-control border border-border-weak-base bg-background-base px-6 py-12 text-center">
                      <p class="text-13-regular text-text-weak">
                        Background agents spawned by this session appear here.
                      </p>
                    </div>
                  }
                >
              ...
              historyRetry={() => void timeline.refetch()}
            />
          </Match>
          <Match when={params.id}>{timelineContent()}</Match>
          <Match when={true}>
  ...
  return (
    <SessionRouteFrame>
      ...
      >
        ...
        <div
          classList={{
            "@container relative min-w-0 shrink-0 flex flex-col min-h-0 h-full flex-1 md:flex-none transition-[width]": true,
...
const REVIEWER_RUN_LABELS: Record<string, string> = {
  applied: "Improvement applied",
  proposed: "Proposal awaiting approval",
  duplicate: "Repeat of an existing proposal",
  no_output: "Reviewed, nothing to change",
  unparseable: "Reviewer reply could not be read",
  unchanged: "Skipped, session unchanged",
  unsafe: "Held back as unsafe to auto-apply",
  failed: "Review failed",
  timeout: "Review timed out",
}

function reviewerRunLabel(outcome: string) {
  return REVIEWER_RUN_LABELS[outcome] ?? outcome
}
