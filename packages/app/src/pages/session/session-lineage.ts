...
export const SESSION_ACTIVATION_MAX_AGE_MS = 60_000
...
export function nextSessionActivation<T extends { startedAt: number; sessionID: string; scope: string }>(input: {
  current: T | undefined
  pending: T | undefined
  sessionID: string
  scope: string
  ...
  const current =
    input.current?.sessionID === input.sessionID && now - input.current.startedAt <= SESSION_ACTIVATION_MAX_AGE_MS
      ? input.current
      : undefined
  if (input.pending?.sessionID !== input.sessionID || input.pending.scope !== input.scope) return current
  if (now - input.pending.startedAt > SESSION_ACTIVATION_MAX_AGE_MS) return current
