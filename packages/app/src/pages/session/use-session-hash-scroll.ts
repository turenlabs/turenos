...
export const useSessionHashScroll = (input: {
  ...
  loadMore: (sessionID: string, intent?: "page" | "seek") => Promise<void>
  ...
  createEffect(() => {
    ...
    if (!input.sessionID() || !input.messagesReady() || !input.historyVisible()) return
    cancel()
  ...
  const attemptHistoryLoad = (force = false) => {
    const sessionID = input.sessionID()
    if (!sessionID || !input.messagesReady() || (!force && !input.historyVisible())) return
    ...
    const attempt = `${input.sessionKey()}\0${targetId}\0${input.historyVersion()}`
    if (failedHistoryLoad === attempt) return
    failedHistoryLoad = attempt

    // A missing history page just leaves the hash target unresolved; it must
    // not escape as an unhandled rejection.
    void input.loadMore(sessionID, "seek").catch(() => {})
  }
