...
import type { SnapshotFileDiff, VcsFileDiff } from "@turenlabs/sdk/v2"
import {
  SESSION_REVIEW_V2_SIDEBAR_WIDTH_MAX,
  SESSION_REVIEW_V2_SIDEBAR_WIDTH_MIN,
  SessionReviewV2,
  SessionReviewV2Sidebar,
} from "@turenlabs/session-ui/v2/session-review-v2"
import { SessionReviewFilePreviewV2 } from "@turenlabs/session-ui/v2/session-review-file-preview-v2"
import { DiffChanges } from "@turenlabs/ui/v2/diff-changes-v2"
import type {
...
} from "@turenlabs/session-ui/session-review"
...
export function ReviewPanelV2(props: ReviewPanelV2Props) {
  ...
  const render = createSessionPanelRenderHold(
    () => props.deferRender?.() ?? panelRenderDeferred(),
    () => ({
      title: props.title,
      empty: props.empty,
      diffs: props.diffs().filter(filterRenderableDiff),
    ...
    }),
    {
      ...
      diffStyle: "unified" as SessionReviewDiffStyle,
      lineCommentActions: undefined,
      comments: undefined,
      focusedComment: undefined,
    },
  )
  const diffs = () => render().diffs
  ...
  const activeDiff = createMemo(() => {
    ...
    const focus = render().focusedComment
    if (focus && diffs().some((diff) => diff.file === focus.file)) return focus.file
    const active = render().activeFile
  ...
  const detailSource = createMemo(() => {
    ...
    const load = render().loadDiff
    if (!diff || !load || !reviewDiffNeedsLoad(diff)) return
    return { diff, load, version: render().diffVersion }
  })
  ...
  const activeItem = createMemo(() => {
    ...
    if (loaded && loaded.source === source && loaded.version === render().diffVersion) return loaded.value
    return source
  })
            ...
            <Show when={activeItem()}>
              {(diff) => (
                <SessionReviewFilePreviewV2
                  ...
                  lineCommentActions={render().lineCommentActions}
                  comments={render().comments}
                  focusedComment={render().focusedComment}
