...
describe("reviewWantsLoad", () => {
  const desktopClosed = {
    desktop: true,
    liveChanges: false,
    desktopFileTreeOpen: false,
    desktopReviewOpen: false,
    activeTab: "timeline",
    mobileChanges: false,
  }

  test("enables the live Changes tab without opening the legacy review pane", () => {
    expect(reviewWantsLoad(desktopClosed)).toBe(false)
    expect(reviewWantsLoad({ ...desktopClosed, liveChanges: true })).toBe(true)
  })

  test("keeps legacy review and mobile changes loading", () => {
    expect(reviewWantsLoad({ ...desktopClosed, desktopReviewOpen: true, activeTab: "review" })).toBe(true)
    expect(reviewWantsLoad({ ...desktopClosed, mobileChanges: true })).toBe(true)
  })
})
