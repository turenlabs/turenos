...
export function reviewWantsLoad(input: {
  desktop: boolean
  liveChanges: boolean
  ...
  if (input.liveChanges || input.mobileChanges) return true
  return input.desktop && (input.desktopFileTreeOpen || (input.desktopReviewOpen && input.activeTab === "review"))
}
