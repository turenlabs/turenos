...
import { Session } from "@turenlabs/sdk/v2/client"
...
import { toaster } from "@turenlabs/ui/toast"
...
export default function LegacyLayout(props: ParentProps) {
  ...
  const prefetchSession = (session: Session, priority: "high" | "low" = "low") => {
    ...
    const cached = untrack(() => !serverSync().session.shouldPrefetch(session.id))
    if (cached) return
  ...
  async function archiveSession(session: Session) {
    ...
    // The home nav renders from the home session index, not the child store -
    // keep it in sync even if the server's session.updated event is missed.
    serverSync().homeSessions.apply(archivedHomeSessionEvent(session, archivedAt))
  ...
  async function renameProject(project: LocalProject, next: string) {
    ...
    try {
      await serverSync().project.update({ directory: project.worktree, projectID: project.id, name })
    } catch (err) {
      showToast({
        variant: "error",
      ...
      sessions
        ...
        .map((session) =>
          serverSDK()
            ...
            // Keep the home session index (which the home nav renders from) in
            // sync even if the server's session.updated event is missed.
            .then(() => {
              serverSync().homeSessions.apply(archivedHomeSessionEvent(session, archivedAt))
              notifySessionTabsRemoved({
                server: ServerConnection.key(serverSDK().server),
  ...
  return (
    <div class="relative bg-background-base flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
      ...
      <div class="flex-1 min-h-0 min-w-0 flex">
        ...
        {shouldShowDebugBar(settings.general.showPerformanceDiagnostics()) && <DebugBar />}
      </div>
