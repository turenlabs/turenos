...
describe("layout workspace helpers", () => {
  ...
  test("does not navigate to an internal lobby session", () => {
    const result = latestRootSession(
      [
        {
          path: { directory: "/root" },
          session: [
            session({
              id: "ses_lobby_hidden",
              directory: "/root",
              metadata: { "forge.internal": true },
              time: { created: 2, updated: 2, archived: undefined },
            }),
            session({ id: "visible", directory: "/root", time: { created: 1, updated: 1, archived: undefined } }),
          ],
        },
      ],
      120_000,
  ...
  test("preserves deterministic ID ordering for recent root sessions", () => {
    const result = latestRootSession(
      [
        {
          path: { directory: "/workspace" },
          session: [
            session({
              id: "recent-z",
              directory: "/workspace",
              time: { created: 110_000, updated: 119_000, archived: undefined },
            }),
            session({
              id: "recent-a",
              directory: "/workspace",
              time: { created: 100_000, updated: 118_000, archived: undefined },
            }),
          ],
        },
      ],
    ...
    expect(result?.id).toBe("recent-a")
  })
