...
import { enrichProject } from "./project-enrich"
...
export const { use: useGlobal, provider: GlobalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const ensureServerCtx = (conn: ServerConnection.Any) => {
      ...
      const connection = JSON.stringify(conn)
    ...
    createEffect(() => {
      for (const conn of server.list) {
        if (serverHealth[ServerConnection.key(conn)]?.healthy !== true) continue
    ...
    createEffect(() => {
      for (const [key] of serverCtxs) {
        const exists = server.list.some((conn) => ServerConnection.key(conn) === key)
        if (exists && serverHealth[key]?.healthy !== false) continue
