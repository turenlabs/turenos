...
export const createDirSyncContext = (
  ...
  return {
    ...
    session: {
      ...
      statusRevision: serverSync.session.statusRevision,
      holdStatusSettlement: serverSync.session.holdStatusSettlement,
      ...
      fetch: async (count = 10) => {
        ...
        const sessions = (response.data ?? [])
          .filter((session) => !!session?.id && !SessionSchema.isInternal(session))
