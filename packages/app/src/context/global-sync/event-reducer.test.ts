...
import type { Message, Part, PermissionRequest, Project, QuestionRequest, Session } from "@turenlabs/sdk/v2/client"
import { createStore } from "solid-js/store"
import type { State } from "./types"
import { applyDirectoryEvent, applyGlobalEvent, cleanupDroppedSessionCaches } from "./event-reducer"

const rootSession = (input: { id: string; parentID?: string; archived?: number; metadata?: Record<string, unknown> }) =>
  ({
    id: input.id,
...
describe("applyDirectoryEvent", () => {
  ...
  test("keeps internal lobby sessions out of passive directory navigation", () => {
    const [store, setStore] = createStore(baseState({ session: [], sessionTotal: 0 }))

    applyDirectoryEvent({
      event: {
        type: "session.created",
        properties: { info: rootSession({ id: "ses_lobby_hidden", metadata: { "forge.internal": true } }) },
      },
      store,
      setStore,
      push() {},
