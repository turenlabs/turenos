...
import { retainStreamedText, streamedText } from "./part-snapshot"
...
export function applyDirectoryEvent(input: {
  ...
  switch (event.type) {
    ...
    case "session.created": {
      ...
      if (SessionSchema.isInternal(info)) {
    ...
    case "session.updated": {
      ...
      if (SessionSchema.isInternal(info)) {
        ...
        cleanupSessionCaches(input.setStore, info.id, input.setSessionTodo)
        if (result.found && !info.parentID) input.setStore("sessionTotal", (value) => Math.max(0, value - 1))
        break
      }
    ...
    // A V2 rename touches one field, so it cannot reorder or trim the list the way a full
    // `session.updated` snapshot can. Patch in place and leave the rest of the row alone.
    case "session.next.title.updated": {
      ...
      if (!result.found) break
      input.setStore("session", result.index, "title", props.title)
      break
    }
    ...
    case "message.part.updated": {
      ...
      if (SKIP_PARTS.has(incoming.type)) break
      const existing = input.store.part[incoming.messageID]?.find((item) => item.id === incoming.id)
      const part = retainStreamedText(incoming, streamedText(input.store.part_text_accum_delta, existing))
      input.setStore(
        produce((draft) => {
          delete draft.part_text_accum_delta[part.id]
        }),
