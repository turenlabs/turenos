...
export function migrateCanonicalLocalServerState(value: unknown, canonicalLocalServer?: ServerConnection.Key) {
  ...
  if (canonicalLocalServer && canonicalLocalServer !== "local") {
    const projects = isRecord(next.projects) ? next.projects : undefined
    const lastProject = isRecord(next.lastProject) ? next.lastProject : undefined
  ...
  return promoteLastProjects(next)
}

function promoteLastProjects(value: Record<string, unknown>) {
  if (!isRecord(value.projects) || !isRecord(value.lastProject)) return value
  const projects = { ...value.projects }
  const lastProject = value.lastProject
  let changed = false

  Object.entries(projects).forEach(([scope, entries]) => {
    const directory = lastProject[scope]
    if (!Array.isArray(entries) || typeof directory !== "string") return
    const index = entries.findIndex(
      (project) =>
        isRecord(project) && typeof project.worktree === "string" && pathKey(project.worktree) === pathKey(directory),
    )
    if (index <= 0) return
    const next = [...entries]
...
export function serverForAvailableSelection(input: {
  servers: ServerConnection.Any[]
  health: Record<ServerConnection.Key, { healthy: boolean } | undefined>
  selected: ServerConnection.Key
  fallback: ServerConnection.Key
}) {
  const selected = input.servers.find((conn) => ServerConnection.key(conn) === input.selected)
  if (selected && input.health[ServerConnection.key(selected)]?.healthy !== false) return selected
  const healthy = (conn: ServerConnection.Any) => input.health[ServerConnection.key(conn)]?.healthy === true
  return (
    input.servers.find((conn) => ServerConnection.key(conn) === input.fallback && healthy(conn)) ??
    input.servers.find(healthy)
  )
}
