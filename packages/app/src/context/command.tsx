...
/**
 * Single registration slot shared by every surface that owns `command.palette`.
 * See {@link useCommandPalette}.
 */
const PALETTE_REGISTRATION = "surface.palette"
...
export interface CommandOption {
  ...
  slash?: string
  /**
   * Slash command takes a free-text argument. Picking it from the `/` popover
   * inserts `/<slash> ` into the composer instead of running `onSelect`, the
   * same way server-provided commands behave.
   */
...
/**
 * Registrations are newest-first: the most recent one wins the keybind, the palette
 * entry and `trigger`. A keyed registration replaces the previous holder of that key,
 * which is how surfaces that own the same command ids (see `useCommandPalette` and
 * `useSurfaceCommands`) hand ownership over on navigation instead of stacking up and
 * having the registry drop one of them by mount order.
 */
        ...
        for (const opt of reg.options()) {
          if (seen.has(opt.id)) {
            if (import.meta.env.DEV && !warnedDuplicates.has(opt.id)) {
              ...
              console.warn(
                `[command] duplicate command id "${opt.id}" registered; keeping the most recent registration`,
              )
            }
            continue
          }
 ...
 */
export function useCommandPalette(onSelect: () => void) {
  const command = useCommand()
  const language = useLanguage()

  command.register(PALETTE_REGISTRATION, () => [
    {
      id: PALETTE_ID,
