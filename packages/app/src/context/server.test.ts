...
test("Agents selection leaves a disconnected saved server for a healthy fallback", () => {
  const local = { type: "sidecar", variant: "base", http: { url: "http://127.0.0.1:4096" } } as const
  const disconnected = {
    type: "http",
    displayName: "Disconnected lab",
    http: { url: "http://127.0.0.1:59999" },
  } as const
  const selected = ServerConnection.key(disconnected)

  expect(
    serverForAvailableSelection({
      servers: [local, disconnected],
      health: {
        [ServerConnection.key(local)]: { healthy: true },
        [selected]: { healthy: false },
      },
      selected,
      fallback: ServerConnection.key(local),
    }),
  ).toBe(local)
})

test("Agents selection does not abandon a server before its first health result", () => {
  const local = { type: "sidecar", variant: "base", http: { url: "http://127.0.0.1:4096" } } as const
  const pending = { type: "http", http: { url: "https://pending.example.test" } } as const

  expect(
    serverForAvailableSelection({
      servers: [local, pending],
      health: { [ServerConnection.key(local)]: { healthy: true } },
      selected: ServerConnection.key(pending),
      fallback: ServerConnection.key(local),
    }),
  ).toBe(pending)
})
...
describe("createServerProjects", () => {
  ...
  test("moves the most recently touched project to the front", () => {
    createRoot((dispose) => {
      const [scope] = createSignal(ServerScope.local)
      const [store, setStore] = createStore({ projects: {}, lastProject: {}, recentlyClosed: {} })
      const projects = createServerProjects({ scope, store, setStore })
      ...
      expect(projects.list().map((project) => project.worktree)).toEqual(["/a", "/c", "/b"])
      expect(projects.last()).toBe("/a")
      dispose()
    })
  })
...
describe("migrateCanonicalLocalServerState", () => {
  test("promotes the persisted last project on startup", () => {
    expect(
      migrateCanonicalLocalServerState({
        projects: {
          local: [
