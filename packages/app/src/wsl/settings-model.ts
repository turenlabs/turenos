...
export function wslForgeAction(check?: WslForgeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install TurenOS"
  if (check.matchesDesktop === false) return "Update TurenOS"
}
...
export function addServerViewModel(input: {
  ...
  const forgeCheck = selectedDistro ? (state?.forgeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.forgeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateForge" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.forgeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installForge" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingForge = input.state?.job?.kind === "install-forge" && input.state.job.distro === input.selectedDistro
  if (!ready || probingForge) {
    return {
      variant: "contrast",
  ...
  if (!addServerForgeReady(input.forgeCheck)) {
    const update = !!input.forgeCheck?.resolvedPath && input.forgeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingForge
        ? { key: "wsl.onboarding.updatingForge" }
        : update
          ? { key: "wsl.onboarding.updateForge" }
          : { key: "wsl.onboarding.installForge" },
      disabled: !!input.state?.job || input.adding,
      action: "install-forge",
      loading: installingForge,
...
function addServerForgeReady(check: WslForgeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.forgeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.forgeChecks[item.name]) return [`forge:${item.name}`]
    return []
  })
