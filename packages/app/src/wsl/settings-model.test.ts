...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when TurenOS needs attention", () => {
    expect(wslForgeAction(undefined)).toBeUndefined()
    expect(
      wslForgeAction({
        distro: "Debian",
        resolvedPath: null,
  ...
  test("does not report ready when TurenOS is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        forgeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.forge/bin/opencode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installForge" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-forge")
  })
