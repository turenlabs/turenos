    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.forgeChecks[item.config.distro]
          const opencodeAction = () => wslForgeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-forge" && wsl.data.job.distro === item.config.distro
          return (
                ...
                <Show when={opencodeAction()}>
                  {(label) => (
                    <ButtonV2
                      ...
                      onClick={() => api && request.mutate(() => api.installForge(item.config.distro))}
                    >
