...
import { resolveCurrentTab } from "@/context/tab-navigation-intent"
...
export function useTitlebarRightMount() {
  ...
  onMount(() => setMount(document.getElementById("forge-titlebar-right")))
  return mount
}
...
export function Titlebar(props: { update?: TitlebarUpdate }) {
  ...
  createEffect(() => {
    ...
    sessionInteractionTrace("route.committed", { route, sessionID: params.id })
    requestAnimationFrame(() => {
      sessionInteractionTrace("route.painted", { route, sessionID: params.id })
    })
  })
  ...
  makeEventListener(window, SESSION_TABS_REMOVED_EVENT, (event) => {
    ...
    if (detail) tabs.removeSessions(detail)
  })
      ...
      <Switch>
        <Match when={useV2Titlebar()}>
          {(_) => {
            ...
            const currentTab = () => {
              const intent = tabs.navigationIntent()
              return resolveCurrentTab({
                routing: isRouting(),
                pending: intent && tabsStore.find((tab) => tabKey(tab) === intent.destinationKey),
                committed: matchRoute(layout.route()),
              })
            }
            ...
            const openNewTab = () => {
              ...
              if (!workspace) return
              tabs.newDraft({ server: workspace.server, directory: workspace.directory }, "", workspace.model)
            }
            ...
            return (
              ...
              >
                ...
                <TitlebarTabStrip
                  ...
                  onNavigate={(tab, el) => {
                    sessionInteractionTrace("tab.navigation-requested", {
                      fromSessionID: params.id,
                      tabType: tab.type,
                      targetTabID: tab.type === "session" ? tab.sessionId : tab.draftID,
                    })
                    startupTrace("tabs", "navigation.requested", {
                      tabType: tab.type,
                      tabID: tab.type === "session" ? tab.sessionId : tab.draftID,
                    })
                    tabs.select(tab)
                    sessionInteractionTrace("tab.navigation-dispatched", {
                      fromSessionID: params.id,
                      tabType: tab.type,
                      targetTabID: tab.type === "session" ? tab.sessionId : tab.draftID,
                    })
                    ...
                    requestAnimationFrame(() =>
                      startupTrace("tabs", "navigation.painted", {
                        tabType: tab.type,
                        tabID: tab.type === "session" ? tab.sessionId : tab.draftID,
                      }),
                    )
                  ...
                  onReorder={(keys, movedKey, dragLayout) => tabsStoreActions.reorder(keys, movedKey, dragLayout)}
                  onToggleGroup={tabsStoreActions.toggleGroup}
                />
                <div class="flex-1" />
        ...
        <Match when>
          ...
          >
              ...
              <div class="flex items-center gap-1 shrink-0">
                ...
                <div class="hidden xl:flex items-center shrink-0">
                  ...
                  >
                    ...
                    <div id="forge-titlebar-left" class="flex items-center gap-3 min-w-0 px-2" />
                    <ChannelIndicator />
                  </div>
                </div>
              </div>
            ...
            <div
              ...
              onMouseDown={drag}
            >
              <div id="forge-titlebar-right" class="flex items-center gap-1 shrink-0 justify-end" />
              <Show when={windows()}>
...
function TitlebarV2Right(props: { state: TitlebarV2RightState }) {
  return (
    <div class="relative z-20 flex shrink-0 items-center justify-end gap-0 overflow-visible">
      ...
      <div id="forge-titlebar-right" class="flex shrink-0 items-center justify-end gap-0" />
    </div>
  )
}
...
function ChannelIndicator() {
  const channel = import.meta.env.VITE_FORGE_CHANNEL
  return (
    <>
