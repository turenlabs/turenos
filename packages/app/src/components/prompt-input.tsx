...
import { DockShellForm, DockTray } from "@turenlabs/ui/dock-surface"
import { Icon } from "@turenlabs/ui/icon"
import { ProviderIcon } from "@turenlabs/ui/provider-icon"
import { Tooltip, TooltipKeybind } from "@turenlabs/ui/tooltip"
...
import { createPromptSubmit, type PromptGoalControls } from "./prompt-input/submit"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const surfaceList = createMemo((): AtOption[] => [
    {
      type: "surface",
      surface: "automations",
      name: "automations",
      display: "automations",
      description: "Scheduled Automations in this app",
    },
    {
      type: "surface",
      surface: "handoff",
      name: "handoff",
      display: "handoff",
      description: "Continue this work in a new top-level session, optionally in another project",
    },
    {
      type: "surface",
      surface: "swarm",
      name: "swarm",
      display: "swarm",
      description: "Run a broad, bounded team investigation",
    },
  ])
  ...
  const atKey = (x: AtOption | undefined) => {
    ...
    if (x.type === "surface") return `surface:${x.surface}`
    return `file:${x.path}`
  }
  ...
  const {
    ...
    items: async (query) => {
      ...
      if (!query.trim()) return [...surfaces, ...agents, ...pinned]
      const paths = await files.searchFilesAndDirectories(query)
      const fileOptions: AtOption[] = paths
        .filter((path) => !seen.has(path))
        .map((path) => ({ type: "file", path, display: path }))
      return [...surfaces, ...agents, ...pinned, ...fileOptions]
    },
    key: atKey,
    filterKeys: ["display"],
    skipFilter: (item) => item.type === "file" && !item.recent,
    groupBy: (item) => {
      if (item.type === "surface") return "surface"
  ...
  const slashCommands = createMemo<SlashCommand[]>(() => {
    const builtin = command.options
      ...
      .map((opt) => ({
        ...
        arguments: opt.slashArguments,
      }))

    const custom = visibleCustomSlashCommands(sync().data.command).map((cmd) => ({
      id: `custom.${cmd.name}`,
      trigger: cmd.name,
      title: cmd.name,
  ...
  const resolveRegisteredSkillSlash = (text: string) =>
    resolveSkillSlash(
      text,
      command.options.flatMap((option) => {
        if (option.disabled || !option.id.startsWith("skill.") || !option.slash) return []
        return [
          {
            id: option.id,
            name: option.slash,
  ...
  const handleSlashSelect = (cmd: SlashCommand | undefined) => {
    ...
    if (cmd.type === "custom" || cmd.arguments) {
  ...
  const createPill = (part: FileAttachmentPart | AgentPart | SurfacePart) => {
    ...
    if (part.type === "surface") pill.setAttribute("data-surface", part.surface)
  ...
  const isNormalizedEditor = () =>
    Array.from(editorRef.childNodes).every((node) => {
      ...
      if (el.dataset.type === "surface") return true
  ...
  const renderEditor = (parts: Prompt) => {
    ...
    for (const part of parts) {
      ...
      if (part.type === "file" || part.type === "agent" || part.type === "surface") {
  ...
  const parseFromDOM = (): Prompt => {
    ...
    const pushSurface = (surface: HTMLElement) => {
      const content = surface.textContent ?? ""
      parts.push({
        type: "surface",
        surface: surface.dataset.surface as SurfaceID,
  ...
  const variantControl = () => (
    <Show when={!providersLoading() && store.mode !== "shell" && showVariantControl()}>
      <div data-component="prompt-variant-control" classList={{ "animate-in fade-in": providersShouldFadeIn() }}>
        <TooltipV2
          placement="top"
          gutter={4}
          value={
        ...
        >
          ...
          >
            <Popover.Trigger
              ...
              variant="ghost-muted"
              size="normal"
              class="max-w-[160px] justify-start"
              style={control()}
            >
              <span class="truncate leading-5">{currentEffort().label}</span>
  ...
  const fastMode = createMemo(() =>
    getFastMode(props.controls.model.selection.current(), props.controls.model.selection.list()),
  )
  const toggleFastMode = () => {
    const mode = fastMode()
    if (!mode) return
    const target = mode.enabled ? mode.base : mode.fast
  ...
  const { abort, handleSubmit, interrupting } =
    ...
    createPromptSubmit({
      ...
      onPendingPrompt: props.onPendingPrompt,
      resolveSkillSlash: resolveRegisteredSkillSlash,
                ...
                <div class="relative max-h-[180px] overflow-y-auto no-scrollbar" ref={(el) => (scrollRef = el)}>
                  <div
                    ...
                    classList={{
                      ...
                      "[&_[data-type=surface]]:text-syntax-type": true,
                      ...
                      <Show when={store.mode !== "shell"}>
                        ...
                        <Show when={showVariantControl()}>
                          ...
                          >
                            ...
                            >
                              <Popover gutter={6} modal={false} placement="top-start">
                                <Popover.Trigger
                                  as={Button}
                                  data-action="prompt-model-variant"
                                  variant="ghost"
                                  size="normal"
                                  class="max-w-[160px] justify-start text-13-regular text-text-base"
                                  style={control()}
                                >
                                  <span class="truncate">{currentEffort().label}</span>
                                  <Icon name="chevron-down" size="small" class="shrink-0" />
                                </Popover.Trigger>
                                <Popover.Portal>
                                  <Popover.Content
