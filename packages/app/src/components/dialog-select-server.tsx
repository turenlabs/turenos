...
export function useServerManagementController(options: { onSelect?: () => void; navigateOnAdd?: boolean } = {}) {
  ...
  const replaceServer = (original: ServerConnection.Http, next: ServerConnection.Http) => {
    ...
    // Teardown-before-server.remove ordering already holds here, and nothing
    // stops the old HTTP server, so the batch stays fire-and-forget.
    void tabs.removeServer(originalKey)
