...
import { ProgressCircle } from "@turenlabs/ui/progress-circle"
import { ProgressCircleV2 } from "@turenlabs/ui/v2/progress-circle-v2"
...
export function SessionContextUsage(props: SessionContextUsageProps) {
  ...
  const tooltipValue = () => (
    <div class="flex w-[120px] flex-col gap-2">
      ...
      {/* A model with no published context limit has no knowable percentage. "—" says that;
          "0%" would be a fabrication, and the ring below stays empty for the same reason. */}
      <ContextTooltipRow
        name={language.t("context.usage.usage")}
        value={context()?.usage === undefined || context()?.usage === null ? "—" : `${context()!.usage}%`}
      />
      <ContextTooltipRow
