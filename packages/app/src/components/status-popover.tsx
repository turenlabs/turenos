...
import { Popover } from "@turenlabs/ui/popover"
...
export function StatusPopover() {
  ...
  const ready = createMemo(() => serverHealth() === false || sync().data.lsp_ready)
  const issue = createMemo(() => hasNonBlockingServiceIssue((sync().data.lsp ?? []).map((item) => item.status)))
