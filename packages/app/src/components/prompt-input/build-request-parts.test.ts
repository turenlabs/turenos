...
import { buildRequestParts, toV2Prompt } from "./build-request-parts"

describe("buildRequestParts", () => {
  test("submits @swarm for Core normalization without app-side handoff guidance", () => {
    const result = buildRequestParts({
      prompt: [{ type: "surface", surface: "swarm", name: "swarm", content: "@swarm", start: 0, end: 6 }],
      context: [],
      images: [],
      text: "@swarm compare X and Y",
      messageID: "msg_swarm_guidance",
      sessionID: "ses_swarm_guidance",
      sessionDirectory: "/repo",
    })

    expect(result.v2Prompt.text).toBe("@swarm compare X and Y")
    expect(result.v2Prompt.text).not.toContain("<handoff-surface>")
    expect(result.requestParts).toHaveLength(1)
  })

  test("injects the top-level session handoff guidance for @handoff", () => {
    const result = buildRequestParts({
      prompt: [{ type: "surface", surface: "handoff", name: "handoff", content: "@handoff", start: 0, end: 8 }],
      context: [],
      images: [],
      text: "@handoff",
      messageID: "msg_handoff_guidance",
      sessionID: "ses_handoff_guidance",
      sessionDirectory: "/repo",
    })

    expect(result.v2Prompt.text).toContain("<handoff-surface>")
    expect(result.v2Prompt.text).toContain("handoff_session")
    expect(result.v2Prompt.text).toContain("not a subagent child")
    expect(result.v2Prompt.text).toContain("optional project argument")
    expect(result.v2Prompt.text).toContain("absolute path")

    const literal = buildRequestParts({
      prompt: [{ type: "text", content: "Please use @handoff.", start: 0, end: 20 }],
      context: [],
      images: [],
      text: "Please use @handoff.",
      messageID: "msg_handoff_literal",
      sessionID: "ses_handoff_literal",
      sessionDirectory: "/repo",
    })
    expect(literal.v2Prompt.text).toContain("<handoff-surface>")
  })
  ...
  test("builds typed request and optimistic parts without cast path", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.forgeComment &&
          (part.metadata.forgeComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)

    expect(result.optimisticParts).toHaveLength(result.requestParts.length)
    expect(result.optimisticParts.every((part) => part.sessionID === "ses_1" && part.messageID === "msg_1")).toBe(true)
    expect(result.v2Prompt.text).toContain("hello @src/foo.ts @planner")
    expect(result.v2Prompt.text).toContain("check this")
    expect(result.v2Prompt.parts).toEqual(
      result.requestParts
    ...
    expect(result.v2Prompt.files?.map((file) => file.name)).toEqual(["foo.ts", "bar.ts", "a.png"])
    expect(result.v2Prompt.files?.[0]?.source).toEqual({
      text: "@src/foo.ts",
      start: 5,
      end: 16,
    })
    expect(result.v2Prompt.agents).toEqual([
      {
        name: "planner",
        source: { text: "@planner", start: 16, end: 24 },
      },
    ])
  })

  test("keeps ignored structured text durable without returning it to aggregate model text", () => {
    const result = buildRequestParts({
      prompt: [],
      context: [],
      images: [],
    ...
    const visible = result.requestParts[0]
    if (visible?.type !== "text") throw new Error("Expected text part")
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "forge.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.forge.desktop.beta\\forge.global.dat",
          mime: "text/plain",
