...
describe("attachmentMime", () => {
  test("preserves supported image MIME variants", async () => {
    const variants = [
      ["pixel.png", "image/png"],
    ...
    expect(
      await Promise.all(
        variants.map(([name, type]) => attachmentMime(new File([Uint8Array.of(0, 255)], name, { type }))),
      ),
    ).toEqual(variants.map(([, type]) => type))
  })
  ...
  test("attaches binary files as-is instead of rejecting them", async () => {
