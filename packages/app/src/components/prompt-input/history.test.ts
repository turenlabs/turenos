...
describe("prompt-input history", () => {
  ...
  // The list is persisted as one value under a 1 MiB per-value cap, so a single retained data URL
  // rejects the write for every entry beside it and re-serializes megabytes on each submit.
  test("prompt history never carries image payloads", () => {
    const image = (value: string): Prompt => [
      { type: "text", content: value, start: 0, end: value.length },
      { type: "image", id: "1", filename: "img.png", mime: "image/png", dataUrl: `data:image/png;base64,${"A".repeat(2048)}` },
    ]

    const entries = prependHistoryEntry([], image("look at this"))
    expect(entries).toHaveLength(1)
    expect(JSON.stringify(entries)).not.toContain("base64")
    expect(normalizePromptHistoryEntry(entries[0]!).prompt).toEqual(text("look at this"))

    // A prompt that was nothing but an attachment has nothing left to recall.
    const imageOnly = prependHistoryEntry(entries, [
      { type: "image", id: "2", filename: "img.png", mime: "image/png", dataUrl: "data:image/png;base64,abc" },
    ])
    expect(imageOnly).toBe(entries)

    // Entries written before the cap was understood heal on read rather than being carried forward.
    const legacy = normalizePromptHistoryEntry({ prompt: image("legacy"), comments: [] })
    expect(legacy.prompt).toEqual(text("legacy"))

    // ...and the next submit sheds them from the stored list, so a poisoned store shrinks itself.
    const poisoned = [{ prompt: image("older"), comments: [] }]
    const shed = prependHistoryEntry(poisoned, text("newer"))
    expect(JSON.stringify(shed)).not.toContain("base64")
    expect(shed).toHaveLength(2)

    // A duplicate submit still commits the shed list rather than returning the oversized one.
