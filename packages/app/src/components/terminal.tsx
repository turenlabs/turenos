import { withAlpha } from "@turenlabs/ui/theme/color"
import { useTheme } from "@turenlabs/ui/theme/context"
import { resolveThemeVariant } from "@turenlabs/ui/theme/resolve"
import { resolveThemeVariantV2 } from "@turenlabs/ui/theme/v2/resolve"
import type { HexColor, ResolvedV2Theme } from "@turenlabs/ui/theme/types"
...
import { TerminalPtyGoneError } from "@/pages/terminal-lifecycle"
...
const useTerminalBindings = (input: {
...
export const Terminal = (props: TerminalProps) => {
  ...
  const username = auth?.username ?? "forge"
    ...
    return (
      local.pty.shared
        ? client.v2.pty.update({
            ptyID: id,
            location: { directory, workspace: local.pty.workspaceID },
            size: { cols, rows },
          })
        : client.pty.update({ ptyID: id, size: { cols, rows } })
    ).catch((err) => {
  ...
  onMount(() => {
    const run = async () => {
      ...
      const gone = () =>
        (local.pty.shared
          ? client.v2.pty.get(
              { ptyID: id, location: { directory, workspace: local.pty.workspaceID } },
              { throwOnError: false },
            )
      ...
      const retry = (err: unknown) => {
        ...
        reconn = setTimeout(async () => {
          ...
          if (await gone()) {
            if (disposed) return
            // The server confirmed the PTY is gone - the one signal on which
            // persisted-PTY recovery may relaunch. Every
            // other failure path keeps its raw error so recovery never closes
            // a PTY that might still be alive.
            fail(new TerminalPtyGoneError(err))
            return
          }
          if (disposed) return
          tries += 1
      ...
      const open = async () => {
        ...
        const socket = new WebSocket(
          terminalWebSocketURL({
            ...
            id,
            directory,
            workspaceID: props.pty.workspaceID,
            canonical: local.pty.shared,
            cursor: seek,
            ticket,
            sameOrigin,
            username,
  ...
  return (
    <div
      ...
      classList={{
        ...local.classList,
        "select-text": true,
        // overscroll-contain: wheel deltas ghostty deliberately leaves
        // unconsumed (pure-horizontal trackpad gestures) must never chain into
        // ancestor scrollers or viewport overscroll effects such as
        // swipe-to-navigate.
        "size-full px-6 py-3 font-mono relative overflow-hidden overscroll-contain": true,
