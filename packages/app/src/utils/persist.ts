...
function draftStorage(draftID: string) {
  ...
  return `forge.draft.${head}.${sum}.dat`
}
...
// Read-modify-write a persisted value directly in storage, for callers that
// need to amend persisted state while no live `persisted` store owns the key
// (e.g. terminal teardown pruning a workspace store after its provider
// unmounted). `update` returns the replacement value, or undefined to skip the
// write. Best-effort: storage failures are swallowed like removePersisted.
export async function updatePersisted(
  ...
  const storage = (() => {
    if (isDesktop) return platform.storage?.(target.storage)
    if (!target.storage) return localStorageDirect()
  ...
  try {
    ...
    await storage.setItem(target.key, JSON.stringify(next))
  } catch {}
}
...
export async function writePersisted(
  ...
  const storage =
    platform?.platform === "desktop" && platform.storage
      ? platform.storage(target.storage)
      : target.storage
