...
describe("terminalWebSocketURL", () => {
  ...
  test("includes the workspace for a session-shared PTY", () => {
    const url = terminalWebSocketURL({
    ...
    expect(url.pathname).toBe("/api/pty/pty_shared/connect")
    expect(url.searchParams.get("location[workspace]")).toBe("wrk_shared")
  })
})
