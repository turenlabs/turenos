...
>()
const healthTransitions = new Map<string, boolean>()
...
export function useCheckServerHealth() {
  ...
  return (http: ServerConnection.HttpBase, opts?: CheckServerHealthOptions) => {
    const key = `${cacheKey(http)}\n${opts?.timeoutMs ?? "default"}\n${opts?.retryCount ?? "default"}`
    ...
    const refresh = async () => {
      ...
      await Promise.all(
        list.map(async (conn) => {
          ...
          results[key] = result
          if (healthTransitions.get(key) !== result.healthy) {
            healthTransitions.set(key, result.healthy)
            console.info(
              "[server.health]",
              JSON.stringify({
