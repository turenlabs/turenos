...
/**
 * `FORGE_CHANNEL` is a build-time define (see `electron.vite.config.ts`), so
 * this folds to a literal and the dev-only profiler bridge below is dropped
 * from beta and prod bundles rather than shipped and refused.
 */
const PROFILER_ENABLED = import.meta.env.FORGE_CHANNEL === "dev"

const profiler: ProfilerAPI | undefined = PROFILER_ENABLED
  ? {
      status: () => ipcRenderer.invoke("profiler-status"),
      start: () => ipcRenderer.invoke("profiler-start"),
      stop: () => ipcRenderer.invoke("profiler-stop"),
      subscribe: (cb: (status: ProfilerStatus) => void) => {
        const handler = (_: unknown, status: ProfilerStatus) => cb(status)
        ipcRenderer.on("profiler-state", handler)
        void ipcRenderer.invoke("profiler-subscribe")
        return () => {
          ipcRenderer.removeListener("profiler-state", handler)
          void ipcRenderer.invoke("profiler-unsubscribe")
        }
      },
    }
  : undefined
...
const api: ElectronAPI = {
  ...
  wslServers: {
    ...
    installForge: (name) => ipcRenderer.invoke("wsl-servers-install-forge", name),
  ...
  finishFirstLaunchOnboarding: () => ipcRenderer.invoke("finish-first-launch-onboarding"),
  ...
  getWindowLastActiveUrl: (legacy) => ipcRenderer.invoke("get-window-last-active-url", legacy),
  setWindowLastActiveUrl: (value) => ipcRenderer.invoke("set-window-last-active-url", value),
  onMenuCommand: (cb) => {
