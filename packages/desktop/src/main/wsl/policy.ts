import type { WslDistroProbe, WslForgeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  forgeChecks: Record<string, WslForgeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextForgeChecks = { ...forgeChecks }
  delete nextDistroProbes[distro]
  delete nextForgeChecks[distro]
  return { distroProbes: nextDistroProbes, forgeChecks: nextForgeChecks }
}
