...
let releaseForgeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectForgeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectForgeVersion("1.14.35", "1.16.2")).toThrow(
    "TurenOS update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating TurenOS", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.forge/bin/forge",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, forgeChecks: {} })
})
...
test("ignores stale background TurenOS checks after removing a WSL server", async () => {
  persistedServers = []
  releaseForgeResolve = undefined
  ...
  await waitFor(() => !!releaseForgeResolve)
...
test("ignores stale startup TurenOS checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking TurenOS", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveForge: async (distro) => {
      forge.push(distro)
      return "/home/me/.forge/bin/forge"
    },
  })
  ...
  expect(forge).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().forgeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check TurenOS in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const forge: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(forge).toEqual(["Debian"])
  expect(Object.keys(controller.getState().forgeChecks)).toEqual(["Debian"])
})
