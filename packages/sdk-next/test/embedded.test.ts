...
import type { ForgeEvent } from "../src"

test("embedded client uses the real router and handlers", async () => {
  const directory = await mkdtemp(join(tmpdir(), "opencode-embedded-"))
  const database = Flag.FORGE_DB
  Flag.FORGE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, Model, Forge, Prompt, Provider, Session, Tool } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  const model = Model.Ref.make({ id: Model.ID.make("embedded"), providerID: Provider.ID.make("test") })

  try {
    const program = Effect.gen(function* () {
      const opencode = yield* Forge.create()
...
test("Location-owned runner events reach the ready global client", async () => {
  ...
  const database = Flag.FORGE_DB
  Flag.FORGE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, Forge, Prompt, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const prompted = yield* Deferred.make<ForgeEvent>()
      ...
      const event = yield* Deferred.await(prompted).pipe(Effect.timeout("30 seconds"))
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  const database = Flag.FORGE_DB
  Flag.FORGE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, Forge, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const program = Effect.gen(function* () {
      const first = yield* Forge.create()
      const second = yield* Forge.create()
      ...
      const observe = (ready: Latch.Latch, event: Latch.Latch) =>
        Stream.runForEach((notification: ForgeEvent) =>
      ...
      yield* firstEvent.await.pipe(Effect.timeout("30 seconds"))
...
test("embedded client is available as a Layer service", async () => {
  ...
  const database = Flag.FORGE_DB
  Flag.FORGE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, Forge, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const opencode = yield* Forge.Service
        return yield* opencode.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
      }).pipe(Effect.provide(Forge.layer), Effect.scoped),
    )
