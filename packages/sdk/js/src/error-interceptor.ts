 ...
 * into a real `Error` so downstream formatters (App, plugins) get a
...
export function wrapClientError(
  ...
  // NamedError-shaped responses (the common case for TurenOS 4xx) come
  ...
  return new Error(`TurenOS server ${describe(request, response)}: ${reason}`, {
