...
describe("CodeMode public contract", () => {
  ...
  test("reports unparseable code as a ParseError diagnostic, not a thrown defect", async () => {
    // The compiler is loaded through a dynamic import, so a parse failure travels
    // out as an Effect defect and is only a diagnostic because `catchCause` squashes
    // it back. Nothing else covers that path.
    for (const code of ["return (((((", "return 'unterminated", "class {{{"]) {
      const result = await Effect.runPromise(CodeMode.execute({ tools: {}, code }))
      expect(result.ok).toBe(false)
      if (result.ok) return
      expect(result.error.kind).toBe("ParseError")
      expect(result.error.message).toContain("Failed to parse TypeScript")
    }
  })

  test("a caller's timeout budget is not spent loading the compiler", async () => {
    // Must be the *first* execution in its process to mean anything: once the
    // compiler is warm this passes either way, so it runs in a fresh one.
    const child = await Bun.spawn(
      [
        process.execPath,
        "-e",
        `import { Effect } from "effect"
         import { CodeMode } from ${JSON.stringify(new URL("../src/index.js", import.meta.url).href)}
         const r = await Effect.runPromise(CodeMode.execute({ tools: {}, code: "return 1 + 1", limits: { timeoutMs: 50 } }))
    ...
    // batou:ignore deserialize -- output of a subprocess this test spawned with a literal program; JSON.parse is already the safe reader
    expect(JSON.parse(output)).toMatchObject({ ok: true, value: 2 })
  })
