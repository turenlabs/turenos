...
const tools = {
  ...
  browser: { navigate: echo("Navigate somewhere") },
}
...
describe("Object.keys over tool references", () => {
  test("enumerates top-level namespaces (the transcript program)", async () => {
    ...
    `),
    ).toEqual({ namespaces: ["github", "memory", "browser", "$codemode"], count: 4 })
  })
...
describe("for...in", () => {
  ...
  test("enumerates namespaces and tools from the callable tool tree", async () => {
    expect(
      ...
      return names
    `),
    ).toEqual(["github.list_issues", "github.get_issue", "memory.search", "browser.navigate", "$codemode.search"])
  })
