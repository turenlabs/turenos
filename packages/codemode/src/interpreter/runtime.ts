...
// The TypeScript compiler is the single largest module in the server bundle and
// is needed only to transpile a snippet, so it loads on first execution rather
// than at import time. Memoized, and awaited outside the execution budget so the
// one-off load is never charged to a caller's `timeoutMs`.
let compiler: Promise<typeof import("typescript")> | undefined
const loadCompiler = () => (compiler ??= import("typescript"))

const parseProgram = async (code: string): Promise<ProgramNode> => {
...
export const executeWithLimits = <const Tools extends Record<string, unknown>>(
  ...
  const operation = Effect.gen(function* () {
    const program = yield* Effect.promise(() => parseProgram(options.code))
  ...
  // The compiler load is warmed ahead of `operation` so it lands outside the
  // timeout the caller asked for; only their code runs against that budget.
  return Effect.promise(loadCompiler).pipe(
