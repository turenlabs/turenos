...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".forge/glossary/zh-cn.md"
  if (locale === "zht") return ".forge/glossary/zh-tw.md"
  return `.forge/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("TurenOS did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages)) throw new Error("TurenOS returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("TurenOS session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel TurenOS runs for 'all' (default: 4)
      --model <provider/id>  TurenOS model (default: opencode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running TurenOS
...
`)
  ...
  if (failed.length) console.error(`\nTurenOS failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.FORGE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
...
function isolatedEnvironment() {
  ...
  delete env.FORGE_CONFIG
  delete env.FORGE_CONFIG_DIR
  delete env.FORGE_CONFIG_CONTENT
  delete env.FORGE_PERMISSION
  delete env.FORGE_AUTO_SHARE
  return env
}
