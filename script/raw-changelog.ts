...
const repo = process.env.GH_REPO ?? "turenlabs/turenos"
...
function section(areas: Set<string>) {
  const priority = ["core", "app", "desktop", "sdk", "plugin", "github"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/forge packages/sdk packages/plugin packages/desktop packages/app packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/forge/")) areas.add("core")
