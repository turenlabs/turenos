...
| models.dev catalog             | Fetched from `https://models.dev` at server start unless the cached copy is under five minutes old, then every 60 minutes. The cache is written to the cache directory.                                                                                                         | `packages/core/src/models-dev.ts`                                                                                      | `FORGE_DISABLE_MODELS_FETCH`. `FORGE_MODELS_URL` points at another source; `FORGE_MODELS_PATH` reads a local file instead of the cache. |
