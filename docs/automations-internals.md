...
HTTP surface. For how to build and run one in the app, see [Automations](./automations.md).

Automations are durable scheduled runs that execute on the currently selected TurenOS server. Each occurrence
...
- **Project**: the selected server's project directory; the Automation runs in that directory's context
- **Schedule**: exactly one trigger — an **Interval** (a fixed duration between occurrences using `s`, `m`, `h`, or `d`, e.g. `5m`, `1h`, `1d`; minimum 60 seconds), a **Cron** expression (five fields `minute hour day month weekday`, at most 120 characters, e.g. `0 9 * * MON-FRI`), or a server **Event** trigger (file-change or session-end). Cron fire times follow the **Timezone** (an IANA name such as `America/New_York`, default `UTC`). Editing accepts at most one of a new interval, cron expression, or event trigger and replaces the previous schedule
...
- `automation_create`: create a new Automation from a name, one trigger (interval, cron, or server event), and 1 to 12 ordered steps with optional per-step `when` / `on_failure`
...
If an Automation is paused, the selected server is down, or its scheduler is offline, scheduled occurrences are missed. When the scheduler resumes, it does not replay every missed tick. Instead, it schedules the next occurrence one interval after the current time. This prevents catch-up storms when many ticks are overdue.
...
Automation management targets the currently selected configured server. Listing Automations and their metadata reads that server's process-global SQLite index without opening or monitoring project directories. Project context is resolved only when configuring the Automation or at the moment of execution on the selected server.
...
Each run uses a deterministic Session ID derived from the Automation ID and scheduled time. If the hosting server crashes after the Session is created but before it completes:
...
Event Automations replace the ticking schedule with a server trigger stored as `trigger_type` / `trigger_config`:
...
All Automation operations target the selected server:
...
There is deliberately no event endpoint: `fireEvent` exists only on the core `Loop` service and is driven by the selected server's scheduler, file watcher, and session listeners.
