...
| `server`          | object                     | see below                                 | Legacy. Defaults for `forge serve` and `forge web` networking.                                             |
...
| `FORGE_EXPERIMENTAL_FILEWATCHER`                              | off                                    | Also watches the working tree of a version-controlled Location, not only its `.git` directory.                                                                            |
| `FORGE_EXPERIMENTAL_DISABLE_FILEWATCHER`                      | off                                    | Turns off all file watching.                                                                                                                                              |
| `FORGE_DISABLE_LSP_DOWNLOAD`                                  | off                                    | Legacy only: never downloads or installs language servers.                                                                                                                |
...
| `FORGE_DISABLE_AUTOUPDATE`                                    | off                                    | Legacy CLI: same as `autoupdate: false`.                                                                                                                                  |
| `FORGE_ALWAYS_NOTIFY_UPDATE`                                  | off                                    | Legacy CLI: always notifies with the latest version and never installs.                                                                                                   |
| `FORGE_SERVER_USERNAME`, `FORGE_SERVER_PASSWORD`              | `forge`, unset                         | Legacy server Basic Auth. Without a password, `forge serve` refuses a non-loopback hostname unless `--insecure` is set.                                                   |
