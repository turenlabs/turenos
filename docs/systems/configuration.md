...
| `FORGE_SERVER_USERNAME`, `FORGE_SERVER_PASSWORD`              | `forge`, unset                         | Server Basic Auth for both runtimes. Without a password, `forge serve` refuses a non-loopback hostname unless `--insecure` is set.                                        |
