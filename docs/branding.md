...
| Provider attribution       | `X-Title: Forge`, `X-Source: Turen`, `X-BILLING-INVOKE-ORIGIN: Forge`, `X-Cerebras-3rd-Party-Integration: Forge` | These identify registered provider integrations, not the Desktop display name; change them only with provider-specific registration review. |
...
For the source-grounded architecture and system descriptions, see [Architecture](./architecture.md)
and [Systems](./systems.md). The implementation rationale for several retained names is also recorded
