...
**Importantly, the changelog is for users (who are at least slightly technical), and covers Desktop, SDKs, plugins, and the bundled runtime. Be thorough in understanding effects that may not be immediately apparent. For example, a package upgrade may fix a user-visible bug, and a refactor may stabilize a race condition. The PR title/body and commit message usually contain the intended outcome.**
