...
V2 Sessions persist the exact privileged System Context shown to the model. A Context Epoch stores one immutable provider-cache baseline and a model-hidden structured snapshot used to compare independently observed Context Sources. Environment facts, the host-local date, ambient global/upward-project `AGENTS.md` files with `CLAUDE.md` compatibility fallback, and selected-agent available-skill guidance are the initial sources. Location-wide sources come from the System Context Registry; selected-agent guidance composes with them immediately before Context Epoch admission.
...
`session.next.compaction.started.1` durably identifies the attempt. Compaction deltas are live-only progress. `session.next.compaction.ended.1` durably stores the final summary and serialized recent context; only this completed event projects a model-visible compaction message. The summarizer reserves the configured context buffer rather than targeting the model's exact edge. If the summary request itself receives a context-overflow failure before emitting text, it retries at most twice with the fitted history budget halved each time; no partial summary is replayed or combined. On the next provider attempt, the runner observes that completed compaction and directly renders a fresh Context Epoch baseline. A failed or interrupted attempt therefore leaves the previous history boundary active.
...
The public Session message listing uses the same newest-checkpoint boundary as provider history. After compaction, paginated loads return the checkpoint first, followed by its retained structured tail and messages committed after it; pre-checkpoint chat rows no longer reappear when the Session is reopened. Those older rows remain durable and individually addressable for audit/recovery rather than being deleted. Timeline presentation also treats any checkpoint in a raced page load as an authoritative clear boundary and discards rows accumulated before it.
...
| Durable Context Source     | Global and upward project instructions                                   | partial  | Decide whether V2 also discovers deprecated `CONTEXT.md`; `CLAUDE.md` compatibility fallback is complete.                              |
...
| Durable Context Source     | MCP server instructions for servers with a visible tool                  | missing  | V1 renders `<mcp_instructions>`; add a context source alongside the demand-driven MCP registration.                                    |
...
| Per-turn request assembly  | Policy-filtered built-in, MCP, plugin, and structured-output tools       | partial  | MCP tools are wired through demand-driven registration; see the note below. Plugin tools and structured output missing.                |
...
| Per-turn request assembly  | Plugin message, system, parameter, and header transforms                 | missing  | Design `chat.params`/`chat.headers` analogues. Tool execution hooks already landed as `tool/interceptor.ts`.                           |
...
The runner has two bounded retry layers. Request execution invisibly retries connection setup and retryable response statuses; Session execution durably reports longer provider waits and retries rate-limit/internal responses at most eight times. A `200` response whose body stream fails retries at most three times, and malformed provider frames retry at most twice. Guessed backoff is exponentially delayed with stable per-Session jitter to avoid a synchronized retry wave; an explicit provider `Retry-After` is honored without jitter or shortening.

A provider request may be replayed only before visible text or tool input begins. Reasoning-only output remains replay-safe because it has no external side effect and is flushed durably between attempts. After partial text or incomplete tool input, TurenOS does not replay the request. On a route where provider-side tool execution cannot begin before an observable call, if no tool call or step settlement was observed, TurenOS fails the partial assistant and any incomplete tool as not executed, then makes one bounded fresh continuation from reloaded durable history. This continuation keeps the partial text, preserves any newer user input as the highest-priority instruction, tells the model not to repeat completed text, and asks it to regenerate an unfinished tool call from scratch. A second partial-stream failure, an unqualified provider route, any observed tool call, or any step settlement remains terminal, so a request that may already have executed a tool is never replayed. Malformed model-generated tool argument JSON is handled separately from provider replay: TurenOS records the attempted input as a failed, recoverable tool call, does not execute it, and continues so the model can regenerate valid JSON.

Provider timeout and watchdog policy remains intentionally deferred. The runner does not impose a universal provider-stream inactivity or absolute timeout. A future slice should design configurable policy around provider behavior, durable failure reporting, and local drain-chain release rather than hardcoding one default for every provider.

A terminal provider stream failure, including an untyped runtime defect such as a JavaScript stack overflow, settles the assistant step and pauses an active goal. Reconnect recovery does not infer a safe provider replay from a failed or stale Session; an explicit prompt or goal resume is required. A pending inbox input remains eligible for a new wake, and interruptions remain resumable rather than being treated as provider failure.

MCP tools are registered on demand. `tool/mcp.ts` translates a server listing into canonical tools and registers them per Location, and `forge/src/mcp/tool-source.ts` fills its port from the single MCP service the process already owns. The trigger is `ToolRegistry.provision`: a source registers an idempotent installer that the registry runs before the first materialization of a turn, so MCP is touched only when a session actually needs tools. Registering at Location-graph construction would not work — `MCP.Service` is keyed by `InstanceState`, so reaching it runs `InstanceBootstrap.run` (config, plugins, LSP, format, vcs, snapshot) for that directory, and a Location graph is built for any location-scoped request, so file search or a pty would start a V1 instance and spawn one MCP child per configured server. An event-driven approximation (refreshing on prompt admission) races the first turn's materialization and is not a substitute.

Plugin session hooks are deferred, and porting V1's set mechanically is the wrong move. V1 dispatches `(input, output) => Promise<void>` hooks whose contract is in-place mutation of a loosely typed bag; only 8 of its ~30 call sites even read the return value. Sorting the surface by whether V2 has a real analogue:

- **Genuine analogue, worth designing.** `tool.execute.before`/`.after` are **done**; see below. `shell.env` has a TODO waiting for it in `tool/bash.ts` and an existing port-and-adapter precedent in `PtyEnvironment`. `chat.params`/`chat.headers` gate per-request shaping that Copilot, Codex and the Cloudflare AI Gateway still need, and V2's request assembly has the session context those hooks want. `tool.definition` maps to `materialize`, but no plugin implements it.
- **Already met by a V2 capability; do not add a hook.** `experimental.chat.system.transform` is what `SystemContextRegistry` contributors are for. `experimental.provider.small_model` is `catalog.model.default`. `auth`/`provider` are `integration.transform`/`catalog.transform`, already ported. Plugin-supplied tools are `ApplicationTools.register`; only the boot path that reads them is missing.
  The first landed hook pair is `tool.execute.before`/`.after`, exposed as the `tool` namespace on `PluginContext` and owned by `tool/interceptor.ts`. It is a capability registration, not a hook bus: V2 has no `trigger`, so the service that owns a boundary runs its own registrations there. `ToolRegistry.settleRegistration` is the only caller, which is what collapses V1's eight trigger sites into one. Registrations are Location scoped like the plugins that create them, so subagent calls fire the same interceptors under the child Session's ID.

`before` receives the raw provider arguments, since that is the only representation where replacing them still means something. It may return no opinion, `deny`, or `replace`. `replace` composes in registration order and is re-decoded through the tool's own input schema, so a plugin cannot introduce a shape the tool never validated; `deny` is terminal and monotone, stopping the call before any side effect and skipping every later interceptor, so a security veto is not reversible by load order. `after` receives the settled result after execution, encoding and bounding. It is observation-only apart from `notes`, an append-only advisory channel whose text is added to the model-visible output; notes are dropped for denied and failed calls, and their count and length are bounded. Annotation rewrites the result and the projected output together, rendering a structured-only result as text first so an annotation never evicts the result it annotates.

Interceptors are bounded by a whole-phase budget rather than a per-plugin one, so N slow plugins cannot multiply the wait. One that throws, dies, or overruns the budget is treated as having no opinion and the call proceeds; a plugin cannot both crash and block. Interruption is never swallowed. Interceptors do not run for unknown or stale tool names, because nothing executes on those paths.

- **No analogue, because V2 removed the decision point or the data shape.** `experimental.compaction.autocontinue` has nothing to decide — V2 always continues after compaction. `experimental.text.complete` and `chat.message` would rewrite records that are already durably published. `experimental.chat.messages.transform` is typed against V1 message/part rows. `experimental.session.compacting` targets a compaction lifecycle that has already drifted from V1's. `permission.ask` is declared but never triggered on either path, so it should be deleted rather than ported.
