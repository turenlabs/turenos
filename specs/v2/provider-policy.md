...
- Policies are read by traversing authored config documents in reverse order, so user-global policy can override repository policy.
...
The relative policy precedence of direct project files and `.forge` files is intentionally deferred until `.forge` configuration is reviewed.
