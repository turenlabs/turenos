...
description: Write, update, move, restructure or audit TurenOS documentation following the repository's documentation method. Engineering docs go in docs/ sections (architecture, systems, providers, operations, experimental, assets). Normative contracts stay in specs/. Area docs sit beside tools/, services/catalog and each package, and prototypes go in mockups/. The systems catalog indexes every system page. Enforces TurenOS branding (TurenOS in prose, `forge` identifiers exact), source-grounded pages and link hygiene, and ships a checker and a link-preserving page mover. Use it whenever a change adds, edits, moves or reorganizes anything under docs/, documents a system or feature, or fixes documentation links.
---

# turen-documentation

TurenOS documentation is **federated**. Cross-cutting engineering docs live in `docs/`. Normative contracts live in `specs/`. Each self-contained area (a WASM tool, the extension catalog, a package) documents itself beside its code. This skill is the method for keeping it that way, and it works with any coding agent. Run every command from the repository root. The only prerequisite is Bun, which the repository already requires.
...
| Engineering docs  | `docs/<section>/`                                                                                                                                                                        | How TurenOS systems work, how to run and ship them, and experimental work. The sections are below.                                                                                          |
| Contracts         | `specs/`, `packages/<pkg>/specs/`                                                                                                                                                        | Normative. See the rule above.                                                                                                                                                              |
| Package docs      | `packages/<pkg>/README.md`, plus `packages/<pkg>/docs/` when one README would get long                                                                                                   | What the package is, its commands, and links to `docs/`. Internals such as `packages/llm/docs/architecture.md` go in focused pages under the package's `docs/`. Don't duplicate docs pages. |
| Area docs         | `tools/README.md`, `tools/docs/`, `tools/<target>/README.md` (+ `PROVENANCE.md`, `VERIFY_WASM.md`); `services/catalog/README.md`, `services/catalog/docs/`                               | Areas that ship in the monorepo but document themselves, under the same writing rules. `docs/README.md` links them under "Elsewhere in the repo".                                           |
...
Before writing a new page, run down this table. If the topic belongs to one area or one package, it goes there, not in `docs/`.
...
2. It's about one area that documents itself (a WASM tool target, catalog manifests, one package's API). **That area's README or docs.**
...
7. It describes how a named system behaves. **`systems/`** (see the next section).
...
- **Page size.** Keep a page under about 250 lines, covering one topic. The checker warns past 300. Split a long page along its `##` sections instead of letting it grow: `move.ts` turns `page.md` into `page/README.md`, then `split.ts` moves sections into sibling pages and rewrites every anchor link. Give each moved section a one-sentence summary and link in the main page. The systems catalog is exempt because it is one index table.
...
1. **Survey.** Run the checker for a baseline. It enforces the tables and conventions above, plus the naming, link, anchor, orphan and index rules from the practices:
   ...
   ```
2. **New page**: run the federated map, then "Where a page goes". Write it under the practices and the conventions above. Then update the indexes and the catalog row.
