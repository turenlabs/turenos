...
description: Write, update, move, restructure or audit TurenOS documentation following the repository's documentation method. All documentation is centralized in docs/ sections (architecture, systems, providers, operations, development, experimental, assets); package and area READMEs are short entry points that link there, AGENTS.md files point there for detail, normative contracts stay in specs/, and prototypes go in mockups/. The systems catalog indexes every system page. Enforces TurenOS branding (TurenOS in prose, `forge` identifiers exact), source-grounded pages, one topic per page, and link hygiene, and ships a checker, a link-preserving page mover, and a section splitter. Includes a confusion-audit method for finding wrong claims, contradictions between pages, and undocumented systems. Use it whenever a change adds, edits, moves or reorganizes documentation, documents a system, package, or feature, fixes documentation links, or audits the docs.
---
...
| Not documentation       | Agent instruction files (`AGENTS.md`, `CLAUDE.md`), `.forge/` (runtime config, commands, translation glossaries), other `.agents/` content, `tools/*/vendor/` and imported upstream docs | Leave alone. Docs may name `AGENTS.md` as a concept, always in backticks (the checker flags a bare mention as a reference), but never link one or depend on its contents; the `turen-context` skill maintains `AGENTS.md`. A fact a person needs belongs in `docs/` even when an `AGENTS.md` also states it as a rule. |
...
- **Title and headings** in sentence case (`# Release guide`, `## Key loading and failure`). Product and feature names keep their capitals. The checker flags headings that capitalize a small word ("Create An Automation") or three or more words; it can't tell a two-word Title Case heading from a product name, so check those by eye. Changing only case never changes an anchor.
...
- **Source-grounded.** Cite implementing files as `packages/<pkg>/src/...` paths, verify each one exists, and end every system page with `## Source` listing them. The architecture overview keeps `## Source map`. The checker resolves both linked and backticked repository paths, so a rename shows up as an error.
- **Written for readers, not for one agent run.** A page never addresses a single implementer ("You implement exactly one file", "keep it as stubbed", "bun is not installed here"). When a page describes guidance TurenOS gives its own agents, say so ("The default guidance tells the parent to…") instead of issuing the instructions.
- **Code samples compile.** Put `yield*` examples inside `Effect.gen(function* () { ... })`; Prettier rewrites a bare top-level `yield*` into `yield *`, which reads as multiplication.
...
- **Inbound links from outside `docs/`** break on every move. The mover rewrites the Markdown ones. This grep finds all of them while skipping external URLs and the `tools/` and `services/` areas, whose `docs/` links point at their own trees:
  ...
  ```
  Typical hits are the root `README.md`, `AGENTS.md`, `CONTRIBUTING.md`, package READMEs and scripts that read `docs/assets/`. Prompt text, `.forge/glossary` and test strings also match but aren't links. The checker resolves every `docs/` path and `#anchor` cited from Markdown outside `docs/` (READMEs, `AGENTS.md`, `CONTRIBUTING.md`), skipping fenced examples and `.forge/`, so a rename made without the mover shows up as an error. A Markdown link resolves relative to the file that holds it, as on GitHub, so a nested README writes `../../docs/...`; a bare or backticked mention may name the path from the repository root. Code comments often cite a page by its bare file name ("per `CONVENTIONS.md`"), which neither the mover nor the checker can resolve, so also run `git grep -n -F '<file name>'` for each moved page and update those comments.

## Workflow

1. **Survey.** Run the checker for a baseline. It enforces the tables and conventions above, plus the naming, link, anchor, orphan, folder-index and source-path rules from the practices. `--coverage` also lists workspace packages no page mentions:
   ```bash
   bun .agents/skills/turen-documentation/scripts/check.ts docs --coverage
   ```
   A clean run proves structure, not truth: it can't see a wrong default, a contradiction between two pages, or a system with no page. For those, follow [Auditing for confusion](references/practices.md#auditing-for-confusion).
...
3. **Restructure.** Build the full mapping (current path, new path, the rule that places it) and **show it to the user before moving anything.**
   ...
   3. Let the mover do the mechanical part. It prints the plan by default. With `--apply` it moves each file with `git mv`, so history follows it, and rewrites every relative link inside `docs/`, backticked path mentions, and `docs/` references in Markdown outside it. It also lists references in code and config for you to fix by hand:
      ...
      ```
   4. To break up a long page, move it into its own folder first, then split sections out. `split.ts` refuses duplicate heading slugs, whose `-1`/`-2` suffixes would change across the resulting pages; rename duplicate headings before splitting:
      ```bash
      bun .agents/skills/turen-documentation/scripts/split.ts docs/<section>/<page>/README.md \
        --into <file>.md --title "<Title>" --section "<heading text, without ##>" [--section ...] [--into ...]   # add --apply
      ```
   5. Write the section READMEs, add missing catalog rows and links, and update `docs/README.md`.
4. **Audit** (when asked, or after a large restructure). Follow [Auditing for confusion](references/practices.md#auditing-for-confusion) and report before editing: findings in severity order, each with `file:line`, what the page says, what the source shows, and a one-line fix.
5. **Verify.** Re-run the checker until it reports no errors or warnings you haven't decided on. Re-read changed pages against the branding rule, and confirm every moved or edited claim against the source or spec.
