---
name: turen-context
description: Audit, place and maintain the AGENTS.md files that coding agents load while developing TurenOS. Verifies every path and package script they cite, keeps each directory chain inside Codex's 32 KiB instruction budget, checks that parent files point to nested ones, flags stray @mentions and repeated lines, and proposes new AGENTS.md files only where the evidence says an area needs its own rules. Use it when adding, editing, splitting or moving any AGENTS.md, when an agent keeps missing a repo convention, when a package gains its own commands or gotchas, or for a periodic audit. Not for docs/ (use turen-documentation), and not for how the TurenOS product loads its users' instruction files.
---

# turen-context

`AGENTS.md` files are instructions for the coding agents (Codex, OpenCode, Claude Code, TurenOS and others) that developers use to build TurenOS. They are harness configuration: loaded into an agent's context automatically, paid for in every session, and followed as rules. They are not documentation. Run every command from the repository root; Bun is the only prerequisite.
...
| ----------- | ----------------------------------------------------------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------- |
| Codex       | root down to the working directory, concatenated, deeper files last                 | never                                | stops adding files once the chain reaches 32 KiB (`project_doc_max_bytes` default) |
| Claude Code | working directory and every folder above it (only when no `CLAUDE.md` exists there) | when it reads a file in that subtree | about 200 lines per file for good adherence                                        |
| OpenCode    | walks up from the working directory                                                 | not documented                       | none documented                                                                    |

TurenOS's own loader, inherited from OpenCode, stacks every ancestor file and attaches deeper ones when a file is read (`packages/forge/src/session/instruction.ts`). If a tool's behavior matters to a decision, re-check its current docs.
...
2. **A nested file holds only what is true in its subtree and not above it.** Restating the root costs each agent's budget twice and lets copies drift into contradictions.
3. **Every nested file is named by its parent**, with a line like "follow `tools/AGENTS.md` when working in `tools/`". A Codex session started above the folder never loads the nested file, so this line is how it learns the file exists.
...
5. **No `CLAUDE.md` unless it contains an `@AGENTS.md` line.** Claude Code reads no `AGENTS.md` at all in or below a folder that has a `CLAUDE.md`.
...
2. **Every claim is true.** `scripts/check.ts` proves paths, package scripts, links and `@` mentions. Verify the rest by reading the code: symbol names, flags and env vars (`rg` for them), versions (manifests and lockfiles), and invariants ("the runner reloads projected history before continuing"). Prefer fixing a stale claim to deleting it when the rule still applies.
